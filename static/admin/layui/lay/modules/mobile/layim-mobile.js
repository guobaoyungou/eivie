...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="'+ (options.index ? '{{'+ options.index +'}}' : '{{data.type}}{{data.id}}') +'" class="layim-'+ (options.type === 'history' ? '{{data.type}}' : options.type) +'{{data.id}} {{ data.status === "offline" ? "layim-list-gray" : "" }}"><div><img src="{{data.avatar}}"></div><span>{{ data.username||data.groupname||data.name||"暂无" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
    ...
    ,'<div class="layim-tab-content">'
      ...
      ,'<ul class="layim-list-friend">'
        ...
        //  ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
          ,'<ul class="layui-layim-list {{# if(true || spread === "true"){ }}'
          ,' layui-show'
          ,'{{# } }}">'
            ,listTpl({
              type: "friend"
  ...
  ,'<ul class="layui-unselect layui-layim-tab">'
    ,'<li style="width:25%" title="雷达" layim-event="tab" lay-type="leida" class="layim-this"><i class="layui-icon">&#xe665;</i><span>雷达</span><i class="layim-new" id="LAY_layimLeiDa"></i></li>'
    ,'<li style="width:25%" title="消息" layim-event="tab" lay-type="message"><i class="layui-icon">&#xe611;</i><span>消息</span><i class="layim-new" id="LAY_layimNewMsg"></i></li>'
    ,'<li style="width:25%" title="客户" layim-event="tab" lay-type="friend"><i class="layui-icon">&#xe770;</i><span>客户</span><i class="layim-new" id="LAY_layimNewList"></i></li>'
    ,'<li style="width:25%" title="我的" layim-event="tab" lay-type="more"><i class="layui-icon">&#xe612;</i><span>我的</span><i class="layim-new" id="LAY_layimNewMore"></i></li>'
  ,'</ul>'].join('');
  ...
  var pushChatlog = function(message){
    ...
    if(chatlog[message.type + message.id]){
			if(message.msgid){
				for(var i=0;i<chatlog[message.type + message.id].length;i++){
					if(chatlog[message.type + message.id][i].msgid == message.msgid){
						return;
					}
				}
			}
  ...
  'modules/layim/mobile/layim.css?v=2.00'
  ,'skinlayim-mobilecss'
);