        ...
        if(verify[thisVer]){
          ...
          if(isTrue){
                  ...
                  if(item.tagName.toLowerCase() === 'select' || /^checkbox|radio$/.test(item.type)){
                    return othis.next();
								...
								if(othis.attr('type') == 'hidden'){
									return othis.parents('.layui-form-item').find('#picPreview .layui-imgbox-img');
								}else{
									return othis;
								}
            ...
            if(!device.android && !device.ios)  item.focus(); //非移动设备自动定位焦点
						if(othis.attr('type') == 'hidden'){
							othis.parents('.layui-form-item').find('#picPreview .layui-imgbox-img').addClass('layui-form-dangeradmin')
							othis.parents('.layui-form-item').find('.layui-btn').focus()
						}else{
