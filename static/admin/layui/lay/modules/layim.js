...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="layim-chat-textarea"><textarea id="textarea-{{ d.data.type }}{{ d.data.id }}"></textarea></div>'
  ...
  var banRightMenu = function(){
	...
	//自定主面板联系人右键功能 --- 修改了主模板渲染 添加了 layui-friend 样式名
	layimMain.find('.layui-friend').on('contextmenu', 'li', function (e) {
		var othis = $(this);
		var html = '<ul data-id="' + othis.attr('class') + '" data-index="' + othis.data('index') + '">' +
  ...
  // 监听粘贴事件
  var listenPaste = function (data) {
      // 监听粘贴事件
      document.getElementById('textarea-' + data.type + data.id).addEventListener('paste', function(e){
          ...
          if (clipboardData) {
              ...
              if (item && item.kind === 'file' && item.type.match(/^image\//i)) {
                  ...
                  request.open('POST', '/index/upload/uploadBlobImg');
  ...
  var imgReader = function (item, id) {
      var blob = item.getAsFile(),
  ...
  'modules/layim/layim.css?v=3.50Pro'
  ,'skinlayimcss'
);