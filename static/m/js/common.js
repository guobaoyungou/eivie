...
function _editorFormSubmit(obj,formid,tourl){
	...
	for(var i in formdata){
		if($.inArray(formdata[i].name,formKeys) > -1){
			newformdata[formdata[i].name] = newformdata[formdata[i].name]+','+formdata[i].value;
		}else{
			formKeys.push(formdata[i].name);
...
function _tocart(proid,menuindex,isrefresh){
	...
	$.post('/m.php?s=shop/getproductdetail/aid/'+_getaid(),{id:proid},function(res){
		...
		html+='			</div>';
		html+='			<div class="op">';
		html+='				<button class="addcart" style="'+(guigelist[ks].stock <= 0 ? 'display:none' : '')+'">加入购物车</button>';
		html+='				<button class="tobuy" style="'+(guigelist[ks].stock <= 0 ? 'display:none' : '')+'">立刻购买</button>';
		html+='				<button class="nostock" style="'+(guigelist[ks].stock > 0 ? 'display:none' : '')+'">库存不足</button>';
		html+='			</div>';
		html+='	</div>';
		html+='</div>';
		$('#_ggnum').val('1')
		...
		$('#_buydialog .item2').click(function(){
			...
			if(guigelist[ks].stock > 0){
				$('#_buydialog .addcart').show();
				$('#_buydialog .tobuy').show();
				$('#_buydialog .nostock').hide();
			}else{
				$('#_buydialog .addcart').hide();
				$('#_buydialog .tobuy').hide();
				$('#_buydialog .nostock').show();
			}
		})
		//加入购物车
