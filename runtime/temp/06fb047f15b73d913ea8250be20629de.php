<?php /*a:4:{s:50:"/www/wwwroot/eivie/app/view/commission/record.html";i:1766974787;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title><?php echo t('佣金'); ?>记录</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> <?php echo t('佣金'); ?>记录<?php if(input('param.isopen')==1): ?><i class="layui-icon layui-icon-close" style="font-size:18px;font-weight:bold;cursor:pointer" onclick="closeself()"></i><?php endif; ?></div>
          <div class="layui-card-body" pad15>
			<div class="layui-col-md4" style="padding-bottom:10px;">
				...
				<button class="layui-btn layui-btn-primary layuiadmin-btn-list" data-form-export="<?php echo url('recordexcel'); ?>">导出EXCEL</button>
			</div>
				...
				<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<select name="status">
								...
								<option value="0" <?php if(input('param.status')==='0'): ?>selected<?php endif; ?>>未发放</option>
								<option value="1" <?php if(input('param.status')==='1'): ?>selected<?php endif; ?>>已发放</option>
							</select>
						</div>
				</div>
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'frommid', title: '<?php echo t('佣金'); ?>来源',templet:function(d){
				if(d.frommid && d.fromheadimg){
					return '<img src="'+d.fromheadimg+'" style="width:35px"> '+d.fromnickname;
				}else if(d.frommid){
					return d.frommid;
				}
				return '';
      }},
      {field: 'commission', title: '<?php echo t('佣金'); ?>',sort:true,width:120},
		  
			{field: 'score', title: '<?php echo t('积分'); ?>',sort:true,width:120},
		  <?php if(getcustom('commission_money_percent') || getcustom('yx_network_help')): ?>
		  {field: 'money', title: '<?php echo t('余额'); ?>',sort:true,width:120},
		  <?php endif; ?>
		  
		  
		  
		  
			{field: 'ordernum', title: '订单编号',width:160},
      {field: 'createtime', title: '产生时间',sort:true,templet:function(d){ return date('Y-m-d H:i:s',d.createtime)}},
      {field: 'endtime', title: '发放时间',sort:true,templet:function(d){ return date('Y-m-d H:i:s',d.endtime)}},
      ...
      {field: 'op', title: '操作',templet:function(d){
				var html = '';
		  <?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionOrder',$auth_data))): ?>
				if(d.type == 'shop'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('ShopOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'collage'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('CollageOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'kanjia'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('KanjiaOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'tuangou'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('TuangouOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'scoreshop'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('ScoreshopOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'luckycollage'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('LuckyCollageOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}

				if(d.type == 'yuyue'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('YuyueOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'kecheng'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('KechengOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'maidan'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('Maidan/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				
				if(d.type == 'restaurant_takeaway'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('RestaurantTakeawayOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				if(d.type == 'restaurant_shop'){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('RestaurantShopOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				}
				<?php if(getcustom('wx_channels')): ?>
				  if(d.type == 'channels'){
					  html += '<button class="table-btn" onclick="openmax(\'<?php echo url('WxChannelsOrder/index'); ?>&orderid='+d.orderid+'&isopen=1&showtype=all\')">查看订单</button>';
				  }
				<?php endif; ?>
				
			<?php endif; if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionDel',$auth_data))): ?>
	...
	function datadel(id){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('recorddel'); ?>",{ids:ids},function(data){
				layer.close(index);
