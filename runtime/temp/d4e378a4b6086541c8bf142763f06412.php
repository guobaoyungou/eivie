<?php /*a:4:{s:47:"/www/wwwroot/eivie/app/view/money/moneylog.html";i:1764308965;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title><?php echo t('余额'); ?>明细</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> <?php echo t('余额'); ?>明细
          ...
          <div class="layui-card-body" pad15>
						<div class="layui-col-md2" style="padding-bottom:10px">
							...
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" data-form-export="<?php echo url('moneylogexcel'); ?>">导出EXCEL</button>
						</div>
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'money', title: '变更金额',sort:true,width:120,templet:function(d){ if(d.money>0)return '<span style="color:#03bc01;font-size:18px">+'+d.money+'</span>';else return '<span style="color:#000;font-size:16px">'+d.money+'</span>';}},
      {field: 'after', title: '变更后金额',sort:true,width:120},
	  ...
	  {field: 'paytype', title: '支付方式',width:120,templet:function(d){
		  if(d.paytype=='wxpay')return '微信';
		  else if(d.paytype=='alipay')return '支付宝';
		  else if(d.paytype=='cash')return '现金';
		  else if(d.paytype=='bank')return '银行卡';
		  else return '';
	  }},
      ...
      {field: 'op', title: '操作',templet:function(d){
				var html = '';
				if(d.rechargeid){
					html += '<button class="table-btn" onclick="openmax(\'<?php echo url('Money/rechargelog'); ?>&isopen=1&id='+d.rechargeid+'\')">充值明细</button>';
				}
				
					<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('MoneylogDel',$auth_data))): ?>
		  ...
		  if(curr == 1){
			  $.post("<?php echo url('money_statistics'); ?>",datawhere,function(res){
				  $('#yuemoney').html('￥'+res.tongji.yue_money);
				  $('#givemoney').html('￥'+res.tongji.give_money);
				  $('#total_money').html(res.tongji.total_money ?? 0);
				  $('#total_money_today').html(res.tongji.total_money_today ?? 0);
				  $('#dec_money_today').html(res.tongji.dec_money_today ?? 0);
			  })
		  }
	  }
  });
	...
	function datadel(id){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('moneylogdel'); ?>",{ids:ids},function(data){
				layer.close(index);
