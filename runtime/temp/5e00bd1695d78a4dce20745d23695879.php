<?php /*a:4:{s:45:"/www/wwwroot/eivie/app/view/member/index.html";i:1766974796;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<!--						</div>-->
						<div class="layui-form layui-col-md12 layui-form-search">
							...
							<?php if(getcustom('fuwu_usercenter')): ?>
							<div class="layui-inline">
								<label class="layui-form-label"><?php echo t('服务中心'); ?></label>
								<div class="layui-input-inline">
									<input type="text" name="fuwu_name" autocomplete="off" class="layui-input">
								</div>
							</div>
							...
							<?php if(getcustom('mendian_usercenter')): ?>
							<div class="layui-inline layuiadmin-input-useradmin">
								<label class="layui-form-label">所属<?php echo t("门店"); ?>ID</label>
								<div class="layui-input-inline" style="width:80px">
									<input type="text" name="lock_mdid" autocomplete="off" class="layui-input" placeholder="锁定<?php echo t("门店"); ?>ID">
								</div>
							</div>
							...
							<?php endif; if(getcustom('member_unused_days')): ?>
								<div class="layui-inline layuiadmin-input-useradmin">
									<label class="layui-form-label">未消费天数</label>
									<div class="layui-input-inline" style="width:90px">
										<input type="number" name="unused_days" autocomplete="off" class="layui-input" placeholder="开始天数">
									</div>
									至
									<div class="layui-input-inline" style="width:90px">
										<input type="number" name="unused_days_end" autocomplete="off" class="layui-input" placeholder="结束天数">
									</div>
								</div>
							<?php endif; ?>
							<div class="layui-inline">
	...
	<div id="ywtimeModel" style="width:500px;display:none;margin-top:30px">
  	...
  	<div id="xianjinrechargeModel" style="width:500px;display:none;margin-top:30px">
			<div class="layui-form" lay-filter="">
				<input type="hidden" name="xinjinrechargemid" id="xinjinrechargemid"/>
				<div class="layui-form-item">
					<label class="layui-form-label">充值方式</label>
					<div class="layui-input-inline">
						<select name="rechargetype">
				...
				<div class="layui-form-item">
					<div class="layui-input-block">
						<button class="layui-btn layui-btn-normal" lay-submit lay-filter="formXinjinRecharge">确定充值</button>
					</div>
				</div>
			</div>
		</div>
  ...
  <script type="text/html" id="leftbtn">
	  <div class="layui-btn-container">
		  <?php if($is_fuwu==0): if(getcustom('member_add')): ?>
		  ...
		  <button class="layui-btn layui-btn-primary layuiadmin-btn-list" onclick="editLimitOverdraftmoney()">批量修改额度</button>
		  <?php endif; ?>
		  <?php endif; if(getcustom('pay_huifu') && ($auth_data=='all' || in_array('Member/clearHuifu',$auth_data))): ?><button class="layui-btn layui-btn-primary layuiadmin-btn-list" onclick="clearHuifu()">清除汇付进件信息</button><?php endif; ?>
		  <?php endif; ?>
	  </div>
  </script>
...
<script>
  ...
  var tableIns = table.render({
	...
	cols: [[ //表头
      ...
      {field: 'nickname', title: '推荐人',templet: function(d){
			  var html = '';
				...
				if(d.parent && d.parent.id){
					html= '<img src="'+d.parent.headimg+'" data-onerror="/static/img/touxiang.png" style="height:30px"/><br>'+d.parent.nickname;
					html += '<br><?php echo t('会员'); ?>ID：'+d.pid;
				}else{
					html= '无';
				}
			  if(d.parent_origin && d.parent_origin.id){
				  html += '<div><img src="'+d.parent_origin.headimg+'" data-onerror="/static/img/touxiang.png" style="height:30px"/><br>'+d.parent_origin.nickname+'(原推荐人)</div>';
			  }

			  return html;
			},width:70},
		<?php if(getcustom('fuwu_usercenter')): ?>
		{field: 'fuwu_name', title: '所属<?php echo t('服务中心'); ?>'},
		<?php endif; ?>
      {field: 'nickname', title: '头像昵称',templet:function(d){
			  	var html = '';
			  	if(d.headimg){
					html+='<img src="'+d.headimg+'" data-onerror="/static/img/touxiang.png" style="height:50px"/>';
				}
				 html += d.nickname;
				if(d.remark){
				...
				<?php if(getcustom('member_blocked')): ?>
				if(d.is_blocked == 1){
					html += '<br><span style="color:#f55">已拉黑</span>';
				}
				<?php endif; ?>
				return html;
			}},
			{field: 'realname', title: '会员信息',templet:function(d){
				...
				if(d.levelendtime){
					html+='<li>到&nbsp;&nbsp;期：'+date('Y-m-d',d.levelendtime)+'</li>';
				}
				
				<?php if(in_array('mp',$platform) && in_array('wx',$platform)): ?>
				html+='<li>来&nbsp;&nbsp;源：'+platform+'</li>';
				<?php endif; ?>
					html+='<li ><?php echo t('优惠券'); ?>：<a  href="javascript:void(0)"  onclick="openmax(\'<?php echo url('Coupon/record'); ?>&isopen=1&bid=all&mid='+d.id+'\')">'+d.coupon_count+'</a></li>';
				<?php if(in_array('mp',$platform)): ?>
					if(d.card_code){
				...
				<?php endif; if(getcustom('mendian_bind')): ?>
				if(d.bind_mendian > 0){
                    html+='<li>门店ID：'+d.bind_mendian+'</li>';
                }
                <?php endif; if(getcustom('member_show_is_business')): ?>
					html+='<li>是否开店：'+(d.member_is_business ==1 ?'是':'否') +'</li>';
                <?php endif; ?>
				html+' </ul>';
				return html;
			},width:155},
			...
			<?php endif; if(getcustom('deposit') && ($auth_data=='all' || in_array('DepositMember/*',$auth_data))): ?>
				{field: 'deposit_totalmoney', title: '<?php echo t('押金'); ?>',sort: sortdata,width:100,templet:function(d){
					return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('DepositMember/index'); ?>/isopen/1/mid/'+d.id+'\')">'+d.deposit_totalmoney+'</a>';
				}},
			<?php endif; ?>
			...
			<?php if(getcustom('member_total_amount')): ?>
			{field: 'totalmoney', title: '累计<?php echo t('余额'); ?>',width:100,templet:function(d){
					return d.totalmoney;
			}},
			<?php endif; if(getcustom('gold_bean')): ?>	
			{field: 'gold_bean', title: '<?php echo t('金豆'); ?>',sort: sortdata,width:80,templet:function(d){
				var html = '<ul style="line-height: 25px">';
				html+= '<li><?php echo t('金豆'); ?>：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('GoldBean/log'); ?>&isopen=1&mid='+d.id+'\')">'+d.gold_bean+'</a></li>';
				html+=' </ul>';
				return html;
			},width:150},
			<?php endif; ?>
			...
			<?php if(getcustom('member_total_amount')): ?>
			{field: 'totalscore', title: '累计<?php echo t('积分'); ?>',width:100,templet:function(d){
				return d.totalscore;
			}},
			<?php endif; ?>
			{field: 'commission', title: '<?php echo t('佣金'); ?>',sort: true,width:100,templet:function(d){
				...
				<?php endif; if(!getcustom('member_commission_max') && getcustom('show_commission_total')): ?>
					html+= '<li>累计：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/commissionlog'); ?>&isopen=1&mid='+d.id+'\')">'+d.totalcommission+'</a></li>';
			...
			<?php if(getcustom('member_total_amount')): ?>
			{field: 'totalcommission', title: '累计<?php echo t('佣金'); ?>',width:100,templet:function(d){
				return d.totalcommission;
			}},
			<?php endif; if(getcustom('member_business')): if($showbusiness): ?>
	  ...
	  <?php endif; if(getcustom('consumer_value_add') && ($auth_data=='all' || in_array('Consumer/*',$auth_data))): ?>
		  ...
		  <?php endif; if(getcustom('active_coin') && ($auth_data=='all' || in_array('ActiveCoin/*',$auth_data))): ?>
			  ...
			  <?php endif; if(getcustom('freeze_money') && ($auth_data=='all' || in_array('FreezeMoney/*',$auth_data))): ?>
		  {field: 'freezemoney', title: '<?php echo t('冻结资金'); ?>',sort: sortdata,width:100,templet:function(d){
			  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('FreezeMoney/log'); ?>&isopen=1&mid='+d.id+'\')">'+d.freezemoney+'</a>';
		  }},
		  <?php endif; if(getcustom('yx_network_help') && ($auth_data=='all' || in_array('NetworkHelp/*',$auth_data))): ?>
			  {field: 'help_score', title: '<?php echo t('互助积分'); ?>',sort: sortdata,width:100,templet:function(d){
				  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('NetworkHelp/helpscorelog'); ?>&isopen=1&mid='+d.id+'\')">'+d.help_score+'</a>';
			  }},
			  <?php endif; if(getcustom('commission_xianjin_percent') && ($auth_data=='all' || in_array('Commission/xianjinlog',$auth_data))): ?>
				  {field: 'xianjin', title: '<?php echo t('现金'); ?>',sort: sortdata,width:100,templet:function(d){
					  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/xianjinlog'); ?>&isopen=1&mid='+d.id+'\')">'+d.xianjin+'</a>';
				  }},
				  <?php endif; if(getcustom('yx_buyer_subsidy') && ($auth_data=='all' || in_array('Subsidy/*',$auth_data))): ?>
					  {field: 'subsidy_score', title: '<?php echo t('返现积分'); ?>',sort: sortdata,width:100,templet:function(d){
						  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Subsidy/score_log'); ?>&is_business=0&isopen=1&mid='+d.id+'\')">'+d.subsidy_score+'</a>';
					  }},
	  ...
	  <?php endif; if(getcustom('yx_team_yeji_fenhong')): ?>
		  {field: 'team_yeji', title: '团队业绩',width:100},
		  {field: 'self_yeji', title: '自身业绩',width:100},
		  ...
		  {field: 'gudong_max', title: '股东分红信息',templet:function(d){
			  ...
			  html+='<li>剩余股东分红：'+d.gudong_remain+'</li>';
			  html+' </ul>';
			  return html;
		  },width:150},	  
	  ...
	  <?php endif; if(getcustom('fenhong_gudong_huiben')): if($auth_data=='all' || in_array('gdfenhong_huiben',$auth_data)): ?>
		  ...
		  <?php endif; if($auth_data=='all' || in_array('gdfenhong_huiben_maximum',$auth_data)): ?>
			  {field: 'total_fenhong_huiben2', title: '累计<?php echo t('回本股东分红'); ?>',width:100,templet:function(d){
				  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/fenhonglog'); ?>&isopen=1&fenhong_remark=<?php echo t('回本股东分红'); ?>&mid='+d.id+'\')">'+d.total_fenhong_huiben2+'</a>';
			  }},
			  {field: 'huiben_maximum', title: '<?php echo t('回本股东分红额度'); ?>',width:100,templet:function(d){
				  return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/huiben_maximum'); ?>&isopen=1&fenhong_remark=<?php echo t('回本股东分红额度'); ?>&mid='+d.id+'\')">'+d.huiben_maximum+'</a>';
			  }},
			  <?php endif; ?>
	  <?php endif; if(getcustom('yx_cashback_multiply') && getcustom('yx_cashback_decmoney_lock')): ?>
		  {field: 'gudong_max', title: '<?php echo t('购物返现'); ?>数据',templet:function(d){
			  var html = '<ul>';
			  html+='<li>待返现余额：'+d.cashback_remain+'</li>';
			  html+='<li>冻结余额：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Cashback/cashback_locklog'); ?>&isopen=1&mid='+d.id+'\') ">'+d.cashback_lock+'</a></li>';
			  html+' </ul>';
			  return html;
		  },width:150},

		  <?php endif; if(getcustom('yx_new_score')  && ($auth_data=='all' || in_array('NewScore/*',$auth_data))): ?>
		  {field: 'new_score', title: '<?php echo t("新积分"); ?>数据',templet:function(d){
			  var html = '<ul>';
			  html+='<li><?php echo t("新积分"); ?>：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('NewScore/score_log'); ?>&isopen=1&mid='+d.id+'\') ">'+d.new_score+'</a></li>';
			  html+='<li><?php echo t("待返现额度"); ?>：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('NewScore/score_money_log'); ?>&isopen=1&mid='+d.id+'\') ">'+d.new_score_money+'</a></li>';
			  html+='<li><?php echo t("冻结额度"); ?>：<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('NewScore/score_lock_log'); ?>&isopen=1&mid='+d.id+'\') ">'+d.new_score_lock+'</a></li>';
			  html+' </ul>';
			  return html;
		  },width:150},
	  ...
	  <?php endif; if(getcustom('member_unused_days')): ?>
			{field: 'unused_days', title: '未消费天数',width:100},
		<?php endif; ?>
      ...
      {field: 'createtime', title: '加入时间',sort: true,templet:function(d){ return date('Y-m-d H:i',d.createtime)}},
			...
			<?php endif; if(getcustom('mendian_usercenter')): ?>
				{field: 'lock_mdid', title: '所属<?php echo t("门店"); ?>',templet:function(d){
					var html = '<ul>';
					if(d.lock_mdid == 0) return '无';
					html+='<li>ID：'+d.lock_mdid+'</li>';
					html+='<li>名称：'+d.lock_mendian+'</li>';
					html+' </ul>';
					return html;
				}},
      ...
      <?php endif; if(getcustom('team_minyeji_count')): ?>
		  ...
		  <?php endif; if(getcustom('fenhong_jiaquan_area')): ?>
			{field: 'areagxz', title: '区域贡献值',width:100,templet:function(d){
				return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Member/jqfenhonggxz'); ?>&isopen=1&type=1&mid='+d.id+'\')">'+d.areagxz+'</a>';
			}},
			<?php endif; if(getcustom('fenhong_jiaquan_gudong')): ?>
			{field: 'gdgxz', title: '股东贡献值',width:100,templet:function(d){
			return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Member/jqfenhonggxz'); ?>&isopen=1&type=2&mid='+d.id+'\')">'+d.gdgxz+'</a>';
			}},
		  ...
		  <?php endif; if(getcustom('member_upgradescore') && $memberupgradescoreauth): ?>
		  	{field: 'upgradescore', title: '<?php echo $text["升级积分"]; ?>',sort: sortdata,width:100,templet:function(d){
					return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Money/upgradescorelog'); ?>&isopen=1&mid='+d.id+'\')">'+d.upgradescore+'</a>';
	      }},
		  <?php endif; if(getcustom('member_forzengxcommission')): ?>
			{field: 'forzengxcommission', title: '冻结<?php echo t('佣金'); ?>',sort: true,width:100,templet:function(d){
				return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/forzengxcommissionlog'); ?>&isopen=1&mid='+d.id+'\')">'+d.forzengxcommission+'</a>';
			}},
			<?php endif; if(getcustom('commission_withdraw_limit')): ?>
			{field: 'commission_withdraw_limit', title: '<?php echo t('佣金提现额度'); ?>',width:120,templet:function(d){
				return d.commission_withdraw_limit_infinite==1 ? '无限' : d.commission_withdraw_limit;
			}},
			<?php endif; if(getcustom('teamfenhong_jichamoney')): ?>
			{field: 'teamfenhong_jichamoney', title: '<?php echo t('级差奖励'); ?>',width:120,templet:function(d){
				return '<a href="javascript:void(0)" onclick="openmax(\'<?php echo url('Commission/jichamoneylog'); ?>&isopen=1&mid='+d.id+'\')">'+d.jichamoney+'</a>';
			}},
			<?php endif; ?>
      {field: 'operation', title: '操作',templet: function(d){
				var html = '';
		  		<?php if($is_fuwu==1): ?>return html;<?php endif; if(getcustom('plug_luckycollage')): ?>
				...
				<?php endif; if(getcustom('gold_bean') && ($auth_data=='all' || in_array('addGoldBean',$auth_data))): ?>
				html += '<button class="table-btn" id="jf" onclick="checklock('+d.islock+',function(){addGoldBean('+d.id+')})">加<?php echo t('金豆'); ?></button>';
				<?php endif; if($freeze_show && $haveaddscoreAuth): ?>
				html += '<button class="table-btn" id="jf" onclick="checklock('+d.islock+',function(){addfreezescore('+d.id+')})">加冻结<?php echo t('积分'); ?></button>';
				...
				<?php endif; if(getcustom('member_upgradescore') && $haveaddupgradescoreAuth && $memberupgradescoreauth): ?>
					html += '<button class="table-btn" id="jf" onclick="checklock('+d.islock+',function(){addupgradescore('+d.id+')})">加<?php echo $text["升级积分"]; ?></button>';
				...
				html += '<button class="table-btn" onclick="checklock('+d.islock+',function(){openmax(\'<?php echo url('HotelOrder/index'); ?>/showtype/all/isopen/1/mid/'+d.id+'\')})"><?php echo $text['酒店']; ?>订单</button>';
				...
				<?php endif; ?>
				<?php endif; if(getcustom('commission_withdraw_limit') && ($auth_data=='all' || in_array('Member/addCommissionWithdrawLimit',$auth_data))): ?>
					if(d.commission_withdraw_limit_infinite==0){
						html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addCommissionWithdrawLimit('+d.id+')})">加<?php echo t('佣金提现额度'); ?></button>';
					}
				<?php endif; if(getcustom('commission_xianjin_percent') && ($auth_data=='all' || in_array('Member/addxianjin',$auth_data))): ?>
						html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){xianjinrecharge('+d.id+')})">充值<?php echo t('现金'); ?></button>';
					<?php endif; if(getcustom('plug_yuebao') && $adminset['open_yuebao']): ?>
				...
				<?php endif; if(getcustom('team_auth') && ($auth_data=='all' || in_array('MemberTeam/*',$auth_data))): ?>
				...
				<?php endif; if(getcustom('member_goldmoney_silvermoney')): if($showsilvermoney): ?>
						...
						<?php endif; ?>
				<?php endif; if(getcustom('freeze_money') && ($auth_data=='all' || in_array('FreezeMoney/*',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addfreezemoney('+d.id+')})">加<?php echo t('冻结资金'); ?></button>';
					<?php endif; if(getcustom('fenhong_gudong_huiben') && ($auth_data=='all' || in_array('gdfenhong_huiben',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addhuibenmaximum('+d.id+')})">加<?php echo t('回本股东分红额度'); ?></button>';
				...
				<?php endif; if(getcustom('form_tan')): ?>
					if(d.form_tan_id > 0){
						html += '<button class="table-btn"  onclick="openmax(\'<?php echo url('FormTan/detail'); ?>/isopen/1/id/'+d.form_tan_id+'\')">碳表单</button>';
					}
				<?php endif; if(getcustom('teamfenhong_jichamoney') && ($auth_data=='all' || in_array('Member/addjichamoney',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addjichamoney('+d.id+')})">加<?php echo t('级差奖励'); ?></button>';
				<?php endif; if(getcustom('fenhong_max') && getcustom('fenhong_max_add') && ($auth_data=='all' || in_array('Member/addfenhongmax',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addfenhongmax('+d.id+')})">加股东分红额度</button>';
					<?php endif; if(getcustom('deposit') && ($auth_data=='all' || in_array('DepositMember/*',$auth_data))): ?>
					html += '<button class="table-btn" id="cz"  onclick="checklock('+d.islock+',function(){openmax(\'<?php echo url('DepositMember/index'); ?>/isopen/1/mid/'+d.id+'\')})"><?php echo t('押金'); ?>记录</button>';
				<?php endif; if(getcustom('member_blocked') && ($auth_data=='all' || in_array('Member/setblocked',$auth_data))): ?>
					if(d.checkst == 1){
						if(d.is_blocked == 1){
							html+='<button class="table-btn" onclick="checklock('+d.islock+',function(){setblocked('+d.id+',0)})">取消拉黑</button>';
						}else{
							html+='<button class="table-btn" onclick="checklock('+d.islock+',function(){setblocked('+d.id+',1)})">拉黑</button>';
						}
					}
				<?php endif; if(getcustom('yx_network_help') && ($auth_data=='all' || in_array('NetworkHelp/*',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addscorehelp('+d.id+')})">加<?php echo t('互助积分'); ?></button>';
					<?php endif; if(getcustom('active_coin') && ($auth_data=='all' || in_array('ActiveCoin/*',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){addactivecoin('+d.id+')})">加<?php echo t('激活币'); ?></button>';
					<?php endif; if(($auth_data=='all' || in_array('Coupon/record',$auth_data))): ?>
					html += '<button class="table-btn" id="cz" onclick="checklock('+d.islock+',function(){openmax(\'<?php echo url('Coupon/record'); ?>/bid/all/from_member/1/mid/'+d.id+'\')})">优惠券</button>';
					<?php endif; ?>
				return html;
      },width:170}
    ]],done: function(res, curr, count){
        //头像加载失败处理
        $('img[data-onerror]').each(function () {
            const $img = $(this);
            const onError = () => {
                const src = $img.data('onerror');
                if ($img.attr('src') !== src) {
                    $img.attr('src', src).removeAttr('data-onerror');
                }
            };
            ...
            if (img.complete && img.naturalWidth === 0) {
                ...
                $img.off('error load').on('error', onError).on('load', () => $img.off('error'));
            }
        });
    }
  });
	...
	<?php if(getcustom('gold_bean')): ?>
	function addGoldBean(id){
		var html = '';
		html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
		html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
		html+='		<div class="layui-input-inline" style="width:200px">';
		html+='			<input type="text" id="addgoldbean" class="layui-input"/>';
		html+='		</div>';
		html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo t('金豆'); ?></div>';
		html+='	</div>';
		html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
		html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
		html+='		<div class="layui-input-inline" style="width:350px">';
		html+='			<input type="text" id="addgoldbeanremark" class="layui-input"/>';
		html+='		</div>';
		html+='	</div>';
		var addgoldbeanLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
			yes:function(){
				var index = layer.load();
				$.post("<?php echo url('addGoldBean'); ?>",{id:id,gold_bean:$('#addgoldbean').val(),remark:$('#addgoldbeanremark').val()},function(res){
					layer.close(index);
					dialog(res.msg,res.status);
					layer.close(addgoldbeanLayer);
					tableIns.reload()
				})
			}
		})
	...
	<?php endif; ?>
	function addfreezescore(id){
		var html = '';
		html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
		html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
		html+='		<div class="layui-input-inline" style="width:160px">';
		html+='			<input type="text" id="addfreezescorescore" class="layui-input"/>';
		html+='		</div>';
		html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除冻结<?php echo t('积分'); ?></div>';
		html+='	</div>';
		html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
		html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
		html+='		<div class="layui-input-inline" style="width:350px">';
		html+='			<input type="text" id="addfreezescoreremark" class="layui-input"/>';
		html+='		</div>';
		html+='	</div>';
		var addscoreLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
			yes:function(){
				var index = layer.load();
				$.post("<?php echo url('addfreezescore'); ?>",{id:id,score:$('#addfreezescorescore').val(),remark:$('#addfreezescoreremark').val()},function(res){
					layer.close(index);
					dialog(res.msg,res.status);
					layer.close(addscoreLayer);
					tableIns.reload()
  ...
  layui.form.on('submit(formRecharge)', function(obj){
	...
	function daoru(){
		...
		<?php elseif(getcustom('fenhong_max_add')): ?>
		html+='		<div class="layui-form-mid"> <a href="/static/demo_member_fenhong.xls">点击下载查看导入格式</a></div>';
		<?php else: ?>
  ...
  function huigui(id)
  {
	...
	<?php if(getcustom('member_goldmoney_silvermoney')): if($showgoldmoney): ?>
		...
		<?php endif; ?>
	<?php endif; if(getcustom('commission_withdraw_limit')): ?>
		function addCommissionWithdrawLimit(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:200px">';
			html+='			<input type="number" id="addCommissionWithdrawLimit" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux layui-clear" style="margin-left: 110px;">输入负值表示扣除<?php echo t('佣金提现额度'); ?></div>';
			html+='	</div>';
			var addCommissionWithdrawLimitLayer = layer.open({type:1,area:['600px','300px'],title:'<?php echo t('佣金提现额度'); ?>充值',content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addCommissionWithdrawLimit'); ?>",{id:id,addCommissionWithdrawLimit:$('#addCommissionWithdrawLimit').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addCommissionWithdrawLimitLayer);
						tableIns.reload()
					})
				}
			})
	...
	<?php endif; if(getcustom('commission_xianjin_percent')): ?>
	var xianjinrechargelayer;
	function xianjinrecharge(id){
		$('#xinjinrechargemid').val(id);
		xianjinrechargelayer = layer.open({type:1,area: ['500px', '380px'],title:'<?php echo t('现金'); ?>充值',content:$('#xianjinrechargeModel'),shadeClose:true})
	}
	//充值提交
  layui.form.on('submit(formXinjinRecharge)', function(obj){
		var index= layer.load();
	    $.post("<?php echo url('addxianjin'); ?>",obj.field,function(data){
			layer.close(index);
			dialog(data.msg,data.status,data.url);
			if(data.status==1){
				tableIns.reload({
			...
			layer.close(xianjinrechargelayer);
		})
  });
	<?php endif; if(getcustom('member_shopscore') && $membershopscoreauth): ?>
	...
	<?php endif; if(getcustom('member_upgradescore') && $memberupgradescoreauth): ?>
		function addupgradescore(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:200px">';
			html+='			<input type="text" id="addupgradescorescore" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo $text["升级积分"]; ?></div>';
			html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addupgradescoreremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addupgradescoreLayer = layer.open({type:1,area:['600px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addupgradescore'); ?>",{id:id,upgradescore:$('#addupgradescorescore').val(),remark:$('#addupgradescoreremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addupgradescoreLayer);
						tableIns.reload()
					})
				}
			})
	...
	<?php endif; ?>
	function clearHuifu(){
		layer.confirm('确定要清除所有会员汇付进件信息吗？清除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			//do something
			var index = layer.load();
			$.post("<?php echo url('clearHuifu'); ?>",{},function(data){
				layer.close(index);
				dialog(data.msg,data.status);
				tableIns.reload()
			})
		...
		function addfreezemoney(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:150px">';
			html+='			<input type="text" id="addfreezemoney" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo t('冻结资金'); ?></div>';
			html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addfreezemoneyremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addfreezemoneyLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addfreezemoney'); ?>",{id:id,freezemoney:$('#addfreezemoney').val(),remark:$('#addfreezemoneyremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addfreezemoneyLayer);
						tableIns.reload()
					})
				}
			})
		}
		<?php endif; if(getcustom('fenhong_gudong_huiben')): ?>
		function addhuibenmaximum(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:150px">';
			html+='			<input type="text" id="addhuibenmaximum" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo t('回本股东分红额度'); ?></div>';
			html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addhuibenmaximumremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addhuibenmaximumLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addhuibenmaximum'); ?>",{id:id,huibenmaximum:$('#addhuibenmaximum').val(),remark:$('#addhuibenmaximumremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addhuibenmaximumLayer);
						tableIns.reload()
					})
				}
			})
		...
		<?php endif; if(getcustom('teamfenhong_jichamoney')): ?>
		function addjichamoney(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:200px">';
			html+='			<input type="number" id="addjichamoney" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux layui-clear" style="margin-left: 110px;">输入负值表示扣除<?php echo t('级差奖励'); ?></div>';
			html+='	</div>';
			var addjichamoneyLayer = layer.open({type:1,area:['600px','300px'],title:'<?php echo t('级差奖励'); ?>充值',content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addjichamoney'); ?>",{id:id,addjichamoney:$('#addjichamoney').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addjichamoneyLayer);
						tableIns.reload()
					})
				}
			})
		}
	<?php endif; if(getcustom('fenhong_max') && getcustom('fenhong_max_add')): ?>
		function addfenhongmax(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:200px">';
			html+='			<input type="number" id="addfenhongmax" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux layui-clear" style="margin-left: 110px;">输入负值表示扣除分红额度</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addfenhongmaxremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addjichamoneyLayer = layer.open({type:1,area:['600px','400px'],title:'股东分红额度修改',content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('addfenhongmax'); ?>",{id:id,addfenhongmax:$('#addfenhongmax').val(),remark:$('#addfenhongmaxremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addjichamoneyLayer);
						tableIns.reload()
		...
		<?php endif; if(getcustom('member_blocked')): ?>
	function setblocked(id,st){
		layer.confirm('确定要'+(st==1?'拉黑':'取消拉黑')+'吗?',{icon: 7, title:'操作确认'}, function(index){
			var index = layer.load();
			$.post("<?php echo url('setblocked'); ?>",{id:id,st:st},function(res){
				layer.close(index);
				dialog(res.msg,res.status);
				tableIns.reload()
			})
	...
	<?php endif; if(getcustom('yx_network_help')): ?>
		function addscorehelp(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:100px">';
			html+='			<input type="text" id="addhelpscore" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo t('互助积分'); ?></div>';
			html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addhelpscoreremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addhelpscoreLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('NetworkHelp/addhelpscore'); ?>",{id:id,score:$('#addhelpscore').val(),remark:$('#addhelpscoreremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addhelpscoreLayer);
						tableIns.reload()
					})
				}
			})
		...
		function addactivecoin(id){
			var html = '';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">增加数量：</label>';
			html+='		<div class="layui-input-inline" style="width:100px">';
			html+='			<input type="text" id="addactivecoin" class="layui-input"/>';
			html+='		</div>';
			html+='		<div class="layui-form-mid layui-word-aux">输入负值表示扣除<?php echo t('激活币'); ?></div>';
			html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;margin-right:20px;">';
			html+='		<label class="layui-form-label" style="width:80px">备注信息：</label>';
			html+='		<div class="layui-input-inline" style="width:350px">';
			html+='			<input type="text" id="addactivecoinremark" class="layui-input"/>';
			html+='		</div>';
			html+='	</div>';
			var addhelpscoreLayer = layer.open({type:1,area:['500px','300px'],title:false,content:html,shadeClose:true,btn: ['确定', '取消'],
				yes:function(){
					var index = layer.load();
					$.post("<?php echo url('ActiveCoin/addactivecoin'); ?>",{id:id,score:$('#addactivecoin').val(),remark:$('#addactivecoinremark').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(addhelpscoreLayer);
						tableIns.reload()
