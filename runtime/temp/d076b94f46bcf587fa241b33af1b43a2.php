<?php /*a:3:{s:48:"/www/wwwroot/eivie/app/view/backstage/index.html";i:1768886477;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<link rel="stylesheet" type="text/css" href="/static/fonts/iconfont.css?v=20201218" media="all">
  <base target="mainFrame" />
	...
	.wb-subnav .menu-header{line-height: 50px;height:50px;/*border-top: 1px solid #f7f7f8;*/cursor: pointer;padding-left: 15px;white-space: nowrap;overflow: hidden;}
	...
	.shortcut-menu:hover {color: #000;}
	.more-shortcut-menu-box .shortcut-menu:hover {background-color: #f2f2f2;}
	.shortcut-menu-box #more-button {
		position: absolute;
		top: 17px;
		right: 0;
		font-size: 16px;
  ...
  <div id="LAY_app" class="layadmin-tabspage-none layadmin-side-shrink2">
    <div class="layui-layout layui-layout-admin">
      <div class="layui-header">
		...
		<?php if(($auth_data=='all' || in_array('ShortcutMenu/*',$auth_data)) && !empty($shortcutMenus)): ?>
		<ul class="layui-nav shortcut-menu-box" style="display: none;">
			<?php foreach($shortcutMenus as $menu): ?>
			<li class="layui-nav-item " lay-unselect>
				<a href="javascript:void(0)" class="shortcut-menu" onclick="gotoShortcutMenu('<?php echo url($menu['value']); ?>')" lay-text="<?php echo $menu['label']; ?>"><?php echo $menu['label']; ?></a>
			</li>
			<?php endforeach; ?>
			 <i id="more-button" class="layui-icon layui-icon-triangle-d hidden"></i>
		</ul>
		<?php endif; ?>
        <ul class="layui-nav layui-layout-right" lay-filter="layadmin-layout-right">
			<?php if($auth_data=='all' || in_array('ShortcutMenu/*',$auth_data)): ?>
			<li class="layui-nav-item shortcut-menu-settings" lay-unselect title="快捷菜单">
				<a href="javascript:void(0)" class="flexY" onclick="menuGoto('<?php echo url('ShortcutMenu/index'); ?>')" lay-text="快捷菜单">
				<i class="layui-icon layui-icon-set" style="font-size: 16px;top:0px"></i><?php if(empty($shortcutMenus)): ?>&nbsp;&nbsp;<span>快捷菜单</span><?php endif; ?>
				</a>
			</li>
			<?php endif; if($isadmin==2): ?>
	...
	<div class="more-shortcut-menu-box" style="display: none;"></div>
      <!-- 侧边菜单 -->
...
<script>
	...
	function adjustShortcutMenu() {
		...
		if (window.innerWidth < 420) {
			$('.shortcut-menu-settings').hide();
		} else {
			if ($('.shortcut-menu-settings').css('display') === 'none') {
				$('.shortcut-menu-settings').show();
			}
		}
		var leftPosition = parseFloat($('.layui-layout-left').css('width')) + parseFloat($('.layui-side-menu').css('width'));
		var shortcutWidth = parseFloat($('.layui-header').css('width'))
			- leftPosition - 50
			- parseFloat($('.layui-layout-right').css('width'));
		//shortcutWidth = Math.min(600, shortcutWidth);
		var container = $('.shortcut-menu-box');
		if (shortcutWidth<100) {
			container.hide();
			return;
		} else {
			container.show();
		}
		container.css("width", `${shortcutWidth}px`).css("left", `${leftPosition}px`);
		$('.more-shortcut-menu-box').css("width", `${shortcutWidth}px`).css("left", `${leftPosition}px`);
		var items = container.children('.shortcut-menu');
		var containerHeight = container.height();
		var scrollHeight = container.length > 0 ? container[0].scrollHeight : 0;

		// 按钮位置
		var buttonLeftPosition = leftPosition + shortcutWidth - 30;
		$('.show-more').css("left", `${buttonLeftPosition}px`);
		// 判断是否超出（超出表示内容被隐藏了）
	...
	function showClippedShortcutMenus() {
		var parent = $('.shortcut-menu-box');
		const pWidth  = parent.innerWidth();
		const pHeight = parent.innerHeight();

		parent.children().each(function () {
			const child = $(this);
			const pos = child.position();  // 相对父元素位置

			const right  = pos.left + child.outerWidth();
			const bottom = pos.top  + child.outerHeight();
			...
			if (isClipped && child.hasClass('layui-nav-item ')) {
				$('.more-shortcut-menu-box').append(child.children().clone());
			}
		});
		$('.more-shortcut-menu-box').show();
	}
	function hideClippedShortcutMenus() {
		$('.more-shortcut-menu-box').html('').hide();
	}
	let hideMoreShortcutMenuBox = false;
	function hideMoreButton() {
		if ($('#more-button').hasClass('shown')) {
			hideClippedShortcutMenus();
			$('#more-button').removeClass('shown').addClass('hidden').css('transform','');
			hideMoreShortcutMenuBox = false;
		}
	}
	function gotoShortcutMenu(url) {
		hideMoreButton();
		menuGoto(url);
	}
	$(function(){
		...
		$('.more-shortcut-menu-box,.shortcut-menu-box').on('mouseenter', function(e) {
			hideMoreShortcutMenuBox = false;
		})
		$('.more-shortcut-menu-box,.shortcut-menu-box').on('mouseleave', function(e) {
			hideMoreShortcutMenuBox = true;
			setTimeout(() => {
				if (hideMoreShortcutMenuBox) {
					hideMoreButton();
				}
			}, 300);
		})
		$('#more-button').on('mouseover', function() {
			if ($('#more-button').hasClass('hidden')) {
				showClippedShortcutMenus();
				$('#more-button').removeClass('hidden').addClass('shown').css('transform', 'rotate(180deg)');
			}
		})
		//低版本IE提示
