<?php /*a:4:{s:54:"/www/wwwroot/eivie/app/view/money/wx_transfer_log.html";i:1755842761;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
							...
							<div class="layui-inline">
								...
								<div class="layui-input-inline">
									<select name="state">
										...
										<?php foreach($state_arr as $state_code=>$state_name): ?>
										<option value="<?php echo $state_code; ?>" <?php if(input('param.state')===$state_code): ?>selected<?php endif; ?>><?php echo $state_name; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'op', title: '操作',templet:function(d){
		...
		if(d.data_tbl=='member_withdrawlog' && d.data_id){
			html += '<button class="table-btn" onclick="openmax(\'<?php echo url('Money/withdrawlog'); ?>&isopen=1&id='+d.data_id+'\')">提现记录</button>';
		}
		  if(d.data_tbl=='member_commission_withdrawlog' && d.data_id){
			  html += '<button class="table-btn" onclick="openmax(\'<?php echo url('Commission/withdrawlog'); ?>&isopen=1&id='+d.data_id+'\')">提现记录</button>';
		  }
		  if(d.data_tbl=='business_withdrawlog' && d.data_id){
			  html += '<button class="table-btn" onclick="openmax(\'<?php echo url('BusinessMoney/withdrawlog'); ?>&isopen=1&id='+d.data_id+'\')">提现记录</button>';
		  }
			  if(d.data_tbl=='yuyue_worker_withdrawlog' && d.data_id){
				  html += '<button class="table-btn" onclick="openmax(\'<?php echo url('YuyueMoney/withdrawlog'); ?>&isopen=1&id='+d.data_id+'\')">提现记录</button>';
			  }
			  if(d.data_tbl=='peisong_withdrawlog' && d.data_id){
				  html += '<button class="table-btn" onclick="openmax(\'<?php echo url('PeisongMoney/withdrawlog'); ?>&isopen=1&id='+d.data_id+'\')">提现记录</button>';
			  }
	...
	function cancel(id){
		layer.confirm('确定要撤销ID：'+id+'的转账吗？',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('wx_transfer_cancel'); ?>",{id:id},function(data){
				layer.close(index);
				dialog(data.msg,data.status);
				tableIns.reload()
			})
