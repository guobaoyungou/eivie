<?php /*a:4:{s:47:"/www/wwwroot/eivie/app/view/score/scorelog.html";i:1764308970;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title><?php echo t('积分'); ?>明细</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> <?php echo t('积分'); ?>明细
          ...
          <div class="layui-card-body" pad15>
						<div class="layui-col-md6" style="padding-bottom:10px">
							...
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" data-form-export="<?php echo url('scorelogexcel'); ?>">导出EXCEL</button>
						</div>
						<div class="layui-form layui-col-md6 layui-form-search">
							...
							<div class="layui-inline layuiadmin-input-useradmin">
								...
								<div class="layui-input-inline">
									<input type="text" name="remark" autocomplete="off" class="layui-input" value="">
								</div>
							</div>
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'score', title: '变更<?php echo t('积分'); ?>',sort:true,width:120,templet:function(d){ if(d.score>0)return '<span style="color:#03bc01;font-size:18px">+'+d.score+'</span>';else return '<span style="color:#000;font-size:16px">'+d.score+'</span>';}},
      {field: 'after', title: '变更后剩余',sort:true,width:120},
      {field: 'createtime', title: '变更时间',sort:true,templet:function(d){ return date('Y-m-d H:i:s',d.createtime)}},
      ...
      {field: 'op', title: '操作',templet:function(d) {
			  var html = '';
			  if(d.status != -1 && d.is_cancel != 1){
				  <?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('ScoreLogCancel',$auth_data))): ?>
			  	html += '<button class="table-btn" onclick="datacancel('+d.id+')">撤销</button>';
				  <?php endif; ?>
			  }
			  <?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('ScoreLogDel',$auth_data))): ?>
	  ...
	  ,done:function(res, curr, count){
			  if(curr == 1){
				  $.post("<?php echo url('score_statistics'); ?>",datawhere,function(res){
					  $('#total_score').html(res.total.total_score ?? 0);
					  $('#total_score_today').html(res.total.total_score_today ?? 0);
					  $('#dec_score').html(res.total.dec_score ?? 0);
				  })
			  }
		  }
  });
	...
	function datadel(id){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('scorelogdel'); ?>",{ids:ids},function(data){
				layer.close(index);
	...
	function datacancel(id){
		...
		layer.confirm('确定要撤销吗?',{icon: 7, title:'操作确认'}, function(index){
			//do something
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('cancel'); ?>",{ids:ids},function(data){
				layer.close(index);
				dialog(data.msg,data.status);
				tableIns.reload()
			})
	...
	function setst(id,st){
		...
		layer.confirm('确定要'+(st==0?'驳回':'确认')+'吗?',{icon: 7, title:'操作确认'}, function(index){
			//do something
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('moneylogsetst'); ?>",{ids:ids,st:st},function(data){
				layer.close(index);
				dialog(data.msg,data.status);
				tableIns.reload()
			})
