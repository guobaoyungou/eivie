<?php /*a:4:{s:57:"/www/wwwroot/eivie/app/view/commission/commissionlog.html";i:1764308935;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title><?php echo t('佣金'); ?>明细</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> <?php echo t('佣金'); ?>明细
          ...
          <div class="layui-card-body" pad15>
						<div class="layui-col-md4" style="padding-bottom:10px;">
							...
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" data-form-export="<?php echo url('commissionlogexcel'); ?>">导出EXCEL</button>
						</div>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'nickname', title: '<?php echo t('会员'); ?>信息',templet:'<div>{{# if(d.nickname){ }}<img src="{{d.headimg}}" style="width:50px"> {{d.nickname}}{{# } }}</div>'},
			 {field: 'frommid', title: '佣金来源',templet:function(d){
				if(d.frommid && d.fromnickname){
					return '<img src="'+d.fromheadimg+'" style="width:35px"> '+d.fromnickname;
				}
				return '';
      }},
      {field: 'commission', title: '变更金额',sort:true,width:120,templet:function(d){ if(d.commission>0)return '<span style="color:#03bc01;font-size:18px">+'+d.commission+'</span>';else return '<span style="color:#000;font-size:16px">'+d.commission+'</span>';}},
      ...
      {field: 'op', title: '操作',templet:function (d) {
			  var html ='';
			  <?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionDel',$auth_data))): ?>
			  html += '<div><button class="table-btn" onclick="datadel('+d.id+')">删除</button></div>';
			  <?php endif; ?>
			  return html;
		  },width:100}
    ]]
	  ,done:function(res, curr, count){
		  if(curr == 1){
			  $.post("<?php echo url('commission_statistics'); ?>",datawhere,function(res){
				  $('#total_commission').html(res.total.total_commission ?? 0);
				  $('#total_commission_today').html(res.total.total_commission_today ?? 0);
				  $('#total_withdraw').html(res.total.total_withdraw ?? 0);
			  })
		  }
	  }
  });
	...
	function datadel(id){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('commissionlogdel'); ?>",{ids:ids},function(data){
				layer.close(index);
	...
	function setst(id,st){
		layer.confirm('确定要'+(st==0?'驳回':'确认')+'吗?',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post('',{op:'setst',id:id,st:st},function(data){
				layer.close(index);
				dialog(data.msg,data.status);
				tableIns.reload()
			})
