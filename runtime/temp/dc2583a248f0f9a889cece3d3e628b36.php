<?php /*a:4:{s:55:"/www/wwwroot/eivie/app/view/commission/withdrawlog.html";i:1764308936;s:43:"/www/wwwroot/eivie/app/view/public/css.html";i:1764308967;s:42:"/www/wwwroot/eivie/app/view/public/js.html";i:1766974798;s:49:"/www/wwwroot/eivie/app/view/public/copyright.html";i:1648714895;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title><?php echo t('佣金'); ?>提现记录</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> <?php echo t('佣金'); ?>提现记录<?php if(input('param.isopen')==1): ?><i class="layui-icon layui-icon-close" style="font-size:18px;font-weight:bold;cursor:pointer" onclick="closeself()"></i><?php endif; ?></div>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'nickname', title: '<?php echo t('会员'); ?>信息',templet:'<div>{{# if(d.nickname){ }}<img src="{{d.headimg}}" style="width:50px"> {{d.nickname}}{{# } }}</div>'},
		  ...
		  <?php if(getcustom('business_fenxiao')){ ?>
		  {field: 'bid', title: '商户ID',width:80},
		  <?php } ?>
		  {field: 'txmoney', title: '提现金额',width:120},
      {field: 'money', title: '打款金额',sort:true,width:120,templet:function(d){
				var html = '';
				if(d.tomoney > 0){
					html += '打款金额:'+d.paymoney+'<br/>到余额:'+d.tomoney;
				}else{
					html += d.money;
	...
	<?php if(getcustom('product_givetongzheng')){ ?>
      ...
      {field: 'aliaccount', title: '收款账号',templet:function(d){ 
      	...
      	if(d.paytype=="支付宝" || d.paytype=="小额通支付宝" || d.paytype=="灵活薪支付宝"){
      		...
      		html += d.bankname+'<br>'+d.bankcarduser+' '+d.bankcardnum;
      ...
      {field: 'createtime', title: '提现时间',sort:true,templet:function(d){ return date('Y-m-d H:i:s',d.createtime)}},
	...
	return html;
          ...
          <?php if(getcustom('commission_withdraw_upload_invoice')){ ?>
          {field: 'invoice', title: '上传发票',width:80,templet:function(d){
              if(d.invoice){
                  return '<a href="'+d.invoice+'" target="_blank">下载</a>';
              }
              return '';
          }},
          <?php } ?>
      {field: 'operation', title: '操作',templet:function(d){ 
				var ht = '';
				if(d.status==0){
					<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawAudit',$auth_data))): ?>
					ht+= '<button class="table-btn" onclick="setst('+d.id+',1)">审核</button>';
					<?php endif; if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawReject',$auth_data))): ?>
					ht+= '<button class="table-btn" onclick="setst('+d.id+',2)">驳回</button>';
					<?php endif; ?>
				}
				if(d.status==1){
					<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawDakuan',$auth_data))): ?>
					ht+= '<button class="table-btn" onclick="setst('+d.id+',3)">改为已打款</button>';
					<?php endif; ?>
					if(d.paytype=='微信钱包'){
						<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawWxDakuan',$auth_data))): ?>
					...
					<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawReject',$auth_data))): ?>
					ht+= '<button class="table-btn" onclick="setst('+d.id+',2)">驳回</button>';
					<?php endif; ?>
				}
				if(d.status==2) {};
				if(d.status==3) {};
				<?php if(!getcustom('handle_auth') || ($auth_data=='all' || in_array('CommissionWithdrawDel',$auth_data))): ?>
