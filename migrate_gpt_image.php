<?php
/**
 * GPT-Image-2 (gpt-image-1) 模型接入 - 数据库迁移脚本
 * 
 * 功能：
 * 1. 激活 OpenAI 供应商（provider_id=5）
 * 2. 注册 GPT-Image-2 模型记录（model_code: gpt-image-1）
 * 
 * 执行方式: php migrate_gpt_image.php
 * 
 * @date 2026-04-27
 */
...
try {
    ...
    // input_schema: GPT-Image-1 支持的参数
    ...
    // capability_tags: 中英文双标签，支持全部6种能力类型
    ...
    $description = 'OpenAI GPT-Image-2（API模型名gpt-image-1），支持文生图和图片编辑。支持4种尺寸、4种质量等级、透明背景、多种输出格式(png/jpeg/webp)。图片编辑模式下支持传入参考图进行编辑。';

    if ($existing) {
        $stmt = $pdo->prepare("UPDATE `{$modelTable}` SET 
            `model_name` = 'GPT-Image-2', `model_version` = 'gpt-image-1',
            `description` = ?,
    ...
    echo "----------------------------------------\n";
    echo "\n✅ GPT-Image-2 模型注册迁移完成！\n";
    echo "\n⚠️ 注意：请在系统后台「API Key管理」中为 OpenAI 供应商配置有效的 API Key。\n";
    echo "========================================\n\n";
