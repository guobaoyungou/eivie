...
class WxPayRedPack extends WxPayDataBase
{
	...
	**/
	...
	public function SetOrderno($value){
		$this->notsignvalues['orderno'] = $value;
	}

	public function GetOrderno(){
		return $this->notsignvalues['orderno'];
	}

	public function IsOrdernoSet(){
		return array_key_exists('orderno', $this->notsignvalues);
	}
 ...
 *
 */
class WxPayRedPackResult extends WxPayDataBase
{
	public static function xmlToArray($xml){
		$obj = new self();
		return $obj->FromXml($xml);
        // return $obj->GetValues();
	}
	/**
	* 设置微信分配的公众账号ID
	...
	**/
	...
	{
		$this->values['send_time'] = $value;
	}

	public function GetSend_time()
	{
		return $this->values['send_time'];
	}

	public function IsSend_timeSet()
	{
		return array_key_exists('send_time', $this->values);
	}


	public function SetSend_listid($value)
	{
		$this->values['send_listid'] = $value;
	}

	public function GetSend_listid()
	{
		return $this->values['send_listid'];
	}

	public function IsSend_listidSet()
	{
		return array_key_exists('send_listid', $this->values);
	}
 ...
 * @date 2018.5.7
 */
class WxPayBizToCustomer extends WxPayDataBase
{
	var $notsignvalues=array();
	/**
 ...
 * @date 2018.5.7
 */
class WxPayBizToCustomerReturn extends WxPayDataBase
{
	var $notsignvalues=array();
	/**
	* 设置返回状态码
	...
	**/
	...
	//业务结果	 SUCCESS/FAIL，注意：当状态为FAIL时，存在业务结果未明确的情况，所以如果状态FAIL，请务必再请求一次查询接口[请务必关注错误代码（err_code字段），通过查询查询接口确认此次付款的结果。]，以确认此次付款的结果。
	public function SetResult_code($value){
		$this->values['result_code'] = $value;
	}
	...
	//商户订单号 商户订单号，需保持历史全局唯一性(只能是字母或者数字，不能包含有符号)
	public function SetPartner_trade_no($value){
		$this->values['partner_trade_no'] = $value;
	}
	public function GetPartner_trade_no(){
	...
	public function SetPayment_no($value){
		$this->values['payment_no'] = $value;
	}
	public function GetPayment_no(){
		return $this->values['payment_no'];
	}
	public function IsPayment_noSet(){
		return array_key_exists('payment_no', $this->values);
	}

	//微信支付成功时间 企业付款成功时间
	public function SetPayment_time($value){
		$this->values['payment_time'] = $value;
	}
	public function GetPayment_time(){
		return $this->values['payment_time'];
	}
	public function IsPayment_timeSet(){
		return array_key_exists('payment_time', $this->values);
	}
