...
//        $bizcontent['passback_params'] = urlencode($aid.':'.$tablename.':app');
     ...
     * @return array
     */
    public static function build_page_pay($aid, $bid, $mid, $title, $ordernum, $price, $tablename, $notify_url = '', $return_url = ''){
        if(!$notify_url) $notify_url = PRE_URL . '/notify.php';
        ...
        writeLog(json_encode(['pc_alipay_page_pay_request' => $bizcontent]));

        try {
            // 使用pageExecute获取HTML自动提交表单（POST方式生成表单HTML）
            $result = $aop->pageExecute($request);

            if(!empty($result)){
                writeLog('pc_alipay_page_pay_success: form_html generated');
                return ['status'=>1,'msg'=>'success','data'=>['form_html'=>$result, 'pay_method'=>'form']];
            } else {
                writeLog('pc_alipay_page_pay_fail: empty result');
                // 降级为当面付二维码模式
                return self::build_precreate($aid, $bid, $mid, $title, $ordernum, $price, $tablename, $notify_url, 'pc');
            }
        } catch(\Exception $e){
            writeLog('pc_alipay_page_pay_exception: ' . $e->getMessage());
            // 降级为当面付二维码模式
            return self::build_precreate($aid, $bid, $mid, $title, $ordernum, $price, $tablename, $notify_url, 'pc');
        }
    }
