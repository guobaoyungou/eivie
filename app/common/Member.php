...
//                        var_dump($downmidchildre_number);
            ...
            if(getcustom('up_fxorder_condition_new',$aid)){
                ...
                if($lv['up_fxdowncount_new']>0 || $lv['up_fxdowncount2_new']>0 || $lv['up_fxdowncount3_new']>0 || $lv['up_fxdowncount4_new']>0){
                    ...
                    if($up_fxdowncount2>0){
                        $isup = safeNumericComparison($isup_int, $logic_str2, $downmidcount2, $up_fxdowncount2);
                        //dump('逻辑2判断',$isup);
                        $isup_int = $isup==true?1:0;
                    }
                    if($up_fxdowncount3>0){
                        $isup = safeNumericComparison($isup_int, $logic_str3, $downmidcount3, $up_fxdowncount3);
                        //dump('逻辑3判断',$isup);
                        $isup_int = $isup==true?1:0;
                    }
                    if($up_fxdowncount4>0){
                        $isup = safeNumericComparison($isup_int, $logic_str4, $downmidcount4, $up_fxdowncount4);
                        //dump('逻辑4判断',$isup);
                    }
            ...
            if(getcustom('up_level_teamorder',$aid)){
                ...
                if($lv['up_teamorder_num']>0 || $lv['up_teamorder_small_num']>0){
                    ...
                    $isup = safeNumericComparison($isup_int, $logic_str, $teamorder_count, $lv['up_teamorder_num']);
                    //dump('团队订单判断'.$isup_int);
                    //判断团队小区订单逻辑是否符合
                    $isup_int = $isup==true?1:0;
                    $isup = safeNumericComparison($isup_int, $logic_str2, $teamorder_small_count, $lv['up_teamorder_small_num']);
            ...
            if(getcustom('levelup_teamnum_peoplenum',$aid)){
                if($lv['up_team_path_num'] >0){
                   ...
                   $teammids =\app\common\Member::getdownmids($aid,$mid,1,0);
                    ...
                    $isup_int = $isup==true?1:0;
                    $isup = safeNumericComparison($isup_int, $logic_fh, $team_path_count, $lv['up_team_path_num']);
                }
            }
            if(getcustom('member_up_binding_tel',$aid)){
                if($lv['up_binding_tel'] ==1){
                    ...
                    $isup = safeNumericComparison($isup_int, $logic_btel, $is_tel, 1);
                }
            }
            ...
            if(getcustom('levelup_changepid_yeji',$aid)){
                ...
                if($lv['levelup_changepid_yeji']>0){
                    ...
                    $isup_int = $isup==true?1:0;
                    $isup = safeNumericComparison($isup_int, $logic_str, $changepid_yeji, $lv['levelup_changepid_yeji']);
                }
            }
            if(getcustom('levelup_team_yeji_front',$aid)){
                ...
                if($lv['levelup_team_yeji_front_yeji']>0){
                    ...
                    if($downmids){
                        ...
                        $isup_int = $isup==true?1:0;
                        $isup = safeNumericComparison($isup_int, $logic_str, $front_yeji, $lv['levelup_team_yeji_front_yeji']);
                    }
                }
            }
            if($isup) $newlv = $lv;
...
class Member
{
    ...
    public static function uplv($aid,$mid,$type='shop',$params = []){
        ...
        if($minfo){
            ...
            $member_tag_levelup = Db::name('config')->where('aid',$aid)->where('name','member_tag_levelup')->value('value');
            $member_tag_levelup = $member_tag_levelup??0;

            $tags_arr = explode(',',$minfo['tags']);
            $tags_arr = array_filter($tags_arr);

            $tag_ages = Db::name('member_tag')->where('aid',$aid)->where('status',1)->select()->toArray();
            foreach($tag_ages as $tag_age){
                $apply_give_tagid = 0;
                if($tag_age['tag_age_type'] == 0){
                    ...
                    if(!in_array($tag_age['id'],$tags_arr)){
                        ...
                        if($member_tag_levelup && $tag_age['type'] == 3 && $tag_age['up_levelid'] > 0 && $tag_age_level_sort > $member_level_sort){
                            ...
                            if($tag_age['up_levelid'] != $minfo['levelid']){
                                ...
                                Db::name('member_levelup')->insert($order);
                    ...
                    //条件标签
                    $tag_age['condition'] = json_decode($tag_age['condition'],true);
                    $tag_age['condition'] = $tag_age['condition']??[];
                    $tag_age['condition'] = array_filter($tag_age['condition']);

                    $tag_age['condition'] = array_map(function($item){
                        $item['left'] = $item['left']??'';
                    ...
                    $tag_age['condition'] = array_filter($tag_age['condition'],function($item){
                        return $item['left'] && $item['operator'] && $item['middle'] && $item['right'];
                    });

                    $tag_age['condition'] = array_map(function($item){
                        $item['left'] = Db::name('member')->where('aid',$aid)->where('id',$mid)->value($item['left']);
                    ...
                    $tag_age['condition'] = array_map(function($item){
                        return self::safeNumericComparison($item['left'],$item['operator'],$item['middle'],$item['right']);
                    },$tag_age['condition']);

                    $tag_age['condition'] = array_filter($tag_age['condition']);

                    if(count($tag_age['condition']) > 0){
                        if(!in_array($tag_age['id'],$tags_arr)){
                            Db::name('member')->where('aid',$aid)->where('id',$mid)->update(['tags' => $minfo['tags'].','.$tag_age['id']]);
                            $apply_give_tagid = $tag_age['id'];
