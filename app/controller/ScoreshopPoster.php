...
class ScoreshopPoster extends Common
{
    ...
    public function index(){
		...
		$posterset = Db::name('admin_set_poster')->where('aid',aid)->where('type','scoreshop')->where('platform',$type)->order('id')->find();
		if(!$posterset){
			...
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'mp','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'wx','content'=>$data_product_wx]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'alipay','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'baidu','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'toutiao','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'qq','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'h5','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'scoreshop','platform'=>'app','content'=>$data_product_mp]);
			$posterset = Db::name('admin_set_poster')->where('aid',aid)->where('type','scoreshop')->where('platform',$type)->order('id')->find();
		}
	...
	public function save(){
		...
		if(input('post.clearhistory') == 1){
			Db::name('member_poster')->where('aid',aid)->where('type','scoreshop')->where('posterid',$posterset['id'])->delete();
			$msg = '保存成功';
		}else{
			$msg ='保存成功';
		}
