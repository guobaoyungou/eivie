...
//			$where[] = ['maidan_order.bid','=',bid];
			...
			if(input('ogid')){
                $where[] = ['maidan_order.id','=',input('ogid')];
            }
			...
			foreach($data as $k=>$v){
				...
				$data[$k]['un'] = '';
				if($v['uid']){
					$data[$k]['un'] = $userArr[$v['uid']]??'已失效';
				}
                $data[$k]['can_refund_money'] = max(round($v['paymoney'] - $v['refund_money'],2),0);
    ...
    public function wifiprint(){
        $id = input('post.id/d');
        $rs = \app\common\Wifiprint::print(aid,'maidan',$id,0);
        return json($rs);
    }
        ...
        if (request()->isPost()) {
            ...
            if ($rs && $rs['status'] == 1) {
                ...
                if($order['bid'] > 0){
                    ...
                    if($order['paytypeid'] ==2 ){//微信支付
                        $add_business_money = true;
                    }elseif ($order['paytypeid'] ==3 ){//支付宝
                        $add_business_money = true;
                    }
                    //todo 收银台退款 扣除佣金
    ...
    public function businessorderlist(){
        ...
        $mdArr = Db::name('mendian')->where($whereMd)->column('name','id');
        if(request()->isAjax()){
            $page = input('param.page');
            $limit = input('param.limit');
            if(input('param.field') && input('param.order')){
            ...
            $where_status[] = ['maidan_order.status','=',1];
            $where[] = ['maidan_order.paytype','<>','null'];
            $status = input('param.status');
            $transfer_check = input('param.transfer_check');
            if(isset($transfer_check) && $transfer_check != '') {
                if($transfer_check == 0) {
                    $where[] = ['maidan_order.transfer_check','=',$transfer_check];
                    $where[] = ['maidan_order.paytypeid','=',5];
                    $where[] = ['maidan_order.paytype','<>','随行付支付'];
                }else{
                    $where[] = ['maidan_order.transfer_check','=',$transfer_check];
                }
            }
            ...
            foreach($data as $k=>$v){
                ...
                $data[$k]['refund_reason'] = Db::name('maidan_refund_order')->where('orderid',$v['id'])->value('refund_reason');
                $data[$k]['payorder'] = Db::name('payorder')->where('aid',aid)->where('orderid',$v['id'])->where('type','maidan')->find();
                $data[$k]['maidan_transfer'] = true;
                $data[$k]['payorder_check_status'] = $data[$k]['payorder']['check_status'];

                if($v['paytypeid'] == 5 && $v['paytype'] != '随行付支付'){
                    if($v['transfer_check'] == 1){
                        if($data[$k]['payorder_check_status'] == 2){
                            $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:#FC5531">凭证被驳回</span>';
                        }else if($data[$k]['payorder_check_status'] == 1){
                            $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:#FC5531">审核通过</span>';
                        }else{
                            if($data[$k]['payorder']['paypics']){
                                $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:orange">凭证待审核</span>';
                            }else{
                                $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:orange">凭证待上传</span>';
                            }
                        }
                    }else if($v['transfer_check'] == -1){
                        $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:red">已驳回</span>';
                    }else {
                        $data[$k]['paytype'] = $v['paytype'].'<br/><span style="color:#888">待审核</span>';
                    }
                }
            }
            return json(['code'=>0,'msg'=>'查询成功','count'=>$count,'data'=>$data]);
    ...
    public function getrechargeorderdetail()
    {
        ...
        $order = Db::name('maidan_order')->where('aid',aid)->where('id',$orderid)->find();
        $payorder = Db::name('payorder')->where('id',$order['payorderid'])->where('type','maidan')->where('aid',aid)->find();
        if($order['paytypeid'] == 5) {
            ...
            if($order['contract_pic']){
                $order['contract_pic'] = explode(',', $order['contract_pic']);
                foreach ($order['contract_pic'] as $item) {
                    $order['contract_pic_html'] .= '<img src="'.$item.'" style="width:200px;height:200px" onclick="preview(this)"/>';
                }
            }

            if($order['payment_voucher_pic']){
                $order['payment_voucher_pic'] = explode(',', $order['payment_voucher_pic']);
                foreach ($order['payment_voucher_pic'] as $item) {
                    $order['payment_voucher_pic_html'] .= '<img src="'.$item.'" style="width:200px;height:200px" onclick="preview(this)"/>';
                }
            }

            if($order['invoice_pic']){
                $order['invoice_pic'] = explode(',', $order['invoice_pic']);
                foreach ($order['invoice_pic'] as $item) {
                    $order['invoice_pic_html'] .= '<img src="'.$item.'" style="width:200px;height:200px" onclick="preview(this)"/>';
                }
            }

            if($order['delivery_voucher_pic']){
                $order['delivery_voucher_pic'] = explode(',', $order['delivery_voucher_pic']);
                foreach ($order['delivery_voucher_pic'] as $item) {
                    $order['delivery_voucher_pic_html'] .= '<img src="'.$item.'" style="width:200px;height:200px" onclick="preview(this)"/>';
                }
            }
        }
