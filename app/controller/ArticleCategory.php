...
class ArticleCategory extends Common
{
    ...
    public function index(){
		if(request()->isAjax()){
			...
			$where[] = ['bid','=',bid];
			$data = [];
			$cate0 = Db::name('article_category')->where($where)->where('pid',0)->order($order)->select()->toArray();
			foreach($cate0 as $c0){
				$data[] = $c0;
				$cate1 = Db::name('article_category')->where($where)->where('pid',$c0['id'])->order($order)->select()->toArray();
