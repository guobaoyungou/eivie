...
class LuckyCollageOrder extends Common
{
	...
	public function getdetail(){
		...
		$where[] = ['bid','=', bid];
		$where[] = ['id', '=', $orderid];
		$order = Db::name('lucky_collage_order')->where($where)->find();
		if($order['coupon_rid']){
	...
	public function sendExpress(){
		...
		\app\common\System::plog(t('幸运拼团').'订单发货'.$orderid);
		return json(['status'=>1,'msg'=>'操作成功']);
	}
	...
	public function plfh(){
		...
		\app\common\System::plog(t('幸运拼团').'订单批量发货');
    ...
    function orderHexiao(){
        ...
        Db::name('lucky_collage_order')->where($where)->update(['status'=>3,'collect_time'=>time()]);
        ...
        \app\common\System::plog(t('幸运拼团').'订单核销确认收货'.$orderid);
	...
	function orderCollect(){
		...
		\app\common\Member::uplv(aid,$order['mid']);
		\app\common\System::plog(t('幸运拼团').'幸运拼团订单确认收货'.$orderid);
