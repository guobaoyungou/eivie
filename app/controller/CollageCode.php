...
class CollageCode extends Common
{
	...
	public function codelist(){
		if(request()->isAjax()){
			...
			$count = 0 + Db::name('collage_codelist')->where($where)->count();
			$data = Db::name('collage_codelist')->where($where)->page($page,$limit)->order($order)->select()->toArray();
			//foreach($data as $k=>$v){
	...
	public function importexcel(){
		...
		foreach($exceldata as $data){
			...
			$hasinfo = Db::name('collage_codelist')->where($indata)->find();
			if($hasinfo){
				$chongfunum++;
			}else{
				$indata['createtime'] = time();
				Db::name('collage_codelist')->insert($indata);
				$insertnum++;
			}
		}
	...
	public function makecode(){
		...
		for($i=0;$i<$makecount;$i++){
			...
			$hasinfo = Db::name('collage_codelist')->where($data)->find();
			if(!$hasinfo){
				$data['createtime'] = time();
				Db::name('collage_codelist')->insert($data);
				$successnum++;
			}
		}
	...
	public function codelistexcel(){
		...
		$list = Db::name('collage_codelist')->where($where)->select()->toArray();
	...
	public function codelistdel(){
		...
		Db::name('collage_codelist')->where('aid',aid)->where('bid',bid)->where('id','in',$ids)->delete();
