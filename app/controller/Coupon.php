...
class Coupon extends Common
{
	...
	public function index(){
		...
		if(request()->isAjax()){
			...
			if(!$type){
			    if(!input('param.module') || input('param.module') !='registergive'){
                    $where[] = ['type','<>',5];//餐饮类排除
                }
	...
	public function edit(){
        ...
        //预览效果
        $data = jsonEncode($info,true);
        View::assign('type',$info['type'] ?? 1);
	...
	public function save(){
        ...
        if($info['id']){
            $oldcoupon = Db::name('coupon')->where('id',$info['id'])->where('aid',aid)->where('bid',bid)->find();
            if(!$oldcoupon){
                return json(['status'=>0,'msg'=>t('优惠券').'不存在']);
            }
	...
	public function recordexcel(){
		...
		$title[] = '领取时间';
		$title[] = '到期时间';
		if($list[0]['type'] == 3 || input('param.from_member'))
		    $title[] = '总次数/已使用';
		$title[] = '使用时间';
		$title[] = '状态';
        $title[] = '备注';
		...
		foreach($list as $v){
            ...
            if (input('param.from_member')) {
                //商家
                if($v['bid']>0){
                    $tdata[] = Db::name('business')->where('aid', aid)->where('id', $v['bid'])->value('name');
                }else{
                    $tdata[] = '平台';
                }
            }
	...
	public function recordsetst(){
		...
		$rlist = Db::name('coupon_record')->where('aid',aid)->where('id','in',$ids)->select()->toArray();
		foreach($rlist as $k=>$v){
            if($v['bid'] != bid){
                return json(['status'=>0,'msg'=>'不支持修改商户优惠券状态']);
            }
			if($v['type']==3){
				return json(['status'=>0,'msg'=>'计次券不能修改状态']);
			}
            if($v['type']==20){
                return json(['status'=>0,'msg'=>'券包不能修改状态']);
            }
	...
	public function decCouponOne(){
		$rid = input('post.id/d');
        //操作类型 1：计次券减少一次 2：券包增加优惠券减少一张
        $opttype = input('?post.opttype')?input('post.opttype/d'):1;
        ...
        $where[] = ['aid','=',aid];
        $where[] = ['bid','=',bid];
		$cInfo = Db::name('coupon_record')->where($where)->find();
		if (!$cInfo) {
			return json(['status'=>0,'msg'=>'没有找到'.t('优惠券').'信息']);
		}
        if($opttype == 1){
            ...
            if($cInfo['num']<=0 || $cInfo['num'] <= $cInfo['usenum']) {
                return json(['status'=>0,'msg'=>'已使用全部张数']);
            }
            Db::name('coupon_record')->where('aid',aid)->where('bid',bid)->where('id',$rid)->inc('usenum')->update();
        }
        ...
        if($opttype == 1){
            ...
            if($cInfo['usenum']+1>=$cInfo['num']){
                Db::name('coupon_record')->where('id',$rid)->update(['status'=>1,'usetime'=>time()]);
                \app\common\Wechat::updatemembercard(aid,$cInfo['mid']);
            }
            \app\common\System::plog(t('优惠券').'减一张'.$rid);
        }
	...
	public function recorddel(){
		$ids = input('post.ids/a');
        $list = Db::name('coupon_record')->where('aid',aid)->where('id','in',$ids)->select()->toArray();
        if(empty($list)) return json(['status'=>0,'msg'=>'记录不存在或已被删除']);
        foreach ($list as $key=>$val){
            if($val['bid'] != bid){
                return json(['status'=>0,'msg'=>'不支持删除商户优惠券记录']);
            }
        }
    ...
    public function set(){
        if(request()->isPost()){
            ...
            $info['aid'] = aid;
            $info['bid'] = bid;
            $expire_rules = $info['expire_rules']??[];
            $info['expire_rules'] = $expire_rules?implode(',',$info['expire_rules']):'';
