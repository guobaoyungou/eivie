...
class CyclePoster extends Common
{
    ...
    public function index(){
        ...
        $field = input('param.field') ? input('param.field') : 'cycle';
        ...
        if(!$posterset){
            ...
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'mp','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'wx','content'=>$data_product_wx]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'alipay','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'baidu','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'toutiao','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'qq','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'h5','content'=>$data_product_mp]);
            Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'cycle','platform'=>'app','content'=>$data_product_mp]);
            $posterset = Db::name('admin_set_poster')->where('aid',aid)->where('type','cycle')->where('platform',$type)->order('id')->find();
        }
	...
	public function save(){
		...
		Db::name('admin_set_poster')->where('id',$posterset['id'])->update(['content'=>json_encode($data_index,JSON_UNESCAPED_UNICODE)]);
		...
		\app\common\System::plog('编辑周期分享海报');
