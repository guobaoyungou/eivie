...
class ApiRestaurantBooking extends ApiCommon
{
    public function add()
    {
...
//        $bid = $bid?$bid:bid;
        ...
        if(request()->isPost()){
            ...
            // 时间限制
            if(empty($info['time'])){
                return json(['status'=>0,'msg'=>'请选择预约时间']);
            }
            $yxqtime = explode('~',$info['time']);
            $book_starttime = $yxqtime[0];
            $book_starttime = strtotime($book_starttime);
            if($book_starttime < time() + 3600*$set['prehour']){
                return json(['status'=>0,'msg'=>'请重新选择预约时间']);
            }
            $insert = [
                'aid' => aid,
                'bid' => $bid,
            ...
            $where_book[] = ['bid','=',$bid];
            $where_book[] = ['mid','=',mid];
            $where_book[] = ['tableid','=',$info['tableId']];
            $where_book[] = ['booking_time','=',$info['time']];
            $where_book[] = ['status','=',0];
            $restaurant_booking_order = Db::name('restaurant_booking_order')->where($where_book)->find();
            if($restaurant_booking_order){
                Db::name('restaurant_booking_order')->where('aid',aid)->where('id',$restaurant_booking_order['id'])->update($insert);
                $orderid = $restaurant_booking_order['id'];
            }else{
...
//            $rs = \app\common\Sms::send(aid,$member['tel']?$member['tel']:$insert['tel'],'tmpl_restaurant_booking',['restaurant_name'=>$business['name'], 'table' => $tableName, 'time' => $insert['booking_time']]);
            ...
            return $this->json(['status'=>1,'msg' => $insert['totalprice'] > 0 ? '预定成功，请支付' : '预定成功','id'=>$orderid, 'payorderid' => $payorderid,'tableid'=>$info['tableId']]);
        }
        ...
        $is_book_order = 0;
        // 预约可点餐
        return $this->json(['status'=>1,'timeArr'=>$pstimeArr, 'table' => $table,'restaurant_book_date'=>$restaurant_book_date,'textset'=>$textset,'is_book_order'=>$is_book_order]);
    }
...
//        $bid = $bid?$bid:bid;
        ...
        if(input('param.time')){
            ...
            $whereBooking[] = ['status', 'in', [0,1,2,3]];
...
//        $bid = $bid?$bid:bid;
    ...
    public function detail()
    {
        ...
        //兼容审核通过，用户前端取消问题
        if($order['check_status'] ==1 && $order['status'] == 4){
        ...
        $is_book_order = 0;
        // 预定点餐
        return $this->json(['status'=>1, 'data' => $order, 'business' => $business,'textset'=>$textset,'proinfo'=>$proinfo,'prodetail'=>$prodetail,'is_book_order'=>$is_book_order]);
    }
    ...
    public function del(){
        ...
        $update = [];
        if($order['totalprice'] > 0 && $order['refund_status'] !=2 ) {
