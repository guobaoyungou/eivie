...
use think\facade\Db;
class KanjiaOrder extends Common
{
	//订单列表
    public function index(){
		if(request()->isAjax()){
			...
			$count = 0 + Db::name('kanjia_order')->where($where)->count();
			$list = Db::name('kanjia_order')->where($where)->page($page,$limit)->order($order)->select()->toArray();
			...
			foreach($list as $k=>$vo){
				...
				$list[$k]['goodsdata'] = '<div style="font-size:12px;float:left;clear:both;margin:1px 0">'.
					...
					'<div style="float: left;width:160px;margin-left: 10px;white-space:normal;line-height:16px;">'.
						...
						'<div style="padding-top:0px;color:#f60;font-size:12px">原价￥'.$vo['sell_price'].'</div>'.
	...
	public function excel(){
		...
		$list = Db::name('kanjia_order')->where($where)->order($order)->page($page,$limit)->select()->toArray();
        $count = Db::name('kanjia_order')->where($where)->order($order)->count();
		$title = array('订单号','下单人','商品名称','总价','实付款','支付方式','姓名','电话','收货地址','配送方式','配送/提货时间','快递信息','客户留言','后台备注','下单时间','状态','备注','其他');
		$data = [];
		foreach($list as $k=>$vo){
			$member = Db::name('member')->where('id',$vo['mid'])->find();
			$status='';
            ...
            //配送自定义表单
            $vo['formdata'] = \app\model\Freight::getformdata($vo['id'],'kanjia_order');
            if($vo['formdata']) {
                foreach ($vo['formdata'] as $formdata) {
                    if($formdata[2] != 'upload') {
                        if($formdata[0] == '备注') {
                            ...
                            $data[$k][15] .= $formdata[0].':'.$formdata[1]."\r\n";
                        }
                    }
                }
            }
	...
	public function getdetail(){
		...
		$order = Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->find();
		
		$order['formdata'] = \app\model\Freight::getformdata($order['id'],'kanjia_order');
	...
	public function setremark(){
		...
		Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['remark'=>$content]);
	...
	public function changeprice(){
		...
		Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['totalprice'=>$newprice,'ordernum'=>date('ymdHis').aid.rand(1000,9999)]);
		return json(['status'=>1,'msg'=>'修改完成']);
	}
	//关闭订单
	public function closeOrder(){
		$orderid = input('post.orderid/d');
		$order = Db::name('kanjia_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->find();
		if(!$order || $order['status']!=0){
			return json(['status'=>0,'msg'=>'关闭失败,订单状态错误']);
		}
		//加库存
		Db::name('kanjia_product')->where('aid',aid)->where('bid',bid)->where('id',$order['proid'])->update(['stock'=>Db::raw("stock+1"),'sales'=>Db::raw("sales-1")]);
		
		$rs = Db::name('kanjia_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['status'=>4]);
	...
	public function ispay(){
		...
		$orderid = input('post.orderid/d');
		Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['status'=>1,'paytime'=>time(),'paytype'=>'后台支付']);
		//奖励积分
		$order = Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->find();
	...
	public function sendExpress(){
        ...
        if($order['platform'] == 'wx' && $order['paytypeid'] == 2){
            \app\common\Order::wxShipping(aid,$order,'kanjia',['express_com'=>$express_com,'express_no'=>$express_no]);
        }
		
		//订单发货通知
		$tmplcontent = [];
		$tmplcontent['first'] = '您的砍价订单已发货';
	...
	public function plfh(){
		...
		foreach($exceldata as $v){
			...
			$order = Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('ordernum',$ordernum)->find();
			if($order['freight_type']==10){
				$errornum++;
				// 货运托运
				continue;
			...
			if($order['status']!=1){ //修改物流信息
				Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$order['id'])->update(['express_com'=>$express_com,'express_no'=>$express_no]);
				$errornum++;
				continue;
			}

			Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$order['id'])->update(['express_com'=>$express_com,'express_no'=>$express_no,'send_time'=>time(),'status'=>2]);
		...
		\app\common\System::plog('砍价活动订单批量发货');
    ...
    function orderHexiao(){
        ...
        Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['status'=>3,'collect_time'=>time()]);
        \app\common\Member::uplv(aid,$order['mid']);
        //发货信息录入 微信小程序+微信支付
        if($order['platform'] == 'wx' && $order['paytypeid'] == 2){
            \app\common\Order::wxShipping(aid,$order,'kanjia');
        }
        \app\common\System::plog('砍价订单核销确认收货'.$orderid);
	...
	public function refundCheck(){
		...
		$order = Db::name('kanjia_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->find();
		if($st==2){
			Db::name('kanjia_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['refund_status'=>3,'refund_checkremark'=>$remark]);
			...
			Db::name('kanjia_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['status'=>4,'refund_status'=>2]);
            //退款减去商户销量
            \app\model\Payorder::addSales($orderid,'kanjia',$order['aid'],$order['bid'],-$order['num']);
			//积分抵扣的返还
			if($order['scoredkscore'] > 0){
				\app\common\Member::addscore(aid,$order['mid'],$order['scoredkscore'],'订单退款返还');
			}
			//扣除消费赠送积分
        	\app\common\Member::decscorein(aid,'kanjia',$order['id'],$order['ordernum'],'订单退款扣除消费赠送');
			//退款退还佣金
			//退款成功通知
			$tmplcontent = [];
	...
	public function del(){
		$id = input('post.id/d');
		Db::name('kanjia_order')->where('aid',aid)->where('bid',bid)->where('id',$id)->delete();
