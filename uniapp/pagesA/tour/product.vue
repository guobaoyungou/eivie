...
<view class="container">
	<block v-if="isload">
		...
		<view id="scroll_view_tab0">
			<view class="swiper-container">
				<swiper class="swiper" :indicator-dots="false" :autoplay="true" :interval="5000" @change="swiperChange">
					<block v-if="product.pics && product.pics.length>0" v-for="(item, index) in product.pics" :key="index">
        ...
        返回首页
			...
			<view class="tobuy" :style="{background:t('color1')}" @tap="tobuy" ><text>立即购买</text></view>
		</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			var id = that.opt.id;
			that.loading = true;
			app.get('ApiTour/product', {id: id,trid:that.trid}, function (res) {
				that.loading = false;
				if (res.status == 1) {
          if(res.product.detail){
            var pagecontent = res.product.detail;
    ...
    tobuy: function (e) {
      ...
      var trid  = that.trid;
      app.goto('buy?proid=' + proid + '&num=1&trid='+trid);
    },
...
.header .title {padding: 10px 0px;line-height:70rpx;font-size:32rpx;display:flex;}
...
.bottombar .tobuy{ width:50%; height: 90rpx;border-radius:10rpx;color: #fff; background: #df2e24; font-size:28rpx;display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:45rpx;margin-right:16rpx;padding-right:20rpx}

.collage_title{width:100%;height:110rpx;background: #FFF;display:flex;align-items:center;padding:0 3%}
