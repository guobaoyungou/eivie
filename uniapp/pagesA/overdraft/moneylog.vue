...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['消费明细','还款记录']" :itemst="['0','1']" :st="st" :showstatus="showstatus" :ismoney="1" :isfixed="true" @changetab="changetab"></dd-tab>
		<view class="content">
			<block v-if="st==0">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
						<text class="t1">还款金额：{{item.money}}元</text>
						<text class="t2">{{item.createtime}}</text>
				</view>
				<view class="f3">
						<text class="t1" v-if="item.status==0">还款失败</text>
						<text class="t2" v-if="item.status==1">还款成功</text>
				</view>
			</view>
			</block>
		</view>
