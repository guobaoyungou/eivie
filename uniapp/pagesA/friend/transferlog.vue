<template>
	<view class="container">
		<block v-if="isload">
			<dd-tab :itemdata="[t('余额')+'明细',t('积分')+'明细']" :itemst="['0','1']" :st="st" :showstatus="showstatus"
				:ismoney="1" :isfixed="true" @changetab="changetab"></dd-tab>
			<view class="content">
				<view v-for="(item, index) in datalist" :key="index" class="item">
					...
					<view class="f2">
						<text class="t1" v-if="item.num>0" :style="{color:t('color1')}">+{{item.num}}</text>
	...
	export default {
		...
		methods: {
			getdata: function(loadmore) {
				...
				that.nomore = false;
				that.loading = true;
				app.post('ApiFriend/transferlog', {
					st: st,
					pagenum: pagenum,
					fmid: that.fmid
				}, function(res) {
