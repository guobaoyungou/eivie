		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="op">
					...
					<block v-if="item.status==1">
						<view v-if="(item.refund_status==0 || item.refund_status==3) && shopset.refund==1" class="btn2" @tap.stop="goto" :data-url="'refund?orderid=' + item.id + '&price=' + item.totalprice">申请退款</view>
					</block>
...
export default {
  ...
  methods: {
    ...
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
		app.post('ApiTaocan/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
			that.loading = false;
    ...
    toclose: function (e) {
      ...
      app.confirm('确定要关闭该订单吗?', function () {
				app.showLoading('提交中');
        app.post('ApiTaocan/closeOrder', {orderid: orderid}, function (data) {
    ...
    todel: function (e) {
      ...
      app.confirm('确定要删除该订单吗?', function () {
				app.showLoading('删除中');
        app.post('ApiTaocan/delOrder', {orderid: orderid}, function (data) {
					app.showLoading(false);
          app.success(data.msg);
          setTimeout(function () {
            that.getdata();
    ...
    orderCollect: function (e) {
      ...
      app.confirm('确定要收货吗?', function () {
		app.showLoading('提交中');
        app.post('ApiTaocan/orderCollect', {orderid: orderid}, function (data) {
			app.showLoading(false);
			app.success(data.msg);
			setTimeout(function () {
				that.getdata();
