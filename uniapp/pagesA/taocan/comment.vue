			...
			<view class="form-item1">
				...
				<view class="product flex">
					...
					<view class="info flex1">
						<view class="f1">{{og.gg_proname}}</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiTaocan/comment', {ogid: ogid}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiTaocan/comment', {ogid: ogid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
