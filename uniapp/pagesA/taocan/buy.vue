...
<view class="container">
	<block v-if="isload">
		...
		<view class="buydata">
			...
			<view class="bcontent">
				<view class="product">
					<view class="item flex"  v-for="(guige,index) in guige_data">
						<view class="img">
							<image class="img" v-if="guige.pic" :src="guige.pic"></image>
							<image class="img" v-else :src="product.pic"></image>
						</view>
						<view class="info flex1">
							<view class="f1">{{guige.product_name}}</view>
							<view class="f2">规格：{{guige.name}} <view class="f3">× {{guige.num}}</view></view>
						</view>
					</view>
				</view>
				<view class="freight">
				...
				<view class="price">
					<text class="f1">礼包数量</text>
					<text class="f2">{{totalnum}}</text>
				</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiTaocan/buy', {proid: that.opt.proid,guige_num: that.opt.guige_num,num_total: that.opt.num_total}, function (res) {
				that.loading = false;
				if (res.status == 0) {
    ...
    topay: function (e) {
      ...
      var formdata = e.detail.value;
			...
			app.showLoading('提交中');
      app.post('ApiTaocan/createOrder', {
