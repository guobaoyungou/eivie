...
<view>
	<block v-if="isload">
			...
			<view class="datalist">
				...
				<view :data-url="'product?id=' + item.id" @tap="goto" class="collage-product">
					...
					<view class="product-info">
						...
						<view class="p3">
							<view class="t1">礼包数量<text style="font-size:32rpx;color:#f40;padding:0 2rpx;">{{item.perlimit}}</text></view>
							<view class="t2" :style="{borderColor:t('color1'),color:t('color1')}">
								<text class="x2" :style="{backgroundColor:t('color1')}">立即购买</text>
							</view>
						</view>
					</view>
				</view>
		...
		<button class="covermy" @tap="goto" data-url="orderlist">我的套餐</button>
	</block>
...
export default {
  ...
  methods: {
		getdata: function (loadmore) {
			...
			that.nodata = false;
			that.nomore = false;
			app.post('ApiTaocan/index', {bid:that.bid,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
