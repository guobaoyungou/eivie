...
<view class="container">
	<block v-if="isload">
		...
		<view class="bottom notabbarbot">
			...
			<block v-if="detail.status==1 && !detail.hidefahuo">
				<view v-if="detail.product_type==2" class="btn2" @tap="goto" :data-url="'weightOrderFahuo?id='+detail.id">发货</view>
				<!--<view v-else class="btn2" @tap="fahuo" :data-id="detail.id">发货</view>-->
			</block>
			<view v-if="detail.status==2 && detail.freight_type==1" class="btn2" @tap="shouhuo" :data-id="detail.id">收货</view>
			<view v-if="(detail.status==1 || detail.status==2 || detail.status==8) && (detail.freight_type==1 || detail.freight_type==5)" class="btn2" @tap="hexiao" :data-id="detail.id">核销</view>
...
export default {
  ...
  methods: {
		...
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiMendianCenter/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
    ...
    getdjs: function () {
      ...
      if (totalsec <= 0) {
		...
		setremarkconfirm: function (done, remark) {
			...
			var that = this
			app.post('ApiMendianCenter/setremark', { type:'shop',orderid: that.detail.id,content:remark }, function (res) {
				app.success(res.msg);
				setTimeout(function () {
					that.getdata();
				}, 1000)
		...
		changePriceConfirm: function (done, val) {
			...
			var that = this
			app.post('ApiMendianCenter/changePrice', { type:'shop',orderid: that.detail.id,val:val }, function (res) {
				app.success(res.msg);
				setTimeout(function () {
					that.getdata();
				}, 1000)
		...
		confirmfahuo:function(){
			...
			app.post('ApiMendianCenter/sendExpress', { type:'shop',orderid: that.detail.id,express_no:that.express_no,express_com:express_com}, function (res) {
				if(res.status == 0){
					app.error(res.msg);
					return;
				}
		...
		confirmfahuo10:function(){
			...
			var that = this
			var express_com = this.expressdata[this.express_index]
			app.post('ApiMendianCenter/sendExpress', { type:'shop',orderid: that.detail.id,pic:that.express_pic,fhname:that.express_fhname,fhaddress:that.express_fhaddress,shname:that.express_shname,shaddress:that.express_shaddress,remark:that.express_remark}, function (res) {
				app.success(res.msg);
				setTimeout(function () {
					that.getdata();
				}, 1000)
		...
		ispay:function(e){
			...
			app.confirm('确定要改为已支付吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/ispay', { type:'shop',orderid: orderid }, function (data) {
		...
		shouhuo:function(e){
			var that = this;
			var orderid = e.currentTarget.dataset.id
			app.confirm('确定要收货并改为待提货状态吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/shouhuo', {orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		hexiao:function(e){
			...
			app.confirm('确定要核销并改为已完成状态吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/hexiao', { 'op':'confirm',type:'shop',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		delOrder: function (e) {
			...
			app.confirm('确定要删除该订单吗?', function () {
				app.post('ApiMendianCenter/delOrder', { type:'shop',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						app.goto('shoporder');
		...
		closeOrder: function (e) {
			...
			app.confirm('确定要关闭该订单吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/closeOrder', { type:'shop',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function (){
						that.getdata();
		...
		refundnopass: function (e) {
			...
			app.confirm('确定要驳回退款申请吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/refundnopass', { type:'shop',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		refundpass: function (e) {
			...
			app.confirm('确定要审核通过并退款吗?', function () {
				app.showLoading('提交中');
				app.post('ApiMendianCenter/refundpass', { type:'shop',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		peisong:function(){
			var that = this;
			that.loading = true;
			app.post('ApiMendianCenter/getpeisonguser',{type:'shop_order',orderid:that.detail.id},function(res){
				that.loading = false;
				...
				if(paidantype==1){
					...
					app.confirm(tips,function(){
						app.post('ApiMendianCenter/peisong', { type:'shop_order',orderid: that.detail.id,psid:psid}, function (res) {
                            if(res.status ==1){
                                app.success(res.msg);
                                setTimeout(function () {
                                	that.getdata();
		...
		peisongWx:function(){
			...
			app.confirm(tips,function(){
				that.loading = true;
				app.post('ApiMendianCenter/peisongWx', { type:'shop_order',orderid: that.detail.id}, function (res) {
					that.loading = false;
					app.success(res.msg);
					setTimeout(function () {
						that.getdata();
        ...
        goMyt:function(){
            ...
            app.confirm(tips,function(){
                ...
                app.post('ApiMendianCenter/peisong', data, function (res) {
                    that.loading = false;
                    if(res.status == 1){
                        app.success(res.msg);
                        setTimeout(function () {
