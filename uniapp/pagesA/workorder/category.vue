				...
				<view class="nav_right-content">
					...
					<scroll-view class="classify-box" scroll-y="true" @scrolltolower="scrolltolower">
						<view class="product-itemlist">
							<view class="item" v-for="(item,idx2) in datalist" :key="item.id"  >
								<view class="product-info">
									...
									<view class="p1" @click="goto" :data-url="'detail?id='+item.id+'&bid='+bid"><text>{{item.name}}</text></view>
								</view>
								<view class="list" v-if="curIndex2==idx2" v-for="(subitem,subindex) in item.list" @tap="goto" :data-url="'/pagesB/workorder/index?cateid='+subitem.id">
  ...
  onLoad: function (opt) {
	...
	methods: {
		getdata:function(){
			...
			that.datalist = [];
			that.loading = true;
			app.get('ApiWorkorder/getclassify', {cid:nowcid,bid:that.bid}, function (res) {
				that.loading = false;
			  var clist = res.data;
			  that.clist = clist;
			  if (nowcid) {
		...
		getdatalist: function (loadmore) {
			...
			app.post('ApiWorkorder/getsubcate',wherefield, function (res) { 
				that.loading = false;
...
.product-itemlist .product-info .img image{ width: 30rpx; height: 30rpx;}

.product-itemlist .list{ padding-left: 30rpx; display:flex; justify-content:space-between; align-item:center}
