	...
	<block v-if="isload">
        <form @submit="formSubmit">
            <view class="mymoney" :style="{background:t('color1')}">
				<view class="flex">
					<view>
						<view class="f1">可提现{{t('贡献值')}}</view>
						<view class="f2"><text style="font-size:26rpx">￥</text>{{userinfo.bonus_pool_money}}</view>
					</view>
					<view>
						<view class="f1">{{t('贡献值')}}上限</view>
						<view class="f2"><text style="font-size:26rpx">￥</text>{{userinfo.bonus_pool_max_money}}</view>
					</view>
				</view>
				
                <view class="f3" @tap="goto" data-url="/pagesA/bonuspool/commissionlog?st=1"><text>提现记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
				<!-- 
            ...
            <view class="withdrawtype">
                ...
                <view class="f2">
					<view class="item"  @tap.stop="changeradio" data-paytype="余额">
					    <view class="t1"><image class="img" :src="pre_url+'/static/img/pay-money.png'"/>余额</view>
					    <view class="radio" :style="paytype=='余额' ? 'background:'+t('color1')+';border:0' :''"><image class="radio-img" :src="pre_url+'/static/img/checkd.png'"/></view>
					</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAgent/gongxianWithdraw', {}, function (res) {
				that.loading = false;
    ...
    formSubmit: function () {
	  ...
	  if(paytype == '汇付天下银行卡' && (this.userinfo.to_set_adapay && this.userinfo.to_set_adapay==1)) {
	    ...
	    return;
			...
			app.showLoading('提交中');
      app.post('ApiAgent/gongxianWithdraw', {money: money,paytype: paytype}, function (data) {
...
.mymoney .f1{margin:0 0 0 60rpx;color:rgba(255,255,255,0.8);font-size:24rpx;text-align: center;}
.mymoney .f2{margin:20rpx 0 0 60rpx;color:#fff;font-size:50rpx;font-weight:bold}
