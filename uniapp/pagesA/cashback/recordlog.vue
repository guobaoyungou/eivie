		...
		<view class="content">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
						...
						<text v-if="item.remark" class="t2">{{item.remark}}</text>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var pro_id = that.pro_id;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiCashback/recordlog', {pro_id: pro_id,cashback_id: cashback_id,pagenum: pagenum,id:that.id}, function (res) {
				that.loading = false;
