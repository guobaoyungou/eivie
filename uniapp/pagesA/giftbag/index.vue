	...
	<block v-if="isload">
		<view class="container">
			<view class="datalist">
				...
				<view :data-url="'list?gbid=' + item.id+'&bid='+bid" @tap="goto" class="collage-product">
          <view class="product-pic" style="margin-bottom: 20rpx;border-radius: 8rpx;overflow: hidden;">
          	<image :src="item.pic" mode="widthFix"></image>
          </view> 
					<view class="product-info">
						<view class="p1">{{item.name}}</view>
						<view class="p3" style="position: relative;bottom: 0;overflow: hidden;display: block;">
					    <view class="t1" :style="{color:t('color1'),fontSize:'28rpx'}"><text style="font-size:24rpx">￥</text>{{item.sell_price}}</view>
					    <view style="display: flex;">
					      <view class="t1">
					        已领
					        <text v-if="item.sales<100000" style="font-size:28rpx;color:#f40;padding:0 2rpx;">{{item.sales}}</text>
					        <text v-else style="font-size:28rpx;color:#f40;padding:0 2rpx;">10万+</text>
					        件
					      </view>
					      <view style="flex: 1;"></view>
					      <view class="t2" :style="{backgroundColor:t('color1'),borderColor:t('color1'),color:t('color1'),float:'right'}">
					        <text class="x2" :style="{backgroundColor:t('color1')}">去领取</text>
					      </view>
					    </view>
						</view>
					</view>
...
export default {
  ...
  methods: {
      ...
      var st = that.st;
			...
			that.nodata = false;
      that.nomore = false;
      app.post('ApiGiftBag/index', {st: st,pagenum: pagenum,bid:that.bid}, function (res) {
				that.loading = false;
        if(res.status == 1){
...
.datalist{width:700rpx;margin: 0 auto;}
.collage-product {min-height:200rpx; background: #fff; padding:20rpx 20rpx;margin-top: 20rpx;border-radius:20rpx}
.collage-product .product-pic {width: 660rpx;height: 330rpx; background: #ffffff;overflow:hidden}
.collage-product .product-pic image{width: 100%;height:330rpx;}
.collage-product .product-info {padding: 5rpx 10rpx;flex:1}
.collage-product .product-info .p1 {color:#323232;font-weight:bold;font-size:28rpx;line-height:36rpx;margin-bottom:10rpx;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;height:auto}
...
.collage-product .product-info .p3{font-size: 24rpx;height:120rpx;line-height:50rpx;overflow:hidden;display:flex;}
