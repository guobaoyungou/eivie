...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">我的{{t('冻结佣金')}}</view>
			<view class="f2">{{myxiaofei}}</view>
		</view>
		...
		<view class="content" id="datalist">
			...
			<view class="item">
				...
				<view class="f2">
					...
					<block v-else>
						<text class="t2">{{item.commission}}</text>
					</block>
				</view>
			</view>
		</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/xiaofeimoneylog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					uni.setNavigationBarTitle({
						title: that.t('冻结佣金') + '明细'
					});
					that.myxiaofei = res.myxiaofei;
