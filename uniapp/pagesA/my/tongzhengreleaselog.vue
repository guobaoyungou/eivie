  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/tongzheng_releaselog', {st: st,pagenum: pagenum,pid:that.pid}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					uni.setNavigationBarTitle({
						title: that.t('通证') + '释放记录'
					});
