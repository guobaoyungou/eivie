...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">我的{{t('通证')}}</view>
			<view class="f2">
				<view class="f2">{{mytongzheng}}<view class="btn-mini" v-if="tongzheng_transfer" @tap="goto" data-url="/pagesA/my/tongzhengtransfer">转账</view></view>
			</view>
		</view>
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">待释放{{t('通证')}}</view>
			<view class="f2">{{release_total}}</view>
			<view class="f3" @tap="goto" data-url="/pagesA/my/tongzhengorderlog"><text>释放记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
		app.post('ApiMy/tongzhenglog', {st: st,pagenum: pagenum}, function (res) {
			that.loading = false;
			var data = res.data;
			if (pagenum == 1) {
				...
				that.release_total = res.release_total;
				that.tongzheng_transfer = res.tongzheng_transfer;
				that.datalist = data;
				if (data.length == 0) {
					that.nodata = true;
				}
...
.myscore .f3{height:56rpx;padding:0 10rpx 0 20rpx;border-radius: 28rpx 0px 0px 28rpx;background:rgba(255,255,255,0.2);font-size:20rpx;font-weight:bold;color:#fff;display:flex;align-items:center;position:absolute;top:94rpx;right:0}
