...
<view class="container">
	<block v-if="isload">
		<view class="content" id="datalist">
			...
			<view class="item" @tap="goto" :data-url="'/pagesA/my/tongzhengreleaselog?pid='+item.id">
				<view class="f1">
						<text class="t1">订单ID:{{item.orderid}}</text>
						<text class="t2">总数量：{{item.tongzheng}}</text>
						<!-- <text class="t2">释放比例：{{item.release_bili}}%</text> -->
						<text class="t2">剩余：{{item.remain}}</text>
						<text class="t2">状态：{{item.status_str}}</text>
						<text class="t2">{{item.createtime}}</text>
				</view>
				<view class="f2">
					<block>
						<text class="t1">已释放：{{item.release_num}}</text>
					</block>
				</view>
			</view>
		</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/tongzheng_orderlog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					uni.setNavigationBarTitle({
						title: that.t('通证') + '订单记录'
					});
