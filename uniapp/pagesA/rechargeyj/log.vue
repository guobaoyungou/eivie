...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['业绩明细','提现记录']" :itemst="['0','1']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		<view class="content">
			<block v-if="st==0">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
            <text class="t2" v-if="item.remark">{{item.remark}}</text>
						<text class="t2" v-if="item.money>0">充值金额：{{item.money}}</text>
						<text class="t2">{{item.createtime}}</text>
				</view>
				<view class="f2">
						<text class="t1" v-if="item.get_yj>0">+{{item.get_yj}}</text>
						<text class="t2" v-else>{{item.get_yj}}</text>
				</view>
			</view>
			</block>
			<block v-if="st==1">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
            <text class="t1">提现业绩：{{item.rechargeyj_money}}</text>
						<text class="t1">金额：{{item.money}}元</text>
						<text class="t2">{{item.createtime}}</text>
				</view>
				<view class="f3">
...
export default {
    ...
    methods: {
        getdata: function (loadmore) {
            ...
            that.nomore = false;
            that.loading = true;
            app.post('ApiMy/rechargeyjlog', {st: st,pagenum: pagenum}, function (res) {
                that.loading = false;
                var data = res.data;
                if (pagenum == 1) {
