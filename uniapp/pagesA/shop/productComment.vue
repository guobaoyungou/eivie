...
<view class="container">
	<block v-if="isload">
		...
		<view class="form-content">
			<view class="form-item1">
				...
				<view class="product flex">
					<view class="img"><image :src="product.pic"></image></view>
					<view class="info flex1">
						<view class="f1">{{product.name}}</view>
						<view class="f2">{{product.ggname}}</view>
		...
		<button v-if="addcommentbtn"></button>
	</block>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiShop/commentProduct', {proid: that.opt.proid}, function (res) {
				that.loading = false;
				if(res.status){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiShop/commentProduct', {proid: that.opt.proid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
