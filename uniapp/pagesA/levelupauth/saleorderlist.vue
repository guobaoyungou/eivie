	...
	<block v-if="isload">
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'orderdetail?type=2&id=' + item.id">
					<view class="head">
					...
					<view class="op">
						<view @tap.stop="goto" :data-url="'orderdetail?type=2&id=' + item.id" class="btn2">订单详情</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiLevelupAuth/buyorderlist', {st: st,pagenum: pagenum,type:2}, function (res) {
				that.loading = false;
