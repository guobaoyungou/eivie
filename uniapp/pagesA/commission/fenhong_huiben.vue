	...
	<block v-if="isload">
		<view class="topfix">
			<view class="tongji flex-y-center "   :style="{background:color1?color1:t('color1')}">
				...
				<view v-if="huiben_maximum_status==1">
					<view class="price">{{huiben_maximum}}</view>
					<view>{{t('回本股东分红额度')}}</view>
				</view>
				<view>
					<view class="price">{{commissionyj}}</view>
					<view>累计分红</view>
				</view>
			</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.nodata = false;
			that.nomore = false;
			app.get('ApiAgent/fenhong_huiben',{st:st,pagenum: pagenum},function(res){
				that.loading = false;
				var data = res.datalist;
				if (pagenum == 1) {
					uni.setNavigationBarTitle({
						title: that.t('回本股东分红') + '明细'
					});
					if(st == 1){
						that.commissionyj = res.commissionyj;
						that.count = res.count;
						that.huiben_maximum = res.huiben_maximum;
						that.huiben_maximum_status = res.huiben_maximum_status;
					}
					that.datalist = data;
					if (data.length == 0) {
						that.nodata = true;
...
.tongji{width: 94%;margin: 30rpx 3%;padding: 30rpx;border-radius: 20rpx; background-color: #F2350D;height: 160rpx;color: #fff;justify-content: space-evenly;text-align: center;}
