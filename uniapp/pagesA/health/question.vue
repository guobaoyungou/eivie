...
<view :style="{fontSize:fontSize}">
	<block v-if="isload">
		<view class="progress">进度：<text>{{question_index+1}}/{{questionlist.length}}</text></view>
		...
		<view class="bottom" v-if="custom.PSQI">
			...
			<view v-if="question_index==questionlist.length-1" class="btn btn2" @tap="finish">提交</view>
		</view>
		<view class="bottom" v-else>
			...
			<view v-if="question_index==questionlist.length-1" class="btn" :style="{background:themeColor}" @tap="finish">提交</view>
		</view>
	</block>
...
export default {
	...
	methods: {
		getdata:function(){
			...
			that.loading = true;
			app.post('ApiHealth/questionList', {fid:that.opt.fid}, function (res) {
				that.loading = false;
				if (res.status == 1) {
		...
		selectOption: function (e) {
			...
			var questionoption = that.question.optionlist;
			var checked = questionoption[index].checked;
			for(var i in questionoption){
				questionoption[i].checked = 0;
			}
			questionoption[index].checked = 1
			that.question.optionlist = questionoption
			that.questionlist[that.question_index] = that.question;
		}, 
		finish:function(){
			var that=this
			var optionlist = that.question.optionlist
			...
			that.subscribeMessage(function () {
				setTimeout(function(){
					app.showLoading('提交中')
					app.post('ApiHealth/questionDone', {fid:that.fid,questionlist:that.questionlist}, function(res) {
						app.showLoading(false);
						if(res.status == 1){
							app.success(res.msg);
							setTimeout(function () {
								app.goto('result?id='+res.id);
							}, 1000)
						}else{
							that.canSubmit = true;
							app.alert(res.msg);
...
.option_group .option{background:#f6f6f6; border-radius: 10rpx;margin-bottom: 20rpx;line-height: 140%;padding:14rpx 20rpx;}
