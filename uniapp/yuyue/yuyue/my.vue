...
<view>
	...
	<view class="banner">
			...
			<view class="saoyisao" v-if="hexiao_status" @tap="saoyisao">
				<image :src="pre_url+'/static/img/ico-scan-white.png'"></image>
			</view>
			
			<view class="sets" v-if="sets"  @tap="goto" data-url="sets"><image :src="pre_url+'/static/img/set.png'" /></view>
	</view>
	<view class="contentdata">
		...
		<view class="listcontent">
			...
			<view class="list" v-if="searchmember">
				<view class="item" @tap="goto" data-url="/pagesA/searchmember/searchmember">
					<view class="f1"><image :src="pre_url+'/static/img/peisong/ico-shenfen.png'"></image></view>
  ...
  methods: {
		getdata:function(){
			...
			app.get('ApiYuyueWorker/my', {}, function (res) {
				...
				that.sets = res.sets;
				that.hexiao_status =  res.worker.hexiao_status || 0;
				that.loaded();
			});
		},
				...
				jweixin.ready(function () {   //需在用户可能点击分享按钮前就先调用
					jweixin.scanQRCode({
						...
						success: function (res) {
							...
							app.goto('/yuyue/yuyue/hexiao?'+params);
						}
					});
				});
				// #endif
...
.banner .saoyisao{ width:70rpx;height:100rpx;line-height:100rpx;font-size:40rpx;text-align:center}
.banner .saoyisao image{width:50rpx;height:50rpx;border-radius:0}
</style>