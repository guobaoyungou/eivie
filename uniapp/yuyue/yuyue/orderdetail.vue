...
<view class="container">
	<block v-if="isload">
		<view class="ordertop flex" :style="'background:url('+pre_url+'/static/img/orderbg.png);background-size:100%'">
			...
			<view class="f1" v-if="detail.status==2 && detail.worker && detail.worker.endtime">
				<view class="t1">订单确认中</view>
			</view>
			<view class="f1" v-else-if="detail.status==2">
		...
		<view class="orderinfo">
			...
			<view class="item">
				...
				<text class="t2 red">¥{{detail.paymoney}}</text>
			</view>
			...
			<view class="item">
				...
				<text class="t2" v-if="detail.status==2 && detail.worker && detail.worker.endtime">待确认</text>
				<text class="t2" v-else-if="detail.status==2">服务中</text>
      ...
      <view class="item" v-if="detail.remark && detail.show_remark">
      	<text class="t1">备注</text>
      	<text class="t2" @longpress="copyRemark" :data-remark="detail.remark" @click="gotourl(detail.remark_tourl)">{{detail.remark}}</text>
      </view>
      <block v-if="detail.fwbid && detail.fwbinfo">
        ...
        <view class="item" v-if="detail.fwbinfo.address">
          ...
          <view v-else @tap="openLocation" :data-latitude="detail.fwbinfo.latitude" :data-longitude="detail.fwbinfo.longitude" class="t2">
			...
			<view class="item" v-if="detail.guarantee_start_date && detail.guarantee_end_date">
				<text class="t1">保质期</text>
				<text class="t2">{{detail.guarantee_start_date}} - {{detail.guarantee_end_date}}</text>
			</view>
			<view class="item" style="align-items: center;" v-if="detail.remark_pics && detail.remark_pics.length > 0">
				<text class="t1">图片</text>
				<view class="t2">
					<view class="pics-content flex-row">
						<view class="pic-img" v-for="pic in detail.remark_pics">
      ...
      <block v-else>
        ...
        <view class="item">
        	...
        	<text class="t2">{{detail.car_number}}.{{detail.car_color}}.{{detail.car_type==1?'SUV':detail.car_type==2?'MPV':'轿车'}}</text>
        </view>
      </block>
		</view>
		<view class="orderinfo">
			...
			<view class="item" >
				...
				<text class="t2">{{detail.binfo.fulladdress}}</text>
			</view>
		</view>
		<view class="orderinfo" v-if="detail.show_service_pics">
			<view class="title">服务图片</view>
			<view class="item flex-bt" style="align-items: center;" v-if="detail.service_before_pics.length > 0">
				<text class="t1">施工前</text>
				<view class="t2">
					<view class="pics-content flex-row">
						<view class="pic-img" v-for="pic in detail.service_before_pics">
							<image :src="pic" @tap="previewImage" mode="widthFix" :data-url="pic"></image>
						</view>
					</view>
				</view>
			...
			<view class="item flex-bt" style="align-items: center;" v-if="detail.service_pics.length > 0">
				<text class="t1">施工中</text>
				<view class="t2">
					<view class="pics-content flex-row">
						<view class="pic-img" v-for="pic in detail.service_pics">
							<image :src="pic" @tap="previewImage" mode="widthFix" :data-url="pic"></image>
						</view>
					</view>
				</view>
			...
			<view class="item flex-bt" style="align-items: center;" v-if="detail.service_finish_pics.length > 0">
				<text class="t1">完成施工</text>
				<view class="t2">
					<view class="pics-content flex-row">
						<view class="pic-img" v-for="pic in detail.service_finish_pics">
							<image :src="pic" @tap="previewImage" mode="widthFix" :data-url="pic"></image>
						</view>
					</view>
				</view>
		...
		<view class="bottom notabbarbot">
			<block v-if="detail.isworkorder==1">
					<view class="btn2" @tap="goto" :data-url="'/pagesB/workorder/index?type=2&id='+detail.id" :data-id="detail.id">发起工单</view>
			</block>
			...
			<block v-if="detail.status==1">
				<block v-if="detail.paytypeid!='4' && detail.refund_status==0 || detail.refund_status==3">
					<view class="btn2" @tap="goto" :data-url="'refund?orderid=' + detail.id+ '&price=' + detail.paymoney">退款</view>
				</block>
			</block>
			
			<block v-if="hexiao_status && detail.hexiao_code && (detail.status==1 || detail.status==2)">
				<view class="btn2" @tap.stop="showhxqr" :data-id="detail.id" data-hexiao_code="detail.hexiao_code" :data-hexiao_qr="detail.hexiao_qr">核销码</view>
			</block>
			
			<block v-if="(detail.status==2 || detail.status==3 || detail.worker_id>0) && detail.status!=4">
				<view class="btn2" @tap="logistics" :data-express_com="detail.express_com" :data-express_no="detail.worker_orderid"  >查看进度</view>
			</block>
			<!--<block v-if="([1,2,3]).includes(detail.status) && invoice">
				<view class="btn2" @tap="goto" :data-url="'invoice?type=yuyue&orderid=' + detail.id">发票</view>
			</block>0-->
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiYuyue/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
        if(res.status == 1){
    ...
    orderCollect: function (e) {
      ...
      app.confirm('确定已完成服务吗?', function () {
		...
		logistics:function(e){
			...
			app.goto('/yuyue/yuyue/logistics?express_no=' + express_no);
		},
		hideSelectExpressDialog:function(){
			this.$refs.dialogSelectExpress.close();
		},
    copyRemark:function(e){
      var that = this;
      var text = e.currentTarget.dataset.remark;
      uni.showActionSheet({
        itemList: ['复制'],
        success(res) {
...
.orderinfo .item .t2{flex:1;text-align:right; overflow-wrap: break-word;word-break: break-all;}
.orderinfo .item .t3{ margin-top: 3rpx;}
.orderinfo .item .red{color:red}

.bottom{ width: 100%; height:calc(92rpx + env(safe-area-inset-bottom));padding: 0rpx 20rpx;background: #fff; position: fixed; bottom: 0px;left: 0px;display:flex;justify-content:flex-end;align-items:center;z-index: 1;}
...
.pics-content {flex-wrap: wrap;}
.pics-content .pic-img{width: 200rpx;margin: 10rpx;}
.pics-content image{width: 100%;height: 100%;}
</style>