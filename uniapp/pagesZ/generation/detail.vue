...
export default {
	...
	onShareAppMessage: function() {
		...
		return {
			...
			path: '/pagesZ/generation/detail?id=' + that.opt.id + '&type=' + (that.opt.type || that.detail.generation_type || 1) + '&pid=' + mid
		};
	},
	...
	methods: {
		...
		goCreate() {
			...
			var url = '/pagesZ/generation/create?id=' + that.detail.id + '&type=' + (that.opt.type || that.detail.generation_type || 1);
			// 传递pid
			if (that.opt.pid) {
				...
				success: function(res) {
					if (res.tapIndex >= 0) {
						...
						uni.share({
							...
							scene: scene,
							title: that.detail.template_name || 'AI创作模板',
							href: app.globalData.pre_url + '/h5/' + app.globalData.aid + '.html#/pagesZ/generation/detail?id=' + that.opt.id + '&type=' + (that.opt.type || 1) + '&pid=' + mid,
							imageUrl: that.detail.cover_image || ''
						});
					}
				}
