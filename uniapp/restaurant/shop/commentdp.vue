...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiRestaurantShop/commentdp', {orderid: orderid}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiRestaurantShop/commentdp', {orderid: orderid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
