...
export default {
  ...
  methods: {
		getdata: function () {
			...
			var id = that.opt.id;
			that.loading = true;
			app.get('ApiRestaurantShop/commentps', {id: id}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiRestaurantShop/commentps', {id: id,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
