	...
	<block v-if="express_com=='同城配送'">
		...
		<view class="order-box">
			...
			<view class="content" style="border-bottom:0">
				...
				<block v-else>
					<view class="f1" :style="psorder.psid == 0 ? 'justify-content: center;' : ''">
						<view class="t1"><text class="x1">{{psorder.juli}}</text><text class="x2">{{psorder.juli_unit}}</text></view>
						<view class="t2" v-if="psorder.psid > 0"><image :src="pre_url+'/static/img/peisong/ps_juli.png'" class="img"/></view>
						<view class="t3" v-if="psorder.psid > 0"><text class="x1">{{psorder.juli2}}</text><text class="x2">{{psorder.juli2_unit}}</text></view>
					</view>
				</block>
				<view class="f2">
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiRestaurantTakeaway/logistics', {express_com: that.express_com,express_no: that.express_no,type:that.type}, function (res) {
				that.loading = false;
