		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
					<view class="head">
						<view class="f1" v-if="item.bid!=0 && item.binfo" @tap.stop="goto" :data-url="'/restaurant/takeaway/index?bid=' + item.bid"><image :src="pre_url+'/static/img/ico-shop.png'"></image> {{item.binfo.name}}</view>
						<view v-else>订单号：{{item.ordernum}}</view>
						<view class="flex1"></view>
						<text v-if="item.status==0" class="st0">待付款</text>
						<text v-if="item.status==1" class="st1">已付款</text>
					...
					<block v-for="(item2, idx) in item.prolist" :key="idx">
						<view class="content" :style="idx+1==item.procount?'border-bottom:none':''">
							...
							<view class="detail">
								...
								<text class="t2" >{{item2.ggname}}{{item2.jltitle?item2.jltitle:''}}</text>
					...
					<view class="op">
						...
						<block v-if="item.status==1">
							<view class="btn2" @tap.stop="quxiao" :data-id="item.id">取消订单</view>
						</block>
						...
						<block v-if="item.status==2">
							<view class="btn2" @tap.stop="goto" :data-url="'logistics?express_com=' + item.express_com + '&express_no=' + item.express_no + '&type=' + item.express_type" v-if="item.freight_type!=3 && item.freight_type!=4">配送详情</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiRestaurantTakeaway/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
