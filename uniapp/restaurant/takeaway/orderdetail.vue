	...
	<block v-if="isload">
		...
		<view class="address">
			...
			<view class="info">
				...
				<text class="t2" v-if="detail.freight_type==1 && storeinfo" @tap="openLocation" :data-address="storeinfo.address" :data-latitude="storeinfo.latitude" :data-longitude="storeinfo.longitude">取货地点：{{storeinfo.name}} - {{storeinfo.address}}</text>
			</view>
		</view>
		...
		<view class="product">
			<view v-for="(item, idx) in prolist" :key="idx" class="content">
				...
				<view class="detail">
					...
					<view class="t2 flex flex-y-center flex-bt" v-if="shopset.takeaway_comment==1">
		...
		<view class="orderinfo">
			...
			<view class="item" v-if="detail.pstime_discount_money > 0">
				<text class="t1">优惠定折扣</text>
				<text class="t2 red">-¥{{detail.pstime_discount_money}}</text>
			</view>
			<view class="item">
		...
		<view class="bottom notabbarbot">
			...
			<block v-if="detail.status==3 ">  <!-- 平台店铺不允许评价-->
