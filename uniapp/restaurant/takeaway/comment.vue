...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiRestaurantTakeaway/comment', {ogid: ogid}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiRestaurantTakeaway/comment', {ogid: ogid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
