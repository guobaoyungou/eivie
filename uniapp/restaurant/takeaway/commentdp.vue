...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiRestaurantTakeaway/commentdp', {orderid: orderid}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiRestaurantTakeaway/commentdp', {orderid: orderid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
