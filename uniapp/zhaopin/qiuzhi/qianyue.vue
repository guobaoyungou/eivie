<template>
	<view class="content" v-if="isload">
		<view class="xieyibox" v-if="showxieyi">
			<view class="xieyi">
				<rich-text :nodes="basicset.qianyue_xieyi"></rich-text>
			</view>
			<view class="bottom">
		...
		<view v-if="!showxieyi">
			<form @submit="formSubmit">
				<!-- 自定义表单Start -->
				<view class="title"><image src="../../static/img/edit2.png"></image>签约</view>
				...
				<!-- 自定义表单End -->
				<view class="box down">
					签约合同模板：<text class="file" @tap="download">合同模板</text>
					<view class="tips">请先下载合同模板，完成填写后，再上传合同扫描件！</view>
				</view>
				<view class="form-option">
					...
					<button v-if="approve==1" class="btn btn1">已签约</button>
				</view>
			</form>
		</view>
	...
	export default {
		...
		methods: {
			getdata: function() {
				...
				var opt = this.opt
				that.loading = true
				app.get('ApiZhaopin/qiuzhiQianyue', {}, function (res) {
					if(res.status==2){
			...
			formSubmit: function (e) {
			  ...
			  // }
				app.showLoading('提交中');
			  app.post("ApiZhaopin/qiuzhiQianyue", postdata, function (data) {
					app.showLoading(false);
			    if (data.status == 1) {
