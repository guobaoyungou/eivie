<template>
	<view class="content" v-if="isload">
		<form @submit="formSubmit">
			<view class="title"><image src="../../static/img/reg-code.png"></image>个人认证</view>
			<view class="box">
				<view class="form-item">
					<view class="form-label">姓名<text style="color:red"> *</text></view>
					<view class="form-value">
						<input type="text" name="name" placeholder="请输入联系人" :value="info.name" placeholder-style="font-size:28rpx;color:#cccccc" />
					</view>
				</view>
				<view class="form-item">
					<view class="form-label">手机号<text style="color:red"> *</text></view>
					<view class="form-value">
						<input type="text" name="tel" placeholder="请输入联系电话" :value="info.tel" placeholder-style="font-size:28rpx;color:#cccccc" />
					</view>
				</view>
				<view class="form-item">
					<view class="form-label">身份证号<text style="color:red"> *</text></view>
					<view class="form-value">
						<input type="text" name="cardno" placeholder="请输入身份证号" :value="info.tel" placeholder-style="font-size:28rpx;color:#cccccc" />
					</view>
				</view>
				<view class="form-item">
					<view class="form-label">身份证图片<text style="color:red"> *</text></view>
					<view class="form-value">
	...
	export default {
		...
		methods: {
			getdata: function() {
				...
				var opt = this.opt
				that.loading = true
				app.get('ApiZhaopin/qiuzhiApply', {}, function (res) {
					that.loading = false;
					if(res.status==0){
						app.alert(res.msg)
						app.goback(true);
			...
			formSubmit: function (e) {
			  ...
			  if (formdata.name == ''){
				...
				if (!/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(formdata.cardno)) {
					app.alert('身份证号格式错误');return false;
				}
				
			  if (that.pics.length == 0) {
			    app.alert('请上传身份证正反面图片');
			    return;
			  }
				formdata['pics'] = that.pics.join(',');
				app.showLoading('提交中');
			  app.post("ApiZhaopin/qiuzhiApply", {info:formdata}, function (data) {
