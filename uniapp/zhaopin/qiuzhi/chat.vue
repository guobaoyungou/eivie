  ...
  methods: {
    ...
    updateMessageTime: function () {
      ...
      this.datalist = datalist;
		...
		getSendDetail:function(){
			var that = this
			if(that.sid>0){
				app.get('ApiZhaopin/sendDetail', {id:that.sid}, function (res) {
					if(res.status==1){
						that.sendDetail = res.send;
						that.zdetail = res.zhaopin;
						that.detail = res.qiuzhi;
    ...
    getdatalist: function () {
      ...
      if (that.loading) return;
			that.loading = true;
			app.post('ApiZhaopin/getChatNotice',{tbid:that.id,tbname:'zhaopin_qiuzhi',pagenum: that.pagenum}, function (res) {
				that.loading = false;
    ...
    sendimg: function () {
      ...
      app.chooseImage(function (data) {
        for (var i = 0; i < data.length; i++) {
          ...
          app.sendSocketMessage({type: 'qiuzhi',data: msgdata});
        }
      }, 3);
    },
    receiveMessage: function (data) {
			...
			if(data.type == 'qiuzhi' && message.tableid==that.id && message.tablename=='zhaopin_qiuzhi') {
		...
		confirm:function(e){
			...
			app.confirm('确定应聘吗？',function(){
				app.post('ApiZhaopin/zhaopinConfirm',{id:id,id1:that.id}, function (res) {
					that.loading = false;
					if(res.status==1){
						that.getSendDetail()
					}else{
