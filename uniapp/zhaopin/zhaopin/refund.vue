		...
		<form @submit="formSubmit" @reset="formReset" report-submit="true">
			<view class="form-content">
				<view class="form-item">
					<text class="label">保证金退款</text>
				</view>
				<view class="form-item">
					<text class="label">退款金额(元)</text>
					<view class="flex" :style="'font-weight:bold;color:'+(t('color1')?t('color1'):'#222222')">￥{{order.totalprice}}</view>
				</view>
				<view class="form-item">
					<text class="label">退款原因</text>
					<view class="input-item"><textarea placeholder="请输入退款原因" placeholder-style="color:#999;" name="reason" @input="reasonInput" :value="order.refund_reason"></textarea></view>
				</view>
			...
			<button class="btn" v-if="order.refund_status==0 || order.refund_status==3" @tap="formSubmit" :style="{background:t('color1')}">确定</button>
			<button class="btn btn1" v-if="order.refund_status==1">退款待审核</button>
			<button class="btn btn1" v-if="order.refund_status==2">已退款</button>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.post('ApiZhaopin/refundinit', {id: that.orderid}, function (res) {
				that.loading = false;
				if(res.status == 0) {
				...
				that.totalprice = that.order.totalprice;
				that.loaded();
			});
		},
    ...
    formSubmit: function () {
      ...
      if (reason == '') {
        ...
        return;
			...
			that.isloading = 1;
			app.showLoading('提交中');
      app.post('ApiZhaopin/assuranceRefund', {orderid: orderid,reason: reason}, function (res) {
