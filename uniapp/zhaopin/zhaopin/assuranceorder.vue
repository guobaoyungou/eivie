...
<view style="width:100%" v-if="isload">
	<view class="search-container">
		<dd-tab :itemdata="['生效中','已退款']" :itemst="['1','2']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
	</view>
	<view class="product-container">
		<view class="product-itemlist">
			<view class="item" v-for="(item,index) in datalist" :key="item.id" @click="goto" :data-url="'/zhaopin/zhaopin/recorddetail?id='+item.id+'&type='+tabindex">
					<view class="item1 flex-sb">
							<view class="product-info">
								<view class="p1">￥{{item.totalprice}}
								</view>
								<view class="p2">
									<text>缴纳时间：</text>
							...
							<view class="flex-e" >
								<view class="btn" v-if="item.status==1 && (item.refund_status==0 || item.refund_status==3)" @tap.stop="goto" :data-url="'refund?orderid='+item.id" :style="{background:'linear-gradient(-90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)',color:'#ffffff'}">退款</view>
								<view class="btn" v-if="item.status==2 && item.refund_status==2":style="{background:'#dcdcdc',color:'#ffffff'}">已退款</view>
							</view>
					</view>
			</view>
		</view>
	</view>
	<view class="tosign" @tap="goto" data-url="assurancefee" :style="{background:'linear-gradient(-90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)',color:'#ffffff'}">
		<view class="v">缴纳</view>
	</view>
...
export default {
  ...
  methods: {
    ...
    getlist: function (loadmore) {
      ...
      app.post('ApiZhaopin/assuranceOrder',{
				pagenum: pagenum,
...
.product-container {width: 100%;margin-top: 110rpx;font-size:26rpx;padding:0;}
...
.product-itemlist .btn{min-width: 100rpx;text-align: center;padding: 4rpx 10rpx;}
</style>