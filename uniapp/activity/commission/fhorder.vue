	...
	<block v-if="isload">
		...
		<view class="content">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				...
				<view class="f2">
					<view class="t1">
						<block v-if="item.products">
							<block v-for="(item2, index2) in item.products">
				...
				<view class="f3" v-if="item.type && item.type =='teamfenhong' ">
						<view class="x1" >{{t('团队分红伯乐奖')}}：+{{item.bole_money}}</view>
						<!-- <view class="x1" v-if="item.pj_money > 0">{{t('商家团队分红平级奖')}}：+{{item.pj_money}}</view> -->
				</view>
			</view>
			</block>
			<block v-if="st==2">
...
export default {
  ...
  methods: {
      ...
      var st = that.st;
			...
			that.nodata = false;
      that.nomore = false;
			app.get('ApiAgent/fhorder',{st:st,pagenum: pagenum},function(res){
				that.loading = false;
