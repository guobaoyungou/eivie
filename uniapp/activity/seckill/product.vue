	...
	<block v-if="isload">
		...
		<view class="bottombar flex-row" :class="menuindex>-1?'tabbarbot':'notabbarbot'" v-if="product.status==1">
			...
			<view class="op">
				...
				<view class="tobuy flex-x-center flex-y-center" style="background:#ccc" v-else-if="seckill_status==2">秒杀已结束</view>
  ...
  onLoad: function (opt) {
	...
	methods: {
		...
		getdjs:function(){
			...
			var seckill_duration_time = seckill_duration * 3600;
			if(that.product.seckill_time_length > 0){
				seckill_duration_time =  that.product.seckill_time_length;
			}
			var seckill_starttime = that.seckill_starttime*1;
			var seckill_endtime = that.seckill_starttime + seckill_duration_time;
