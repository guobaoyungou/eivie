			...
			<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="content" style="border-bottom:none">
					...
					<view class="detail">
						...
						<view class="t3">
							<text class="x1 flex1">
								<text v-if="showprice_dollar && item.usd_sellprice">${{item.usd_sellprice}} </text>
								￥{{item.sell_price}}
							</text>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiSeckill/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
