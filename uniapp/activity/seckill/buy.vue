...
<view class="container">
	<block v-if="isload">
				...
				<view class="form-item" v-for="(item,idx) in freightList[freightkey].formdata" :key="item.id">
					...
					<block v-if="item.key=='upload'">
						...
						<view class="flex" style="flex-wrap:wrap;padding-top:20rpx">
							...
							<view v-else class="form-uploadbtn" :style="{background:'url('+pre_url+'/static/img/shaitu_icon.png) no-repeat 50rpx',backgroundSize:'80rpx 80rpx',backgroundColor:'#F3F3F3'}" @click="editorChooseImage" :data-idx="idx" data-type="pic"></view>
						</view>
					</block>
		...
		<view class="scoredk flex" v-if="userinfo.score2money > 0 && product.scoredkmaxset !='-1' && userinfo.scoredkmaxpercent > 0">
  ...
  methods: {
		getdata: function () {
			...
			app.get('ApiSeckill/buy', {prodata:that.opt.prodata}, function (res) {
				...
				leadermoney = leadermoney.toFixed(2);
				var leveldkprice = 0;
				if(product.lvprice==0 && product.no_discount == 0){
					leveldkprice = product_price
				}
    ...
    calculatePrice: function () {
      ...
      var totalprice = product_price - leadermoney - coupon_money;
      ...
      that.totalprice = totalprice
			...
			if(that.showprice_dollar && that.usdrate){
					that.usdalltotalprice = (that.totalprice/that.usdrate).toFixed(2);
			}
    ...
    topay: function (e) {
	  ...
	  if (this.freightList[freightkey].isoutjuli == 1 && this.freightList[freightkey].pstype==2) {
        app.error('超出配送距离');
        return;
      }
