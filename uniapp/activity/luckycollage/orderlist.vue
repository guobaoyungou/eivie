		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
				<view class="head">
					...
					<block v-if="item.status==1">
						<block v-if="item.buytype!=1">
							...
							<block v-else>
								<text v-if="item.team.status==2" class="st1">拼团成功,请查看</text>
							</block>
				...
				<view class="bottom">
					...
					<text v-if="item.is_score_pay==1 && item.totalscore>0" style="color:red;margin:0 4rpx;"> {{t('积分')}}支付:{{item.totalscore}}</text>
				...
				<view class="op">
					<block v-if="([1,2,3]).includes(item.status) && item.invoice && item.team.status==2">
						<view class="btn2" @tap="goto" :data-url="'/pagesExt/order/invoice?type=lucky_collage&orderid=' + item.id">发票</view>
					</block>
					<view @tap.stop="goto" :data-url="'orderdetail?id=' + item.id" class="btn2">详情</view>
					<view @tap.stop="goto" :data-url="'team?teamid=' + item.teamid" class="btn2" v-if="item.buytype!=1 && item.teamid>0">查看团</view>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiLuckyCollage/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
