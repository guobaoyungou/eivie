		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="op">
					...
					<view @tap.stop="goto" :data-url="'team?teamid=' + item.teamid + '&teampid=' + item.teamid" class="btn2" v-if="item.buytype!=1 && item.teamid && !item.collage_type">查看团</view>
					<view @tap.stop="goto" :data-url="'/pagesB/collage/jtteam?teamid=' + item.teamid " class="btn2" v-if="item.buytype!=1 && item.teamid && item.collage_type==1">查看团</view>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiCollage/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
