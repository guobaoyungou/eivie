		...
		<view class="form-content">
			<view class="form-item1">
				...
				<view class="product flex">
					<view class="img"><image :src="order.propic"></image></view>
					<view class="info flex1">
						<view class="f1">{{order.proname}}</view>
						<view class="f2">{{order.ggname}}</view>
						<view class="f3">￥{{order.product_price}}</view>
					</view>
				</view>
			</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiYuyue/comment', {oid: oid}, function (res) {
				that.loading = false;
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiYuyue/comment', {oid: oid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
