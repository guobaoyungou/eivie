...
<view class="container">
	<block v-if="isload">
		...
		<view style="padding:5px 3%;font-size:22rpx;color:red">注：商品金额并非实际成交金额，活动结束后根据参团人数计算达成的最低团购价格，多付的金额将退还</view>
  ...
  methods: {
    ...
    calculatePrice: function () {
      ...
      var scoredkmaxpercent = parseFloat(that.userinfo.scoredkmaxpercent); //最大抵扣比例
			...
			if (scoremaxtype == 0 && scoredk_money2 > 0 && scoredkmaxpercent > 0 && scoredkmaxpercent <= 100) {
				scoredk_money = oldtotalprice * scoredkmaxpercent * 0.01;
			} else if (scoremaxtype == 1 && scoredk_money2 > scoredkmaxmoney) {
