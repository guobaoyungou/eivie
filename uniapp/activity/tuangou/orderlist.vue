			...
			<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="content" style="border-bottom:none">
					...
					<view class="detail">
						...
						<view class="t3">
							<text class="x1 flex1">
								<text v-if="showprice_dollar && item.usd_productprice">${{item.usd_productprice}} </text>
								￥{{item.sell_price}}
							</text>
				...
				<view class="bottom">
					<text>共计{{item.num}}件商品 实付:
						<text v-if="showprice_dollar && item.usd_real_price">${{item.usd_real_price}} </text> 
				...
				<view class="op">
					<block v-if="([1,2,3]).includes(item.status) && item.invoice">
						<view class="btn2" @tap="goto" :data-url="'/pagesExt/order/invoice?type=tuangou&orderid=' + item.id">发票</view>
					</block>
					...
					<block v-if="item.status==1">
						<view v-if="item.refund_status==0 || item.refund_status==3" class="btn2" @tap.stop="goto" :data-url="'refund?orderid=' + item.id + '&price=' + item.real_price">申请退款</view>
					</block>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiTuangou/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
...
.order-box .content .detail .t3{display:flex;height:40rpx;line-height:40rpx;color: #ff4246;margin-top:20rpx}
