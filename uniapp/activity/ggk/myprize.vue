						...
						<view v-for="(item, idx) in info.formcontent" :key="idx" class="item flex-y-center">
							...
							<view class="f2 flex flex1">
								...
								<block v-if="item.key=='region'">
									<uni-data-picker style="color: #333;" class="flex1" :border='false' :localdata="items" popup-title="请选择省市区"  @change="region_editorBindPickerChange"	:data-idx="idx" data-tplindex="0">
										<input type="text" :name="'form'+idx" :value="picker_region" placeholder="请选择省市区" placeholder-style="color:#fff;font-size:28rpx" style="border: none;color:#fff;font-size:28rpx;padding: 0rpx;"/>
									</uni-data-picker>
								</block>
							</view>
						</view>
			...
			app.get('ApiChoujiang/myprize', {hid: that.opt.hid}, function (res) {
				...
				if(res.hbdata){
					...
					uni.sendBizRedPacket({
						timeStamp: res.hbdata.timeStamp,
						// 支付签名时间戳，
						nonceStr: res.hbdata.nonceStr,
						// 支付签名随机串，不长于 32 位
						package: res.hbdata.package,
						//扩展字段，由商户传入
						signType: res.hbdata.signType,
						// 签名方式，
						paySign: res.hbdata.paySign,
						// 支付签名
						success: function(res2) {
							console.log(res2);
						},
