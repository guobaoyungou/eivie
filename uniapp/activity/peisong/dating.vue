		...
		<view>
			<view class="search-container">
				<view class="search-box">
					...
					<input class="search-text" placeholder="关键字搜索" placeholder-style="color:#aaa;font-size:24rpx" @confirm="searchConfirm"/>
				</view>
			</view>
			...
			<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="content">
					<!-- 押金回收展示，上部分显示会员地址和距离juli2-->
					<block v-if="item.type =='deposit_refund_order'">
						<view class="f1">
						...
						<view class="f2">
							<view class="t1">{{item.orderinfo.address}}</view>
							<view class="t2">{{item.orderinfo.area}}</view>
							<view class="t3">{{item.binfo.name}}</view>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var keyword = that.keyword;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiPeisong/dating', {st: st,pagenum: pagenum,keyword:keyword,bid:that.bid}, function (res) {
				that.loading = false;
				if(res.status==0){
					app.alert(res.msg);
					return;
