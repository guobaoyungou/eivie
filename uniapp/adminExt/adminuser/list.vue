	...
	<block v-if="isload">
		<view class="topsearch flex-y-center">
			<view class="f1 flex-y-center">
				...
				<input :value="keyword" placeholder="输入账号搜索" placeholder-style="font-size:24rpx;color:#C2C2C2" @confirm="searchConfirm" @input="searchChange"></input>
			</view>
		</view>
		<view class="content" v-if="datalist && datalist.length>0">
			<view class="label">
				<text class="t1">账号列表（共{{count}}人）</text>
				<view class="btn" @tap="goto" data-url="edit">添加管理员</view>
			</view>
			<block v-for="(item, index) in datalist" :key="index">
				<view class="item">
					<view class="f1">
						<view class="t2">
							<view class="x1 flex-y-center"> {{item.un}} </view>
							<block>
								<text class="x2">最后登录：{{item.logintime}}</text>
								<text class="x2">加入时间：{{item.createtime}}</text>
								<text class="x2" v-if="item.groupname">所属权限组：{{item.groupname}}</text>
							</block>
						</view>
					</view>
					<block>
						<view class="f2">
							<view class="btn" @tap="goto" :data-url="'edit?id=' + item.id">编辑</view>
							<view class="btn" @tap="todel" :data-id="item.id" v-if="item.isadmin == 0 && item.id != userid">删除</view>
						</view>
					</block>
				</view>
			</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var keyword = that.keyword;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminUser/index', {keyword: keyword,pagenum: pagenum}, function (res) {
        that.loading = false;
    ...
    searchConfirm: function (e) {
      ...
      that.getdata();
		...
		todel: function (e) {
		  ...
		  app.confirm('确定要删除该账号吗?', function () {
				app.showLoading('删除中');
		    app.post('ApiAdminUser/delUser', {id: id}, function (data) {
...
.content .item .f1 .t2 .x1{color: #222;font-size:30rpx;margin-bottom: 10rpx}
.content .item .f1 .t2 .x2{color: #999;font-size:24rpx;margin-bottom: 10rpx}
