...
<view>
    ...
    <view class="content" :style="bgpic?'background-image:url('+bgpic+');background-repeat: no-repeat;background-size:100% 100%;':''">
      ...
      <!-- s -->
      <view v-if="set.show_gold_price==1 || set.show_bonus_pool==1" style="width: 100%;padding: 40rpx 30rpx;">
        <view style="width: 680rpx;margin: 0 auto;display: flex;justify-content: space-between;align-items: center;">
          <view v-if="set.show_gold_price==1" style="text-align: center;">
            <view>
              <text style="color: #FE0000;font-size: 50rpx;font-weight: bold;">{{set.gold_price || 0}}</text>
            </view>
            <view><text>实时价格</text></view>
          </view>
          <view v-if="set.show_bonus_pool==1" style="text-align: left;line-height: 40rpx;">
            <view style="display: flex;"  >
              <view style="width: 170rpx;">{{t('金币')}}总量</view> <view>{{set.gold_total}}</view>
            </view>
            <view style="display: flex;margin-top: 10rpx;">
              <view style="width: 170rpx;">{{t('奖金池')}}金额</view> <view>{{set.bonus_pool_total}}</view>
            </view>
          </view>
        </view>
      </view>
        ...
        <view class="box" style="width: 680rpx;margin: 0 auto;">
          <view class="box-main range" :style="{borderColor:t('color1')}">
            <view class="range-item" :style="range==1?'color:'+t('color1'):''" @tap="changeRange(1)">
              <view class="range-item-name" :style="range==1?'border-bottom:4rpx solid '+t('color1'):''">日</view>
            </view>
      ...
      <view style="width: 100%;" :style="centerpic?'background-image:url('+centerpic+');background-repeat: no-repeat;background-size:100% 100%;':''">
        <view style="width: 680rpx;margin: 0rpx auto;display: flex;justify-content: space-between;padding: 30rpx 0;">
          <view style="text-align: center;">
            <view><text>我的{{t('金币')}}</text></view>
            <view>
              <text style="color: #FE0000;font-size: 50rpx;font-weight: bold;">{{member.gold}}</text>
            </view>
            <view><text>实时现金价值：</text> <text style="color: #FE0000;">{{member.allgold_price || 0}}元</text></view>
          </view>
        </view>
        <view style="width: 100%;">
          <view class="optred">
            ...
            <view @tap="goto" data-url="/pagesC/bonuspoolgold/withdraw" class="optred1" :style="btnpic?'background-image:url('+btnpic+');background-size:100% 100%;background-repeat: no-repeat;':'background:'+t('color1')">
    ...
    async init() {
      ...
      chart.setOption(this.option)
		...
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiBonusPoolGold/bonuspool', {range:that.range}, function (res) {
				that.loading = false;
				if(res.status == 1){
...
<style>
  ...
  .optred{display: flex;justify-content:space-between;width:680rpx;margin:0rpx auto;color: #fff;text-align: center;padding: 40rpx 0;}
  ...
  .jointitle{text-align: left;width:680rpx;margin: 0rpx auto;font-size: 30rpx;font-weight: bold;line-height: 70rpx;color: #FF593E;display: flex;justify-content: space-between;}
  ...
  .range .range-item-name{width: 40rpx;text-align: center;}
