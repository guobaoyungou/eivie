...
<view>
	<block v-if="isload">
		...
		<!--  #endif -->
			<view class="content">
				<view class="item" v-for="(item,index1) in datalist" :key="index1" @tap="goto" :data-url="'payorderdetail?id=' + item.id">
					<view class="left">
						<view class="f2">
							...
							<text class="t2" v-if="item.paynum && item.paynum != 0" style="white-space: nowrap;">支付单号：{{item.paynum}}</text>
						</view>
					</view>
					<view class="right">
						<view>
							<view class="money" :style="{color:t('color1')}">￥{{item.money}}</view>
							<view v-if="item.refund_money>0" class="refund-money">已退:￥{{item.refund_money}}</view>
						</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var keyword = that.keyword;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminFinance/payorderlog', {keyword:keyword,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1){
