...
<view>
	<block v-if="isload">
		...
		<!--  #endif -->
		<block v-for="(item, index) in datalist" :key="index" v-if="datalist && datalist.length>0">
			<view class="content">
				<view class="item">
					<view class="left">
						<view class="f2">
							<view class="t2">提现金额：{{item.money}}</view>
							<view class="t2">现金支付比例：{{item.withdrawfee_cash_rate}}%</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var keyword = that.keyword;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminFinance/withdrawfeeCashLog', {keyword:keyword,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
