<template>
	<view>
		<form @submit="subform">
			<view class="form-options" style="padding: 10rpx 0;color: red;" v-if="status == 2">
				文章未审核通过，驳回原因：{{info.reason}}
			</view>
		<view class="form-view flex flex-col">
			...
			<view class="form-options flex-col">
				<view class="options-title">详情页显示</view>
		...
		<view class="form-view flex flex-col" style="padding: 0rpx;">
			...
			<view class="form-options flex flex-bt flex-y-center" v-if="needcheck==0">
	...
	export default{
		...
		methods: {
			...
			getdata:function(){
				...
				var id = that.id || 0;
				that.loading = true;
				app.get('ApiAdminArticle/edit', {id:id}, function (res) {
					that.loading = false;
					console.log(res)
					that.clist = res.data
					...
					that.cnames = res.info.cnames || ''
					that.cids = res.info.cids || []
					that.sexIndexArr = res.info.zhanshi || []
					that.canpl = res.info.canpl || 0
					that.canplrp = res.info.canplrp || 0
					that.pinglun_check = res.info.pinglun_check || 0
					that.status = res.info.status || 0
					that.pagecontent = res.pagecontent || []
					that.needcheck = res.needcheck || 0
				})
			},
			  ...
			  app.post('ApiAdminArticle/save', {info:formdata,pagecontent:that.pagecontent}, function (res) {
			    if (res.status == 0) {
			      ...
			      setTimeout(function () {
							...
							eventChannel.emit('refreshData', {refresh: true});
							uni.navigateBack();
			      }, 1000);
			    }
			  });
