...
<!--	<view class="order-content" id="datalist">-->
  <view class="coupon-list">
    <view v-for="(item, index) in datalist" :key="index" class="coupon">
      <view class="order-box">
        <view class="content" style="border-bottom:none">
          <view class="pt_right">
            <view class="f1">
              <view class="t1">出水口编号：{{item.outlet_num}}</view>
              <view class="t3" :style="item.bid>0?'margin-top:0':'margin-top:10rpx'">出水口单价：{{item.outlet_price}}元/升</view>
              <view class="t3" :style="item.bid>0?'margin-top:0':'margin-top:10rpx'">设备编号：{{item.deviceId}}</view>
              <view class="t3" :style="item.bid>0?'margin-top:0':'margin-top:10rpx'">添加时间 {{item.createtime}}</view>
            </view>
          </view>
        </view>
        <view class="op">
          <view class="btn2" style="width: 240rpx" @click="copyUrl(item.qrcode)">复制二维码链接</view>
          <view class="btn2" style="width: 200rpx" @click="showQr" :data-qrcode="item.qrcode">查看二维码</view>
          <view @tap="goto" :data-url="'outletEdit?id='+item.id" class="btn2">编辑</view>
	...
	<view class="bottom-but-view notabbarbot">
		<button class="savebtn" :style="'background:linear-gradient(90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',1) 100%)'" @tap="goto" :data-url="'outletEdit?water_happyti_device_id='+water_happyti_device_id">添加出水口</button>
	</view>
  <uni-popup id="dialogQr" ref="dialogQr" type="dialog">
    <view class="hxqrbox">
      <image :src="qrcode" @tap="previewImage" :data-url="qrcode" class="img"/>
<!--      <view class="txt">请出示核销码给核销员进行核销</view>-->
      <view class="close" @tap="closeQr">
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.water_happyti_device_id = this.opt.id;
		this.getdata();
  },
  ...
  methods: {
    ...
    getdata: function (loadmore) {
     if(!loadmore){
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminWaterHappyti/getOutletList', {water_happyti_device_id:that.water_happyti_device_id,pagenum: pagenum}, function (res) {
        that.loading = false;
    ...
    todel: function (e) {
      ...
      app.confirm('确定要删除该出水口吗?', function () {
        app.post('ApiAdminWaterHappyti/delOutlet', {id: id}, function (res) {
          if (res.status == 1) {
            app.success(res.msg);
            that.getdata();
          } else {
    ...
    //查看核销码
    showQr:function(e){
      this.qrcode = e.currentTarget.dataset.qrcode;
      this.$refs.dialogQr.open();
    },
    closeQr:function(){
      this.$refs.dialogQr.close();
    },
