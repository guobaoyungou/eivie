...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','打水中','已结束']" :itemst="['all','0','1']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
			...
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box">
					<view class="head">
						...
						<text v-if="item.iscd==0" class="st2">打水中</text>
						<text v-if="item.iscd==1" class="st3">已结束</text>
					</view>
					<view class="content" >
						<view class="detail">
							<view class="t1">设备编号：{{item.deviceId}}</view>
							<view class="t1">设备地址：{{item.location}}</view>
							<view class="t1">出水口：{{item.outlet_num}}</view>
							<view class="t1">支付时间：{{item.paytime}}</view>
						</view>
					</view>
					<view class="content">
            <view class="detail">
              <view>订单金额:￥{{item.product_price}}</view>
              <view>实付金额:￥{{item.totalprice}}</view>
              <view >退款金额:￥{{item.refund_money}}</view>
              <view v-if="item.scoredk_money>0">积分抵扣:￥{{item.scoredk_money}}</view>
              <view v-if="item.coupon_money>0">优惠券抵扣:￥{{item.coupon_money}}</view>
            </view>
					</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminWaterHappyti/getOrderList', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
