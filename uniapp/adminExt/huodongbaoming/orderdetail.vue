...
<view class="container">
	<block v-if="isload">
		...
		<view class="product">
			<view class="content">
				<view @tap="goto" :data-url="'/pagesB/huodongbaoming/product?id=' + detail.proid">
					<image :src="detail.propic"></image>
				</view>
				<view class="detail">
					<text class="t1">{{detail.proname}}</text>
					
					<block v-if="detail.totalprice>0 && detail.totalscore>0">
						<text class="t2">实付金额：￥{{detail.totalprice}} + {{detail.totalscore}} </text>
					</block>
					<block v-if="detail.totalprice>0 && detail.totalscore==0">
						<text class="t2">实付金额：￥{{detail.totalprice}} </text>
					</block>
					<block v-if="detail.totalprice==0 && detail.totalscore>0">
						<text class="t2">实付金额：{{detail.totalscore}} {{t('积分')}}</text>
					</block>
		...
		<view class="orderinfo">
			<view class="item">
				...
				<image :src="detail.member.headimg" style="width:80rpx;height:80rpx;margin-right:8rpx"/>
				<text  style="height:80rpx;line-height:80rpx">{{detail.member.nickname}}</text>
			</view>
			<view class="item">
		...
		<view class="orderinfo">
			...
			<view class="item" v-if="detail.status>0 && detail.paytime">
				...
				<text class="t2" v-if="detail.totalmoney > 0 || detail.totalscore > 0">{{detail.paytype}}</text>
				<text class="t2" v-else="detail.totalmoney > 0 || detail.totalscore > 0">无须付款</text>
		...
		<view class="orderinfo">
			...
			<view class="item">
				...
				<text class="t2 red" v-if="detail.totalprice>0 && detail.totalscore>0">¥{{detail.totalprice}} + {{detail.totalscore}}{{t('积分')}}</text>
				<text class="t2 red" v-else-if="detail.totalprice>0">¥{{detail.totalprice}}</text>
				<text class="t2 red" v-else-if="detail.totalscore>0">{{detail.totalscore}}{{t('积分')}}</text>
				<text class="t2 red" v-else>¥{{detail.totalprice}}</text>
			</view>
			...
			<view class="item" v-if="detail.refund_status>0">
				...
				<text class="t2 red">{{detail.refund_reason ? detail.refund_reason : ''}}</text>
			</view>
		...
		<view class="bottom notabbarbot">
			...
			<view v-if="detail.status==1" class="btn2" @tap="hexiao" :data-id="detail.id">核销</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminHuodongOrder/orderdetail', {id: that.opt.id}, function (res) {
				that.loading = false;
    ...
    getdjs: function () {
      ...
      if (totalsec <= 0) {
		...
		setremarkconfirm: function (done, remark) {
			...
			var that = this
			app.post('ApiAdminHuodongOrder/setremark', { type:'huodong_baoming',orderid: that.detail.id,content:remark }, function (res) {
				app.success(res.msg);
				setTimeout(function () {
					that.getdata();
				}, 1000)
		...
		ispay:function(e){
			...
			app.confirm('确定要改为已支付吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHuodongOrder/ispay', { type:'huodong_baoming',orderid: orderid }, function (data) {
		...
		hexiao:function(e){
			...
			app.confirm('确定要核销并改为已完成状态吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHuodongOrder/hexiao', { type:'huodong_baoming',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		delOrder: function (e) {
			...
			app.confirm('确定要删除该订单吗?', function () {
				app.post('ApiAdminHuodongOrder/delOrder', { type:'huodong_baoming',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						app.goto('shoporder');
		...
		closeOrder: function (e) {
			...
			app.confirm('确定要关闭该订单吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHuodongOrder/closeOrder', { type:'huodong_baoming',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function (){
						that.getdata();
		...
		refundnopass: function (e) {
			...
			app.confirm('确定要驳回退款申请吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHuodongOrder/refundnopass', { type:'huodong_baoming',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
		...
		refundpass: function (e) {
			...
			app.confirm('确定要审核通过并退款吗?', function () {
				app.showLoading('提交中');
				app.post('ApiAdminHuodongOrder/refundpass', { type:'huodong_baoming',orderid: orderid }, function (data) {
					app.showLoading(false);
					app.success(data.msg);
					setTimeout(function () {
						that.getdata();
