	...
	<block v-if="isload">
		...
		<block v-for="(dateitem, index) in datalist" :key="index" v-if="datalist && datalist.length>0">
			<view class="content">
				...
				<view class="item" v-for="(item,index1) in dateitem.datelist" :key="index1" @tap.stop="goto" :data-url="'maidandetail?id=' + item.id">
					<view class="left">
						<view class="f2">
							...
							<text class="t2" v-if="item.money_transfer">转账金额：{{item.money_transfer}}</text>
              <text class="t2" v-if="item.transfer_check_name">转账状态：{{item.transfer_check_name}}</text>
              <view v-if="item.transfer_check_is" style="display: flex;">
                <view class="btn zhhk" @tap.stop="topayTransfer" :style="{background:t('color2')}" :data-payorderid="item.payorderid" :data-text="t('转账汇款')">{{t('转账汇款')}}</view>
              </view>
						</view>
					</view>
					<view class="right">
...
export default {
  ...
  methods: {
    ...
    topayTransfer:function(e){
      var that = this;
      var payorderid = e.currentTarget.dataset.payorderid;
      app.goto('/pagesExt/pay/transfer?id='+payorderid);
    },
  }
};
</script>
...
.btn{ height:65rpx;line-height: 65rpx;border-radius:10rpx;margin-top:26rpx;color: #fff;font-size: 26rpx;font-weight:bold;text-align: center;padding: 0 10rpx;width: 200rpx;}
</style>