	...
	<block v-if="isload">
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'goldbeanshoporderdetail?id=' + item.id">
					<view class="head">
					...
					<block v-for="(item2, idx) in item.prolist" :key="idx">
						<view class="content" :style="idx+1==item.procount?'border-bottom:none':''">
							<view @tap.stop="goto" :data-url="'/pages/scoreshop/product?id=' + item2.proid">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminOrder/goldbeanshoporder', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
			...
			app.post('ApiAdminOrder/getpeisonguser',{type:'gold_bean_shop_order',orderid:that.detail.id},function(res){
				...
				if(paidantype==1){
					...
					app.confirm(tips,function(){
						app.post('ApiAdminOrder/peisong', { type:'gold_bean_shop_order',orderid: that.detail.id,psid:0}, function (res) {
							app.success(res.msg);
							setTimeout(function () {
								that.getdata();
							}, 1000)
