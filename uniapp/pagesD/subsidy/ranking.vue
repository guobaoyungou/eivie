...
<view class="container"  >
	<view class="contentbox">
		<image :src="pre_url + '/static/imgsrc/rank_notext.png'" mode="widthFix">
			<view class="title">
				<view class="t1">每日收款排行榜</view>
				<view class="t2">每日消费排行榜</view>
			</view>
			<view class="top">
				<view class="t1">
					<view class="label">我的排名</view>	
					<view class="value">{{my_ranking}}</view>	
				</view>
				<view class="t2">
					<text style="color: #9e9e9e;">	{{ranktype==1?'累计收款':(ranktype==2?'累计消费':'')}}：</text>{{my_order_price}}
				</view>
			</view>
			
			<view class="content">
			
				<view class="tab1" v-if="rank_type.length > 1">
					<block v-for="(item, index) in rank_type" :key="index" >
						<view :class="'t1 '+(item==ranktype?'on':'')" :style="{color:(item==ranktype?t('color1'):'')}" :data-ranktype = "item" @tap="toranktype">{{item==1?'商家收款':(item==2?'会员消费':'')}}		
				...
				<view class="tab">
					...
					<view class="t2">{{ranktype==1?'商家':(ranktype==2?'会员':'')}}</view>
					<view class="t3" >{{ranktype==1?'商家收款':(ranktype==2?'会员消费':'')}}</view>
				</view>
				...
				<view class="itembox">	
					...
					<view class="item" :style="index < 3 ?'margin-top:20rpx':''">
							...
							<text class="t3" :style="{color:t('color1')}"> {{item.order_price}}</text>
					</view>
					</block>
				</view>
			</view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		app.post('ApiSubsidy/getRanking', {ranktype:ranktype,pagenum: pagenum}, function (res) {
			that.loading = false;
			var data = res.data;
			if(res.status==0){
					app.alert('排行榜未开启')
			...
			that.my_ranking = res.my_ranking;
			that.my_order_price = res.my_order_price;
			that.loaded();
