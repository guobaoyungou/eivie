<template>
	<view v-if="isload">
		...
		<!-- 选项卡 -->
		<dd-tab :itemdata="['日汇总',t('返现积分')+'明细','释放记录']" :itemst="['0','1','2']" :st="st" :showstatus="showstatus" :ismoney="1" :isfixed="false" @changetab="changetab"></dd-tab>
	...
	export default{
		...
		methods: {
		  ...
		  getday: function (loadmore) {
				...
				that.nomore = false;
				that.loading = true;
		    app.post('ApiSubsidy/subsidyScoreDay', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
		  ...
		  getlog: function (loadmore) {
		  	...
		  	that.nomore = false;
		  	that.loading = true;
		  	app.post('ApiSubsidy/subsidyScoreLog', {st: that.st,pagenum: pagenum,day_id:that.day_id,release_index:that.release_index}, function (res) {
		        that.loading = false;
		        var data = res.data;
...
<style>
	...
	.points-view .points-title-view{font-size: 32rpx;color: #ecdd36;align-items: center;width: 100%;justify-content: center;}
