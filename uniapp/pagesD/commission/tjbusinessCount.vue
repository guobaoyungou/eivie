	...
	<block v-if="isload">
		<view class="topfix">
			<view class="tongji flex-y-center "   :style="{background:color1?color1:t('color1')}">
				<view class="stat-item" @tap="goto" data-url="/pagesExt/commission/tjbusinessList">
					<view class="price">{{count.count}}</view>
					<view>商家数量</view>
				</view>
				<view class="stat-item">
					<view class="price">{{count.order_price_total}}</view>
					<view>营业额</view>
				</view>
				<view class="stat-item mb0">
					<view class="price">{{count.commission_today}}</view>
					<view>今日收益</view>
				</view>
				<view class="stat-item mb0">
					<view class="price">{{count.commission_total}}</view>
					<view>累计收益</view>
				</view>
			</view>
			<dd-tab :itemdata="['待结算','已结算']" :itemst="['1','2']" :st="st" :isfixed="false" @changetab="changetab"></dd-tab>
		</view>
		<view style="margin-top:340rpx"></view>
		...
		<view class="content">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				...
				<view class="f2">
					<view class="t1">
						...
						<view class="x3"><image :src="item.b_logo"></image>{{item.b_name}}</view>
					</view>
					<view class="t2">
			...
			<block v-if="st==2">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1"><text>{{item.remark}}</text></view>
				<view class="f2">
					<view class="t1">
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.nodata = false;
			that.nomore = false;
			app.get('ApiAgent/tjbcount',{st:st,pagenum: pagenum},function(res){
				that.loading = false;
...
.tongji{width: 94%;margin: 30rpx 3%;padding: 30rpx;border-radius: 20rpx; background-color: #F2350D;color: #fff;text-align: center;display: flex;flex-wrap: wrap;justify-content:space-between;}
.tongji .stat-item { width: 48%; margin-bottom: 35rpx;}
.tongji .price{font-size: 40rpx;font-weight: 700;}
.tongji .mb0{margin-bottom: 0}
</style>