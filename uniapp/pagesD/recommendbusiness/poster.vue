...
export default {
  data() {
    return {
		...
		posterid:'',
		shopset: [],
		share_url:'/pagesExt/business/apply'
    };
  },
  ...
  onShareAppMessage: function (res){
    var that = this;
    var imageUrl = app.globalData.initdata.logo;
    var title = app.globalData.initdata.name;
    return {
      title: title,
      path: that.share_url + '?scene=pid_' + app.globalData.mid,
      imageUrl: imageUrl,
  ...
  methods: {
		getdata:function(){
			...
			app.get('ApiAgent/memberBusinessPoster', {posterid:posterid}, function (res) {
				...
				that.loaded({title:app.globalData.initdata.name,pic:app.globalData.initdata.logo,link:app.globalData.pre_url +'/h5/'+app.globalData.aid+'.html#'+ that.share_url + '?scene=pid_' + app.globalData.mid});
			});
		},
		sharemp:function(){
        ...
        success: function (res){
					if(res.tapIndex >= 0){
						...
						sharedata.href = app.globalData.pre_url +'/h5/'+app.globalData.aid+'.html#'+that.share_url + '?scene=pid_' + app.globalData.mid;
