	...
	<block v-if="isload">
		...
		<view class="content" id="datalist">
			...
			<view class="item">
				...
				<view class="f2">
					<block v-if="item.bonus>0">
						<text class="t1">+{{item.bonus}}</text>
					</block>
					<block v-else>
						<text class="t2">{{item.bonus}}</text>
					</block>
				</view>
			</view>
		</block>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.bonus_type = this.opt.bonus_type || '';
		this.getdata();
  },
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
		app.post('ApiNetworkHelp/bonuslog', {st: st,pagenum: pagenum,bonus_type:that.bonus_type}, function (res) {
			that.loading = false;
			var data = res.data;
			var title = res.title;
			that.title = title;
