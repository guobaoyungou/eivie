...
<view>
	<block v-if="isload">
		...
		<view class="header">
			<view class="price_share">
				<view class="price">
					<view class="f1" :style="{color:t('color1')}">{{product.score_price}}{{t('金豆')}}<text v-if="product.money_price*1>0">+{{product.money_price}}元</text></view>
				...
				<view class="price2" v-if="product.show_lvprice==1">
					<view v-for="(item,index) in product.new_lvprice_data" :class="item.is_select==1?'f1':'f2'">
						{{item.name}}价:{{item.score}}{{t('金豆')}}<text v-if="item.money*1>0">+{{item.money}}元</text>
					</view>
				</view>
    ...
    <view class="cuxiaodiv" v-if="product.givescore">
    	<view class="cuxiaopoint">
    		<view class="f0">送{{t('金豆')}}</view>
    		<view class="f1" style="font-size:26rpx">购买可得{{t('金豆')}}{{product.givescore}}个</view>
    	</view>
    </view>
				...
				<view class="title flex">
					...
					<view class="flex1">
						<view  class="price" :style="{color:t('color1')}">
							{{nowguige.score_price}}{{t('金豆')}}<text v-if="nowguige.money_price*1>0">+{{nowguige.money_price}}元</text>
							<text v-if="nowguige.market_price > 0" class="t2">￥{{nowguige.market_price}}</text>
						</view>
						<view class="stock" >库存：{{nowguige.stock}}</view>
	...
	methods: {
      ...
      var data = {
        ...
        mendian_id:that.mendian_id
			...
			app.post('ApiGoldBeanShop/product', data, function (res) {
				that.loading = false;
				if (res.status == 0) {
					app.alert(res.msg);
					return;
      ...
      if(that.scoreshop_everytime_buymin && that.product.everytime_buymin > 0 && num < this.product.everytime_buymin){
        ...
        return;
			...
			that.loading = true;
			app.post('ApiGoldBeanShop/addcart', {proid:proid,ggid:ggid,num:num}, function (res) {
				that.loading = false;
				if (res.status == 1) {
					app.success(res.msg);
					that.cartnum+=num;
      ...
      if(this.scoreshop_everytime_buymin && this.product.everytime_buymin > 0 && num < this.product.everytime_buymin){
		...
		addfavorite: function () {
			var that = this;
			var proid = that.product.id;
			app.post('ApiGoldBeanShop/addfavorite', {proid: proid,type: 'gold_bean_shop'}, function (data) {
				if (data.status == 1) {
		...
		showPoster: function () {
			...
			app.post('ApiGoldBeanShop/getposter', {proid: that.product.id}, function (data) {
