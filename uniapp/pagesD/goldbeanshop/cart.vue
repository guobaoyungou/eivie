		...
		<block v-if="cartlist.length>0">
						...
						<view class="content" v-for="(item2,index2) in item.prolist" :key="index2">
							...
							<view class="proinfo" :style="(item.prolist).length == index2+1 ? 'border:0' : ''">
								...
								<view class="detail">
									...
									<view class="price" :style="{color:t('color1')}"><text v-if="item2.product.money_price>0">￥{{item2.product.money_price}}+</text>{{item2.product.score_price}}{{t('金豆')}}</view>
			...
			<view style="height:auto;position:relative">
				...
				<view class="footer flex" :class="menuindex>-1?'tabbarbot':'notabbarbot'">
					...
					<view class="text2" :style="{color:t('color1')}"><text v-if="totalmoney>0">￥{{totalmoney}}+</text><text>{{totalscore}}{{t('金豆')}}</text></view>
					<view class="flex1"></view>
  ...
  methods: {
    getdata: function (){
      var that = this;
			...
			if(bid){
				that.scoreshopindexurl = 'index?bid='+bid;
			}
			that.loading = true;
      app.get('ApiGoldBeanShop/cart', {bid:bid,isnew:1}, function (res) {
				that.loading = false;
    ...
    cartdelete: function (e) {
      ...
      app.confirm('确定要从购物车移除吗?', function () {
        app.post('ApiGoldBeanShop/cartdelete', {id: id}, function (data) {
          app.success('操作成功');
          setTimeout(function () {
            that.getdata();
          }, 1000);
		...
		cartdeleteb:function(e){
			...
			app.confirm('确定要从购物车移除吗?', function () {
				app.post('ApiGoldBeanShop/cartdelete', {bid: bid}, function (data) {
					app.success('操作成功');
					setTimeout(function () {
						that.getdata();
					}, 1000);
    ...
    gwcplus: function (e) {
			...
			this.calculate();
			var that = this;
			app.post('ApiGoldBeanShop/cartChangenum', {id: cartid,num: num + 1}, function (data){
				if (data.status == 1) {
					//that.getdata();
				} else {
					app.error(data.msg);
    ...
    gwcinput: function (e) {
			...
			var that = this;
			app.post('ApiGoldBeanShop/cartChangenum', {id: cartid,num: newnum}, function (data) {
				if (data.status == 1) {
					//that.getdata();
				} else {
					app.error(data.msg);
