		...
		<view class="search-container" :style="history_show?'height:100%;':''">
			...
			<view class="search-navbar" v-show="!history_show">
				...
				<view @tap.stop="sortClick" class="search-navbar-item" data-field="score_price" :data-order="order=='asc'?'desc':'asc'">
					<text :class="field=='score_price'?'active':''">所需{{t('金豆')}}</text>
...
export default {
  ...
  methods: {
		getdata:function(){
			...
			that.loading = true;
			app.get('ApiGoldBeanShop/prolist', {bid:that.bid,cid: cid,gid: gid}, function (res) {
				that.loading = false;
    ...
    getprolist: function () {
      ...
      that.history_show = false;
			...
			that.nodata = false;
      that.nomore = false;
      app.post('ApiGoldBeanShop/getprolist',{bid:that.bid,pagenum: pagenum,keyword: keyword,field: field,order: order,gid: gid,cid: cid,cpid:cpid}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
          that.datalist = data;
...
.product-item .itemcontent{border-bottom:1px solid #E6E6E6;padding:20rpx 0;width: ;}
