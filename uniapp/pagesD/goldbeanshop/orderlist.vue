				...
				<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
					...
					<block v-for="(item2, idx) in item.prolist" :key="idx">
						<view class="content" :style="idx+1==item.procount?'border-bottom:none':''">
							...
							<view class="detail">
								...
								<view class="t3"><text class="x1 flex1"><text v-if="item2.money_price>0">￥{{item2.money_price}}+</text>{{item2.score_price}}{{t('金豆')}}</text><text class="x2">×{{item2.num}}</text></view>
							</view>
						</view>
					</block>
					<view class="bottom">
						<text>共计{{item.procount}}件商品 实付:￥{{item.totalprice}}</text>
						<text v-if="item.refund_status==1" style="color:red"> 退款中￥{{item.refund_money}}  <text v-if="item.refund_gold_bean > 0">+{{item.refund_gold_bean}}{{t('金豆')}}</text></text>
						<text v-if="item.refund_status==2" style="color:red"> 已退款￥{{item.refund_money}}  <text v-if="item.refund_gold_bean > 0">+{{item.refund_gold_bean}}{{t('金豆')}}</text></text>
					...
					<view class="op">
						...
						<block v-if="item.status==2">
							...
							<view class="btn1" :style="{background:t('color1')}" @tap.stop="orderCollect" :data-id="item.id" v-if="item.paytypeid!='4' && !item.type  ">确认收货</view>
						</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiGoldBeanShop/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
    ...
    orderCollect: function (e) {
      ...
      app.confirm('确定要收货吗?', function () {
        app.post('ApiGoldBeanShop/orderCollect', {orderid: orderid}, function (data) {
          app.success(data.msg);
          setTimeout(function () {
            that.getdata();
          }, 1000);
