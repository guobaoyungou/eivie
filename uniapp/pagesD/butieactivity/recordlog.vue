		...
		<view class="content">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
						...
						<text v-if="item.remark" class="t2">{{item.send_day}}{{item.remark}}</text>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.butie_member_product_id = this.opt.butie_member_product_id || 0;
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var butie_member_product_id = that.butie_member_product_id;
      var pro_id = that.pro_id;
			that.nodata = false;
			that.nomore = false;
			that.loading = true;
      app.post('ApiButieActivity/recordlog', {pro_id: pro_id,butie_activity_id: butie_activity_id,butie_member_product_id,butie_member_product_idpagenum: pagenum}, function (res) {
				that.loading = false;
