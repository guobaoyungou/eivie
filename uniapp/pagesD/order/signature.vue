...
export default {
	...
	methods: {
		confirm:function(){
			...
			var signatureurlShop = this.signatureurlShop;
			app.setCache('signatureurlShop',signatureurlShop);
			/*app.showLoading('提交中');
		...
		goback2: function(isreload) {
			...
			if (isreload && pages.length > 1) {
				...
				prePage.$vm.signatureurlShop=this.signatureurlShop
				prePage.$vm.hidehetong();
			}
			if (pages.length == 1) {
				...
				success: (res) => {
					...
					uni.uploadFile({
						...
						success: (uploadFileRes) => {
							...
							that.signatureurlShop = data.url;
							that.confirm();
						}
					})
				},
