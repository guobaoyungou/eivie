...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			...
			<view class="f2">{{myscore_lock}}</view>
		</view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
		app.post('ApiNewScore/score_lock_log', {st: st,pagenum: pagenum}, function (res) {
			that.loading = false;
			var data = res.data;
			if (pagenum == 1) {
				uni.setNavigationBarTitle({
					title: that.t('冻结额度') + '明细'
				});
				that.myscore_lock = res.myscore_lock;
				that.datalist = data;
				that.set = res.set || '';
				if (data.length == 0) {
					that.nodata = true;
