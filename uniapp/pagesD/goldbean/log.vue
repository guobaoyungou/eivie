	...
	<block v-if="isload">
		...
		<view class="content" id="datalist">
			...
			<view class="item">
				...
				<view class="f2">
					<block v-if="item.gold_bean>0">
						<text class="t1">+{{item.gold_bean}}</text>
					</block>
					<block v-else>
						<text class="t2">{{item.gold_bean}}</text>
					</block>
				</view>
			</view>
		</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/goldbeanLog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        ...
        if (pagenum == 1) {
					...
					that.mygoldbean = res.mygoldbean;
					that.datalist = data;
					that.set = res.set;
          if (data.length == 0) {
            that.nodata = true;
