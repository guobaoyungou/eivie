	...
	<block v-if="isload">
			<form @submit="formSubmit">
					<view class="mymoney" :style="{background:t('color1')}">
							<view class="f1">我的可提现{{t('现金')}}</view>
							<view class="f2"><text style="font-size:26rpx">￥</text>{{userinfo.xianjin}}</view>
							<view class="f3" v-if="sysset.xianjinrecord_withdrawlog_show && sysset.xianjinrecord_withdrawlog_show == 1" @tap="goto" data-url="xianjinlog?st=1"><text>提现记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
					</view>
					<view class="content2">
							...
							<view class="tips-box">
								<view class="item4 tips" v-if="sysset.xianjinwithdrawfee>0 || sysset.xianjinwithdrawmin>0">
										<text v-if="sysset.xianjinwithdrawmin>0" style="margin-right:10rpx">最低提现金额{{sysset.xianjinwithdrawmin}}元 </text>
										<text v-if="sysset.xianjinwithdrawfee>0">提现手续费{{sysset.xianjinwithdrawfee}}% </text>
								</view>
							</view>
					</view>
                    ...
                    <view class="radio" :style="paytype == sysset.custom_name ? 'background:'+t('color1')+';border:0' : ''"><image class="radio-img" :src="pre_url+'/static/img/checkd.png'"/></view>
						...
						<view class="banklist" v-if="selectbank && paytype=='银行卡' && bank">
							  <view class="f1">默认银行卡：{{bank.bankname}} {{bank.bankcardnum}}</view>
								<view class="t2" @tap="goto" data-url="/pagesA/banklist/bank?fromPage=xianjin">修改</view>
						</view>
					</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiMy/xianjinWithdraw', {bid:that.bid}, function (res) {
				that.loading = false;
    ...
    moneyinput: function (e) {
      var usermoney = parseFloat(this.userinfo.xianjin);
      var money = parseFloat(e.detail.value);
      if (money < 0) {
        app.error('必须大于0');
      } else if (money > usermoney) {
        app.error('可提现' + this.t('现金') + '不足');
      }
			this.money = money;
    },
    ...
    toFormSubmit(){
      ...
      var paytype = this.paytype;
      app.showLoading('提交中');
      app.post('ApiMy/xianjinWithdraw', {money: money,paytype: paytype,bid:that.bid}, function (data) {
        ...
        if (data.status == 0) {
          ...
          if(that.sysset.xianjinrecord_withdrawlog_show == 1){
            that.subscribeMessage(function () {
            setTimeout(function () {
              app.goto('xianjinlog?st=1');
            }, 1000);
            });
          }else{
           that.getdata();
