    ...
    <view class="container">
        <block v-if="isload">
            ...
            <view class="order-tab">
                <scroll-view id="top-scroll-view" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="'top-category-'+currentTopCategoryId" class="order-tab2">
                    <view v-for="(item, index) in clist" :key="index" :id="'top-category-'+item.id" :class="'item' + (curTopIndex == index ? ' active' : '')" @tap="switchTopTab"
                        :data-index="index" :data-id="item.id">
                        <view class="item-inner">
                            <image class="icon" :style="{borderColor:curTopIndex == index ? t('color1'):'none'}" :src="item.pic" mode="aspectFill"></image>
                            <text class="name" :style="{background:curTopIndex == index ? t('color1'):'none'}">{{item.name.substring(0,4)}}</text>
                        </view>
                    </view>
                </scroll-view>
                <view class="stretch" @tap="showAllCategory">
                    <text>展</text>
                    <text>开</text>
                    <image :src="`${pre_url}/static/img/arrow-3.png`" style="width: 24rpx;height: 24rpx;" mode="widthFix"></image>
                </view>
            </view>
            <view v-show="allCategoryVisible" class="category-popup">
                <view class="category-popup-overlay" @tap.stop="handleClickMask"></view>
                <view class="category-popup-inner">
                    <view class="category-popup-title">
                        <text class="title">全部分类</text>
                        <view class="shrink" @tap="hideAllCategory">
                            <text>收</text>
                            <text>起</text>
                            <image :src="`${pre_url}/static/img/arrow-3.png`" style="width: 24rpx;height: 24rpx;transform: rotate(180deg);" mode="widthFix"></image>
                        </view>
                    </view>
                    <view class="category-popup-content">
                        <view v-for="(item, index) in clist" :key="index" :class="'item' + (curTopIndex == index ? ' active' : '')" @tap="switchTopTab"
                            :data-index="index" :data-id="item.id" :data-popup="1">
                            <image class="icon" :style="{borderColor:curTopIndex == index ? t('color1'):'none'}" :src="item.pic" mode="aspectFill"></image>
                            <text class="name" :style="{background:curTopIndex == index ? t('color1'):'none'}">{{item.name.substring(0,4)}}</text>
                        </view>
                    </view>
            ...
            <view class="content-container">
                <view class="nav_left">
                    <view :class="'nav_left_items ' + (curIndex == -1 ? 'active' : '')"
                        :style="{color:curIndex == -1?t('color1'):'#333'}" @tap="switchRightTab" data-index="-1"
                        :data-id="clist[curTopIndex].id">
                        <view class="before" :style="{background:t('color1')}"></view>全部
                    </view>
                    <block v-for="(item, index2) in clist[curTopIndex].child" :key="index2">
                        <view :class="'nav_left_items ' + (curIndex == index2 ? 'active' : '')"
                            :style="{color:curIndex == index2?t('color1'):'#333'}" @tap="switchRightTab"
                            :data-index="index2" :data-id="item.id">
                ...
                <view class="nav_right">
                    <view class="nav_right-content">
                        <view class="nav-pai">
                            <view class="nav-paili" :style="{color:(!field||field=='sort')?t('color1'):'#323232'}"
                                @tap="changeOrder" data-field="sort" data-order="desc">综合</view>
                            <view class="nav-paili" :style="field=='sales'?'color:'+t('color1'):''" @tap="changeOrder"
                                data-field="sales" data-order="desc">销量</view>
                            <view class="nav-paili" @tap="changeOrder" data-field="sell_price"
                        ...
                        <view class="classify-ul"
                            v-if="curTopIndex>-1 && curIndex>-1 && clist[curTopIndex].child[curIndex].child.length>0">
                            <view class="flex" style="width:100%;overflow-y:hidden;overflow-x:scroll;">
                                <view class="classify-li"
                                    :style="curIndex2==-1?'color:'+t('color1')+';background:rgba('+t('color1rgb')+',0.2)':''"
                                    @tap="changeCTab" :data-id="clist[curTopIndex].child[curIndex].id" data-index="-1">
                                    全部</view>
                                <block v-for="(item, idx2) in clist[curTopIndex].child[curIndex].child" :key="idx2">
                                    <view class="classify-li"
                                        :style="curIndex2==idx2?'color:'+t('color1')+';background:rgba('+t('color1rgb')+',0.2)':''"
                                        @tap="changeCTab" :data-id="item.id" :data-index="idx2">{{item.name}}</view>
                                </block>
                            </view>
                        </view>
                        <scroll-view class="classify-box" scroll-y="true" @scrolltolower="scrolltolower">
                            <view class="product-itemlist">
                                <view class="item" v-for="(item,index) in datalist" :key="item.id" @click="goto"
                                    ...
                                    <view class="product-info">
                                        ...
                                        <view v-if="item.price_show_type =='0' || !item.price_show_type ">
                                            <view class="p2" v-if="item.price_type != 1 || item.sell_price > 0">
                                                <text class="t1" :style="{color:t('color1')}"><text
                                                        style="font-size:20rpx;padding-right:1px">￥</text>{{item.sell_price}}
                                                    <text v-if="item.price_show && item.price_show_text"
                                                        style="margin: 0 15rpx;font-size: 24rpx;font-weight: 400;">{{item.price_show_text}}</text>
                                                    <text
                                                        v-if="!isNull(item.service_fee) && item.service_fee > 0">+{{item.service_fee}}{{t('服务费')}}</text></text>
                                                <text class="t2"
                                                ...
                                                <view class="flex"
                                                    ...
                                                    <view class="member flex" :style="'border-color:' + t('color1')">
                                                        <view :style="{background:t('color1')}"
                                                            class="member_lable flex-y-center">{{item.level_name_show}}
                                                        </view>
                                            ...
                                            <view v-if="item.is_vip == '1'">
                                                ...
                                                <view class="p2" v-if="item.price_type != 1 || item.sell_price > 0">
                                                    <text class="t1" :style="{color:t('color1')}">
                                                        ...
                                                        <text
                                                            :style="item.lvprice =='1'?'font-size:26rpx;':'font-size:32rpx;'">{{item.sell_price_origin}}</text>
                                                        <text v-if="item.price_show && item.price_show_text"
                                                            style="margin: 0 15rpx;font-size: 24rpx;font-weight: 400;">{{item.price_show_text}}</text>
                                                    </text>
                                                    <text class="t2"
                                        ...
                                        <!-- 商品处显示会员价 -->
                                        <view v-if="item.price_show && item.price_show == 1"
                                            style="line-height: 46rpx;">
                                        ...
                                        <view v-if="item.priceshows && item.priceshows.length>0">
                                            <view v-for="(item2,index2) in item.priceshows" style="line-height: 46rpx;">
                                                ...
                                                <text
                                                    style="margin-left: 15rpx;font-size: 22rpx;font-weight: 400;">{{item2.price_show_text}}</text>
                                            </view>
                                        </view>

                                        <view class="p2"
                                            v-if="item.xunjia_text && item.price_type == 1 && item.sell_price <= 0"
                                            ...
                                            <block v-if="item.xunjia_text && item.price_type == 1">
                                                <view class="lianxi" :style="{background:t('color1')}"
                                                    @tap.stop="showLinkChange" :data-lx_name="item.lx_name"
                                                    :data-lx_bid="item.lx_bid" :data-lx_bname="item.lx_bname"
                                                    :data-lx_tel="item.lx_tel" data-btntype="2">
            ...
            <view class="main">
                ...
                <view class="content">
                    ...
                    <view class="row" v-if="lx_bid > 0">
                        ...
                        <view class="f2" style="width: 100%;max-width: 470rpx;display: flex;" @tap="goto"
                            :data-url="'/pagesExt/business/index?id='+lx_bid">
                            <view style="width: 100%;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;">
                                {{lx_bname}}</view>
                    ...
                    <view class="row" v-if="lx_tel">
                        ...
                        <view class="f2" style="width: 100%;max-width: 470rpx;" @tap="goto" :data-url="'tel::'+lx_tel"
                            :style="{color:t('color1')}">{{lx_tel}}
                            <image :src="pre_url+'/static/img/copy.png'" class="copyicon" @tap.stop="copy"
                                :data-text="lx_tel"></image>
                        </view>
                    </view>
                </view>
            </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            scrollTopCategory(id) {
                const query = uni.createSelectorQuery().in(this);
                query.select(`#top-category-${id}`).boundingClientRect();
                query.select('#top-scroll-view').boundingClientRect();
                query.exec(res => {
                  const el = res[0];
                  const scroll = res[1];
                
                  const isVisible = el.left >= scroll.left && el.right <= scroll.right;
                  if (isVisible) {
                      this.currentTopCategoryId = 0;
                  } else {
            ...
            switchTopTab: function(e) {
                ...
                var id = e.currentTarget.dataset.id;
                var index = parseInt(e.currentTarget.dataset.index);
                if (e.currentTarget.dataset.popup) {
                    this.allCategoryVisible = false;
                    this.scrollTopCategory(id);
                } else {
                    this.currentTopCategoryId = 0;
                }
...
<style>
    ...
    .order-tab2 .item .item-inner {
        display: flex;
        flex-direction: column;
        align-items: center;
    }
    .order-tab2 .item .icon {
        width: 90rpx;
        height: 90rpx;
        border-radius: 100%;
    }
    .order-tab2 .item.active .icon {
        border-width: 2rpx;
        border-style: solid;
        box-sizing: border-box;
    }
    .order-tab2 .item .name {
        font-size: 24rpx;;
        width: 120rpx;
        margin-top: 10rpx;
        color: #080808;
    ...
    .order-tab2 .item.active .name {
        color: #fff;
    }
    .order-tab2 /deep/.uni-scroll-view {
        scrollbar-width: none;
    }
    .order-tab2 /deep/.uni-scroll-view::-webkit-scrollbar {
        display: none;
    }
    ...
    .order-tab .stretch {
        ...
        font-size: 24rpx;
        color: #080808;
        box-shadow: -8rpx 0 6rpx 2rpx rgba(0,0,0,0.06);
    }
    ...
    .category-popup-title .title {
        margin-left: 30rpx;
        font-size: 26rpx;
    }
    .category-popup-title .shrink {
        display: flex;
        flex-direction: column;
        align-items: center;
        font-size: 24rpx;
    ...
    .category-popup-content .item .icon {
        width: 90rpx;
        height: 90rpx;
        border-radius: 100%;
    }
    .category-popup-content .item .name {
        font-size: 24rpx;;
        width: 120rpx;
        margin-top: 10rpx;
        color: #080808;
    ...
    .category-popup-content .item.active .icon {
        border-width: 2rpx;
        border-style: solid;
        box-sizing: border-box;
    }
    .category-popup-content .item.active .name {
        color: #fff;
    }
