...
export default {
	...
	methods: {
		...
		goPay: function() {
			// 调试：打印订单数据
			console.log('[unified-order-card] 订单数据:', this.item);
			console.log('[unified-order-card] payorderid:', this.item.payorderid);
			
			// 检查是否有支付订单ID
			if (!this.item.payorderid || this.item.payorderid == 0) {
				uni.showToast({
					title: '支付订单不存在，请先进入订单详情',
					icon: 'none',
					duration: 2500
				});
				// 跳转到详情页
			...
			console.log('[unified-order-card] 跳转到支付页，payorderid:', this.item.payorderid);
