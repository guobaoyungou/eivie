...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}" >
			<view class="f1">我的{{t('金值')}}</view>
			<view class="f2">{{mygoldmoney}}</view>
		</view>
		<view class="content" id="datalist">
			...
			<view class="item">
				...
				<view class="f2">
					<block v-if="item.goldmoney>0">
						<text class="t1">+{{item.goldmoney}}</text>
					</block>
					<block v-else>
						<text class="t2">{{item.goldmoney}}</text>
					</block>
				</view>
			</view>
		</block>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
    app.post('ApiMy/goldmoneylog', {st: st,pagenum: pagenum}, function (res) {
      that.loading = false;
      var data = res.data;
      if (pagenum == 1) {
        ...
        that.mygoldmoney = res.mygoldmoney;
        that.datalist = data;
        if (data.length == 0) {
          that.nodata = true;
        }
