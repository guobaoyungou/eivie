...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}" >
			<view class="f1">我的{{t('佣金上限')}}</view>
			<view class="f2">{{commission_max}}</view>
		</view>
				...
				<view class="item">
					...
					<view class="f2">
						<block v-if="item.value>0">
							<text class="t1" style="color: #03bc01;">+{{item.value}}</text>
						</block>
						<block v-else>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
      app.post('ApiMy/getcommissionmaxlog', {st: st,pagenum: pagenum}, function (res) {
		that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
			that.commission_max = res.commission_max;
			that.datalist = data;
			if (data.length == 0) {
				that.nodata = true;
			}
