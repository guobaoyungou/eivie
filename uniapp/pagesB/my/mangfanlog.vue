          ...
          <view class="item">
            <view class="f1" style="width: 100%;">
                ...
                <block v-if="item.status>=0">
                  <view v-if="item.commission" style="line-height: 50rpx;">
                      <text class="t1" v-if="item.status == 1">分佣：</text>
                      <text class="t1" v-if="item.status == 0">预估分佣：</text>
											<text class="t1" style="margin-left: 10rpx;">{{item.commission}}{{t('佣金')}}</text>
                  </view>
                ...
                <text class="t2" v-if="item.from_ordernum">订单编号：{{item.from_ordernum}}</text>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMangfan/mangfanlog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
