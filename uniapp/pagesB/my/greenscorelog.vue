...
<view class="container">
	<block v-if="isload">
		<!-- <view class="myscore" :style="{background:t('color1')}" >
			...
			<view class="f2">{{myscore}}<view class="btn-mini" @tap="goto" :data-url="withdraw_url">提取</view></view>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.jilu_id = this.opt.jilu_id || 0;
		this.getdata();
  },
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
      app.post('ApiGreenScore/scorelog', {st: st,pagenum: pagenum,jilu_id:that.jilu_id}, function (res) {
		that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
