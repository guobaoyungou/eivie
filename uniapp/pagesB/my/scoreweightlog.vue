...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}" >
			<view class="f1">我的积分权</view>
			<view class="f2">{{myscoreweight}}</view>
		</view>
		<view class="content" id="datalist">
			<block v-for="(item, index) in datalist" :key="index"> 
			<view class="item-moudle" style="background-color: #FFFFFF;">
				...
				<view class="item">
					...
					<view class="f2">
						<block v-if="item.score_weight>0">
							<text class="t1" style="color: #03bc01;">+{{item.score_weight}}</text>
						</block>
						<block v-else>
							<text class="t2">{{item.score_weight}}</text>
						</block>
					</view>
				</view>
			</view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
      app.post('ApiMy/scoreweightlog', {st: st,pagenum: pagenum}, function (res) {
		that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
			
			that.myscoreweight = res.myscoreweight;
			that.datalist = data;
			if (data.length == 0) {
				that.nodata = true;
			}
...
.content .item-moudle{width:100%;margin:20rpx 0;background:#fff;border-radius:5px;padding:20rpx 20rpx;}
