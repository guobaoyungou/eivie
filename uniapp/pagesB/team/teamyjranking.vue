		...
		<view class="container" :style="{backgroundColor:t('color1')}" >
			<view class="contentbox" >
				<image :src="pre_url + '/static/imgsrc/rank.png'" mode="widthFix">
					...
					<view class="content">
						<view class="top">
							<view class="t1">我的{{t('团队业绩')}}：{{total_yeji}}</view>
		...
		<!-- 					<view class="t1">累计佣金：{{totalcommission}}</view>
		 -->				</view>
						...
						<view class="tab">
							...
							<view class="t3" :style="{color:t('color1')}">累计{{t('团队业绩')}}</view>
						</view>
						...
						<view class="itembox">	
							...
							<view class="item">
									...
									<text class="t3"> {{item.totalyeji}}</text>
							</view>
							</block>
						</view>
					</view>
...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      app.post('ApiAgent/teamyjrank', {pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data.data;
				that.total_yeji = res.data.total_yeji
