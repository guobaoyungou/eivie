...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','待付款','已完成','退款']" :itemst="['all','0','3','10']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		...
		<view class="order-content">
			...
			<view class="order-box" @tap="goto" :data-url="'orderdetail?id=' + item.id">
				<view class="head">
					...
					<text v-if="item.status==1" class="st1">使用中</text>
					<text v-if="item.status==3" class="st3">已到期</text>
				...
				<view class="content" style="border-bottom:none">
					...
					<view class="detail">
						...
						<text class="t2">{{item.basan}}</text>
						<view class="t3">
							<text class="x1 flex1">
								￥{{item.sell_price}}
							</text>
							<text class="x2">×{{item.basan_sum}}</text>
						</view>
					</view>
				</view>
				<view class="bottom">
					<text>共计{{item.num}}件商品 实付:<text v-if="showprice_dollar && item.usd_totalprice">${{item.usd_totalprice}} </text> 
				...
				<view class="op">
					<block v-if="([1,2,3]).includes(item.status) && item.invoice">
						<view class="btn2" @tap="goto" :data-url="'/pagesExt/order/invoice?type=seckill&orderid=' + item.id">发票</view>
					</block>
					...
					<block v-if="item.status==0">
						...
						<view class="btn1" :style="{background:t('color1')}" @tap="goto" :data-url="'/pagesExt/pay/pay?id=' + item.payorderid">去付款</view>
					</block>
					<block v-if="item.status==1">
						<view class="btn2" @tap.stop="goto" :data-url="'savebasan?orderid=' + item.id + '&price=' + item.totalprice">更换位置</view>
  ...
  methods: {
    ...
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiFishPond/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
