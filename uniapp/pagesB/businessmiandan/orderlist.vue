...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','待使用','已完成']" :itemst="['all','0','1']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
			...
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
					<view class="head">
						...
						<text v-if="item.status==0" class="st0">待使用</text>
						<text v-if="item.status==1" class="st1">已完成</text>
					</view>
					...
					<view class="op">
						<block v-if="item.status==0">
							<view class="btn2" @tap="showhxqr" :data-hexiao_qr="item.hexiao_qr">核销码</view>
						</block>
						<!-- <block v-if="item.status==0">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiBusinessMiandan/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
	...
	showhxqr:function(e){
		var hexiao_qr = e.currentTarget.dataset.hexiao_qr;
		this.hexiao_qr = hexiao_qr;
    ...
    todel: function (e) {
      ...
      app.confirm('确定要删除该订单吗?', function () {
				app.showLoading('删除中');
        app.post('ApiBusinessMiandan/delOrder', {orderid: orderid}, function (data) {
