		...
		<view class="content" id="datalist">
			<view class="item" @tap="goto" :data-url="'detail?tel='+item.tel+'&posterid='+item.id" v-for="(item, index) in datalist" :key="index">
				<view class="f1">
					...
					<text class="t2">性别：{{item.sex}}</text>
					<text class="t2" v-if="item.id_card">身份证号：{{item.id_card}}</text>
					<text class="t2" v-if="item.tel">手机号：{{item.tel}}</text>
					<text class="t2">证书编号：{{item.certificate_no}}</text>
					<text class="t2">工种：{{item.type}}</text>
					<text class="t2">等级：{{item.certificate_level}}</text>
					<text class="t2">发证日期：{{item.start_time}}</text>
				</view>
			</view>
		</view>
	</block>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.id_card = this.opt.id_card || '';
		this.realname = this.opt.realname || '';
  ...
  methods: {
    ...
    getdata: function (loadmore) {
		...
		this.nomore = false;
		this.loading = true;
		app.post('ApiCertificateQuery/record', {id_card:that.id_card,realname:that.realname,tel:that.tel,pagenum: pagenum}, function (res) {
			that.loading = false;
			var data = res.data;
			if (pagenum == 1) {
				that.datalist = data;
...
.content .item .f1 .t2{color:#666666;margin-top:10rpx;font-size: 30rpx;}
