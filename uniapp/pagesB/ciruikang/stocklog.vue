...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			<view class="f1">我的库存</view>
      <view class="f2">{{mynum}}</view>
		</view>
		<view class="content" id="datalist">
			<block v-for="(item, index) in datalist" :key="index"> 
			<view class="item">
				<view class="f1">
          <image :src="item.headimg" style="width: 50rpx;height: 50rpx;border-radius: 50% 50%;margin-right: 10rpx;"></image>
					<text class="t1">{{item.nickname}}</text>
				</view>
				<view class="f2">
					<block >
						<text class="t1">{{item.allnum}}</text>
					</block>
				</view>
			</view>
		</block>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/crk_stcoklog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.mynum = res.mynum;
					that.datalist = data;
          if (data.length == 0) {
            that.nodata = true;
          }
...
.content .item{width:100%;margin:20rpx 0;background:#fff;border-radius:5px;padding:20rpx 20rpx;display:flex;align-items:center;justify-content: space-between;}
.content .item .f1{flex:1;display:flex;align-items:center;}
.content .item .f1 .t1{color:#000000;font-size:30rpx;word-break:break-all;overflow:hidden;text-overflow:ellipsis;max-width:400rpx;}
