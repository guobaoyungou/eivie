...
<view v-if="isload">
  <view>
    <view v-if="set && set.showpool" class="poolmoney" :style="pic?'background:url('+pic+') no-repeat;background-size:100% 100%;':'background-color:'+t('color1')" >
    ...
    <view class="contentbox">
      <view class="content">
        <view class="tab">
          <view class="t1" style="width: 80rpx;">排序</view>
...
export default {
  ...
  methods: {
    getdata: function () {
      ...
      app.post('ApiAgent/shoporderrankingdetail', {id:that.id,st:that.st}, function (res) {
				that.loading = false;
        if(res.status == 1){
          //消费排行榜
