		...
		<view class="content" id="datalist">
			...
			<view class="item" @tap="goto" :data-url="'maidandetail?id=' + item.id">
				<view class="f1">
						...
						<text class="t2" v-if="item.mendian_name && item.mdid > 0">所属门店：{{item.mendian_name}}</text>
				</view>
				<view class="f2">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			this.nomore = false;
			this.loading = true;
      app.post('ApiMaidan/maidanlog', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
