...
<view class="container">
	<block v-if="isload">
		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'xixieorderdetail?id=' + item.id">
				<view class="head">
				...
				<view class="op">
					<view @tap.stop="goto" :data-url="'mendianorderdetail?id=' + item.id" class="btn2">详情</view>
					<block v-if="item.status==1">
						<view class="btn1" :style="{background:t('color1')}" @tap="changeXixieStatus" :data-id="item.id" data-status="2" data-name="取货完成">取货完成</view>
					</block>
                    <block v-if="item.status==2">
                    	<view class="btn1" :style="{background:t('color1')}" @tap="changeXixieStatus" :data-id="item.id" data-status="3" data-name="入库完成">入库完成</view>
                    </block>
                    <block v-if="item.status==3">
                    	<view class="btn1" :style="{background:t('color1')}" @tap="changeXixieStatus" :data-id="item.id" data-status="4" data-name="清洗完成">清洗完成</view>
                    </block>
				</view>
			</view>
			</block>
...
export default {
  ...
  methods: {
    ...
    getdata: function (loadmore) {
        ...
        that.nomore = false;
        that.loading = true;
        app.post('ApiXixie/mendian_orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
			that.loading = false;
            if(res.status == 1){
