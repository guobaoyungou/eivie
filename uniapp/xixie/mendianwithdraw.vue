        ...
        <block v-if="isload">
            <form @submit="formSubmit">
                <view class="mymoney" :style="{background:t('color1')}">
                    <view class="f1">门店{{t('余额')}}</view>
                    <view class="f2"><text style="font-size:26rpx">￥</text>{{mendian.money}}</view>
                    <view class="f3" @tap="goto" data-url="mendianmoneylog?st=1"><text>提现记录</text><text
                ...
                <view class="content2">
                    ...
                    <view class="item4" v-if="mendian && mendian.withdrawfee>0 || mendian.withdrawmin>0">
                        <text v-if="mendian.withdrawmin>0" style="margin-right:10rpx">最低提现金额{{mendian.withdrawmin}}元
                        </text>
                        <text v-if="mendian.withdrawfee>0">提现手续费{{mendian.withdrawfee}}% </text>
                    </view>
                </view>
    ...
    export default {
        ...
        methods: {
            getdata: function() {
                var that = this;
                that.loading = true;
                app.get('ApiXixie/mendian_withdraw', {}, function(res) {
                    that.loading = false;
            ...
            moneyinput: function(e) {
                var mendianmoney = parseFloat(this.mendian.money);
                var money = parseFloat(e.detail.value);
                if (money < 0) {
                    app.error('必须大于0');
                } else if (money > mendianmoney) {
            ...
            formSubmit: function() {
                var that = this;
                var mendianmoney = parseFloat(this.mendian.money);
                var withdrawmin = parseFloat(this.mendian.withdrawmin); //var formdata = e.detail.value;
                ...
                app.showLoading('提交中');
                app.post('ApiXixie/mendian_withdraw', {
                    money: money,
                    paytype: paytype
                }, function(res) {
