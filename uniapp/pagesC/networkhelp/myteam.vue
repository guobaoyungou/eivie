...
export default {
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      app.post('ApiNetworkHelp/myteam', {st: st,pagenum: pagenum,keyword:keyword,active_id:active_id,st:st}, function (res) {
        var data = res.datalist;
        if (pagenum == 1) {
			that.userinfo = res.userinfo;
			that.datalist = data;
			that.tabdata = [that.t('直推')+'('+res.userinfo.myteamCount1+')',that.t('间推')+'('+res.userinfo.myteamCount2+')'];
			that.tabitems = ['1','2'];
