      ...
      <form @submit="formSubmit">
        <view class="form">
          ...
          <view class="form-item">
            ...
            <input class="input" type="text" placeholder="请输入店铺名称" placeholder-style="font-size:28rpx;color:#BBBBBB" name="name" :value="info.name"></input>
          </view>
        </view>
        <button class="savebtn" :style="'background:linear-gradient(90deg,'+t('color1')+' 0%,rgba('+t('color1rgb')+',0.8) 100%)'" form-type="submit">修改</button>
      </form>
		</block>
    ...
    onPullDownRefresh: function () {
		...
		methods:{
			...
			getdata: function (loadmore) {
			  ...
			  var that = this;
				...
				that.nomore = false;
				that.loading = true;
				app.get('ApiPlanorder/editshop', {}, function (res) {
					that.loading = false;
          if(res.status == 1){
            that.info = res.info;
            that.pic  = res.pic;
      ...
      formSubmit: function (e) {
        ...
        var data = {
      	...
      	app.showLoading('提交中');
        app.post('ApiPlanorder/editshop',data, function (res) {
...
<style>
  ...
  .taskprogress{width: 700rpx;margin: 0 auto;display: flex;justify-content: space-between;margin-top: 20rpx;}
