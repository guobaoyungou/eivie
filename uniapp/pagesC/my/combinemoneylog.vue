	...
	<block v-if="isload">
		<view class="content">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
						<view class="t1">
            消耗：{{item.score}}{{t('积分')}}+{{item.commission}}{{t('佣金')}}
            </view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var pagenum = that.pagenum;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMoney/combinemoneylog', {pagenum: pagenum}, function (res) {
				that.loading = false;
        uni.setNavigationBarTitle({
        	title: '合成'+that.t('余额')+'记录'
        });
        if(res.status == 1){
          var data = res.data;
          if (pagenum == 1) {
...
.content .item .f2{ flex:1;font-size:30rpx;text-align:right}
