...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="itemdata" :itemst="['all','0','1','2','3']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap.stop="goto" :data-url="'downshoporderdetail?id=' + item.id">
					<view class="head">
          ...
          <view class="bottom" v-if="item.isdygroupbuy && item.isdygroupbuy==1">
          	<text style="color:red">抖音团购券</text>
					...
					<view class="bottom flex-y-center" v-if="item.mid>0">
						<image :src="item.member.headimg" style="width:40rpx;height:40rpx;border-radius:50%;margin-right:10rpx"/><text style="font-weight:bold;color:#333;margin-right:8rpx">{{item.member.nickname}}</text>(ID:{{item.mid}}) <text v-if="item.dengji">({{item.dengji}})</text>
					</view>
					<view class="op"  >
						<view  class="btn1" v-if="item.status ==2 && item.downorder_collect" :style="{background:t('color1')}" @tap.stop="orderCollect" :data-id="item.id" :data-index="index">确认收货</view>
					</view>
				</view>
			</block>
			</checkbox-group>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAgent/downshoporder', {mid:mid,keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
				var countdata = res.countdata;
				that.itemdata = ['全部('+countdata.count_all+')','待付款('+countdata.count0+')','待发货('+countdata.count1+')','待收货('+countdata.count2+')','已完成('+countdata.count3+')']
