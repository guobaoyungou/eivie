...
<view class="container">
	<block v-if="isload">
		<view v-if="pid<=0" class="myupgradescore" :style="{background:t('color1')}">
			<view class="f1">
				我的{{upgradescorename}}
			</view>
			<view class="item flex-bt flex-y-center">
				<view class="value">{{myupgradescore}}</view>
			</view> 
		</view>
		<view class="content" id="datalist">
            ...
            变动金额：
            <text v-if="item.upgradescore>0" style="color: green;">+{{item.upgradescore}}</text>
            <text v-else style="color: red;">{{item.upgradescore}}</text>
          </view>
				</view>
        <view class="f1">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var pagenum = that.pagenum;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/upgradescorelog', {pagenum: pagenum,pid:that.pid}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
          that.upgradescorename = res.upgradescorename;
          uni.setNavigationBarTitle({
            title: res.upgradescorename+'记录'
          });
					that.myupgradescore = res.myupgradescore;
					that.datalist = data;
					that.set = res.set;
          if (data.length == 0) {
            that.nodata = true;
...
<style>
.myupgradescore{width:94%;margin:20rpx 3%;border-radius: 10rpx 56rpx 10rpx 10rpx;position:relative;display:flex;flex-direction:column;padding:70rpx 0}
.myupgradescore .f1{margin:0 0 0 60rpx;color:rgba(255,255,255,0.8);font-size:24rpx;}
.myupgradescore .f2{margin:20rpx 0 0 60rpx;color:#fff;font-size:64rpx;font-weight:bold; position: relative;}

.myupgradescore .item {min-width: 50%;color: #FFFFFF;padding: 0 10%;margin-top: 30rpx;}
.myupgradescore .item .label{font-size: 26rpx}
.myupgradescore .item .value{font-size: 44rpx;margin-top: 10rpx;font-weight: bold;}

.myupgradescore  .btn{height: 50rpx;text-align: center;border: 1px solid #e6e6e6;border-radius: 10rpx;color: #fff;font-size: 24rpx;font-weight: bold;display: inline-flex;align-items: center;justify-content: center;margin-left: 10rpx;padding: 0 10rpx;}
