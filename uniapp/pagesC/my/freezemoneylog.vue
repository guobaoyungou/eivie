	...
	<block v-if="isload">
		<view v-if="pid<=0" class="mydedamount" :style="{background:t('color1')}">
			...
			<view class="item flex-bt flex-y-center">
				<view class="value">{{myfreezemoney}}</view>
			</view> 
		</view>
		<view class="content" id="datalist">
            ...
            变动金额：
            <text v-if="item.freezemoney>0" style="color: green;">+{{item.freezemoney}}</text>
            <text v-else style="color: red;">{{item.freezemoney}}</text>
          </view>
				</view>
        <view class="f1">
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var pagenum = that.pagenum;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiMy/freezemoneylog', {pagenum: pagenum}, function (res) {
				that.loading = false;
        if(that.pid>0){
        ...
        var data = res.data;
        if (pagenum == 1) {
					that.myfreezemoney = res.myfreezemoney;
					that.datalist = data;
					that.set = res.set;
          if (data.length == 0) {
            that.nodata = true;
