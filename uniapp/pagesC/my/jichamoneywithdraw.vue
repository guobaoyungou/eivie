	...
	<block v-if="isload">
        <form @submit="formSubmit">
            <view class="mymoney" :style="{background:t('color1')}">
                <view class="f1">我的{{t('级差奖励')}}</view>
                <view class="f2"><text style="font-size:26rpx">￥</text>{{userinfo.jichamoney}}</view>
                <view class="f3" @tap="goto" data-url="jichamoneylog?st=1"><text>提现记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
            </view>
            <block v-if="canwithdraw">
              <view class="content2">
                  ...
                  <view class="item4">
                      <view v-if="sysset.jichawithdrawmin>0">最低提现金额{{sysset.jichawithdrawmin}}元 </view>
                      <view v-if="sysset.jichawithdrawfee>0">提现手续费{{sysset.jichawithdrawfee}}% </view>
                      <view v-if="sysset.jichawithdrawmul>0">提现金额需为{{sysset.jichawithdrawmul}}倍数 </view>
                  </view>
              </view>
              <view class="withdrawtype">
                  <view class="f1">选择提现方式：</view>
                  <view class="f2">
                      <view class="item" v-if="sysset.jichawithdraw_aliaccount==1" @tap.stop="changeradio" data-paytype="支付宝">
                      ...
                      <view class="item" v-if="sysset.jichawithdraw_bankcard==1" @tap.stop="changeradio" data-paytype="银行卡">
                      ...
                      <view class="item" v-if="sysset.jichawithdraw_paycode==1" @tap.stop="changeradio" data-paytype="收款码">
        ...
        <view class="withdraw_desc" v-if="sysset.jichawithdraw_desc">
            <view class="title">说明</view>
            <text>{{sysset.jichawithdraw_desc}}</text>
        </view>
	</block>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiMy/jichawithdraw', {}, function (res) {
				that.loading = false;
				uni.setNavigationBarTitle({
					title: that.t('级差奖励') + '提现'
				});
				if(res.status == 0){
					app.alert(res.msg);
					return;
        ...
        that.canwithdraw = res.canwithdraw;
				...
				var paytype = '';
				if ((!sysset.jichawithdraw_weixin || sysset.jichawithdraw_weixin == 0) && sysset.jichawithdraw_aliaccount == 1) {
					paytype = '支付宝';
				}
				if ((!sysset.jichawithdraw_weixin || sysset.jichawithdraw_weixin == 0) && (!sysset.jichawithdraw_aliaccount || sysset.jichawithdraw_aliaccount == 0) && sysset.jichawithdraw_bankcard == 1) {
					paytype = '银行卡';
				}
        if(sysset.jichawithdraw_paycode == 1 && paytype == ''){
        	paytype = '收款码';
        }
    ...
    moneyinput: function (e) {
      var usermoney = parseFloat(this.userinfo.jichamoney);
    ...
    formSubmit: function () {
      var that = this;
      var usermoney = parseFloat(this.userinfo.jichamoney);
      var withdrawmin = parseFloat(this.sysset.jichawithdrawmin); //var formdata = e.detail.value;
      var withdrawmul = parseFloat(this.sysset.jichawithdrawmul); 
      ...
      app.showLoading('提交中');
      app.post('ApiMy/jichawithdraw', {money: money,paytype: paytype}, function (res) {
				app.showLoading(false);
        if (res.status == 1) {
          app.success(res.msg);
          that.subscribeMessage(function () {
            setTimeout(function () {
              app.goto('jichamoneylog?st=1');
            }, 1000);
          });
        } else {
            app.error(res.msg);
