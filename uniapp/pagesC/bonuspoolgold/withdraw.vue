...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiBonusPoolGold/withdraw', {bid:that.bid}, function (res) {
				that.loading = false;
    ...
    formSubmit: function () {
      ...
      if (withdraw_num > usermoney) {
        app.error(that.t('金币') + '不足');
        return;
      }
		app.showLoading('提交中');
      app.post('ApiBonusPoolGold/withdraw', {withdraw_num: withdraw_num,paytype: paytype}, function (data) {
	...
	allwithdraw:function(){
		//全部兑换
		var usermoney = this.userinfo.gold;
