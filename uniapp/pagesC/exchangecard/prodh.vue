    ...
    <block v-if="isload">
			<form @submit="topay">
				...
				<view v-for="(buydata, index) in allbuydata" :key="index" class="buydata">
					...
					<view class="bcontent">
						<view style="color: red;padding: 20rpx 0;font-size: 30rpx;" v-if="hdinfo.num_type ==2">*该兑换码包含{{buydata.prodata.length}}种礼品，您可以选择其中{{hdinfo.select_product_num}}种兑换</view>
							...
							<view v-for="(item, index2) in buydata.prodata" :key="index2" class="item flex-y-center">
								...
								<view class="flex1 flex-y-center" @click="chooseClick(item,index2)">
									<view class="info flex1">
										...
										<view class="f3"><text style="font-weight:bold;">￥{{item.guige.sell_price}}</text><text style="font-weight:bold;" v-if="item.guige.score_price">+{{item.guige.score_price}}{{t('积分')}}</text><text style="padding-left:20rpx"> × {{item.num}}</text></view>
									</view>
									<view v-if="needChoose" class="radio" :style="inArray(index2,chooseIndex)? 'background:'+t('color1')+';border:0' : ''">
						...
						<view class="price">
							<view class="f1">商品金额</view>
							<view class="f2">
								<text>¥{{needChoose ? (chooseIndex.length==0? buydata.product_price : thisprice) : buydata.product_price}}</text>
								<text v-if="buydata.score_price && buydata.score_price*1 > 0">+{{needChoose ? (chooseIndex === '' ? buydata.prodata[0].score : buydata.prodata[chooseIndex ].score) : buydata.score_price}}{{t('积分')}}</text>
							</view>
						</view>
						<view class="price" v-if="hdinfo.product_type && hdinfo.product_type ==11">
							<view class="f1">预约提货时间</view>
							<view class="f2 picker-class">
								<picker mode="date" :start="start_take_date" :value="start_take_date"  @change="takeDateChange">
									<view class="picker">{{take_date}}</view>
								</picker>
							</view>
						</view>
						<view class="price"  v-if="hdinfo.freight_status ==1">
							<view class="f1">
							{{buydata.freightList[buydata.freightkey].freight_price_txt || '运费'}}
							<text v-if="buydata.freightList[buydata.freightkey].pstype!=1 && buydata.freightList[buydata.freightkey].freeset==1" style="color:#aaa;font-size:24rpx;">（满{{buydata.freightList[buydata.freightkey].free_price}}元包邮）</text></view>
        ...
        <view v-if="hdinfo && hdinfo.fee_items && hdinfo.fee_items.length>0" class="address-add">
          <view style="width: 100%;padding: 20rpx 0rpx;display: flex;align-items: center;color: #111111;font-weight: bold;font-size: 30rpx;">费用</view>
        	<view class="price"  v-for="(hitem,hindex) in hdinfo.fee_items" :key="hindex">
        		<text class="f1" >{{hitem.name}}</text>
            <text class="f2" >￥{{hitem.money}}</text>
        	</view>
        </view>
...
export default {
  ...
  methods: {
    getdata: function () {
      var that = this;
      app.get('ApiExchangeCard/prodh', {dhcode: that.opt.dhcode,cardno:that.opt.cardno}, function (res) {
        if (res.status == 0) {
          if (res.msg) {
		...
		if(that.hdinfo && that.hdinfo.start_take_date){
			setTimeout(function(){
				that.take_date = that.hdinfo.start_take_date;
				that.start_take_date = that.hdinfo.start_take_date;
			},1000)
		}
        that.calculatePrice();
        that.loaded();
		...
		if(res.hdinfo.num_type==1 || res.hdinfo.num_type==2){
	...
	chooseClick(item,index){
		...
		var that = this;
		// var newchooseIndex= this.chooseIndex;
		// if(newchooseIndex.length >= this.hdinfo.select_product_num){
		// 	app.error('超出兑换数量');
		// 	return;
		// }
		// newchooseIndex.push(index);
		// this.chooseIndex = newchooseIndex;
		var chooseIndexs= this.chooseIndex;
		// this.chooseInfo.push(item);
		var newIndexs = [];
		var ischecked = false;
		var newchooseInfo = [];//新的选择
		var chooseInfo = that.chooseInfo;//旧的选择
		for(var i in chooseIndexs){
			if(chooseIndexs[i] != index){
				newIndexs.push(chooseIndexs[i]);
				newchooseInfo.push(chooseInfo[i]);
			}else{
				ischecked = true;
			}
		}
		if(ischecked==false){
			if(newIndexs.length >= that.hdinfo.select_product_num){
				app.error('超出兑换数量');return;
			}
			newIndexs.push(index);
			newchooseInfo.push(item);
		}
		console.log(newchooseInfo,'newchooseInfo');
		this.chooseIndex = newIndexs;
		this.chooseInfo = newchooseInfo;
	...
	computeprice(){
		...
		var allbuydata = that.allbuydata;
		var thisprice = 0;
		var chooseIndex = this.chooseIndex;
			console.log(allbuydata,'allbuydata')
		for (var i in allbuydata) {
			var prodata= allbuydata[i].prodata;
				console.log(prodata,'prodata')
			for(var k in chooseIndex){
				console.log(chooseIndex[k],'kkkkkk')
				var pprice = prodata[chooseIndex[k]].price;
				thisprice =parseFloat(parseFloat(thisprice) + parseFloat(pprice)).toFixed(2);
			}
		}
		that.thisprice = thisprice;
	},
    //积分抵扣
    scoredk: function (e) {
    ...
    calculatePrice: function () {
      ...
      alltotalprice = 0;//定死为0，以上功能不使用
	  //如果开启运费，不为0
	  if(that.hdinfo.freight_status && that.hdinfo.freight_status ==1){
	  		alltotalprice += allfreight_price; 
	  }
	    console.log(alltotalprice,'alltotalprice11');
      if(that.hdinfo && that.hdinfo.fee_items){
        var fee_items = that.hdinfo.fee_items;
        var feelen   = fee_items.length;
        if(feelen){
          for(var i=0;i<feelen;i++){
            var money = parseFloat(fee_items[i]['money']);
            alltotalprice += money;
          }
        }
      }
    ...
    topay: function (e) {
      ...
      for (var i in allbuydata) {
		...
		if(that.needChoose){
			var  chooseinfo = that.chooseInfo;
			console.log(chooseinfo,'chooseinfo');
			let shopInfo =[];
			for(var c in chooseinfo){
				var this_chooseinfo = chooseinfo[c];
				shopInfo.push(this_chooseinfo.product.id + ',' + this_chooseinfo.guige.id + ',' + this_chooseinfo.num) 
			}
			proInfo = shopInfo.join('-',shopInfo);
			console.log(proInfo,'proInfoproInfoproInfo');
		}
		var exchange_card_take_date = that.take_date;
        buydata.push({
      ...
      app.showLoading('提交中');
      app.post('ApiExchangeCard/createOrder', {dhcode:that.opt.dhcode,cardno:that.opt.cardno,frompage: frompage,buydata: buydata,addressid: addressid,linkman: linkman,tel: tel,usescore: usescore,exchange_card_take_date:exchange_card_take_date}, function (res) {
    ...
    openLocation:function(e){
      ...
      uni.openLocation({
		...
		takeDateChange:function(e){
			this.take_date = e.target.value
		}
  }
}
</script>
...
.picker-class{width: 43%;height: 100%;align-items: center;}
.picker{color: #111;white-space: nowrap;text-align: right;width: 100%;} 
</style>