		...
		<block v-for="(item, index) in datalist" :key="index"> 
			<view class="item">
				<view class="f1">
						...
						<text class="t1" >兑换码：{{item.code}}</text>
						<text class="t1" v-if="item.total_take_times > 1 ">总提货次数：{{item.total_take_times}}次</text>
						<text class="t1" v-if="item.total_take_times > 1 ">已提货次数：{{item.take_times}}次</text>
...
export default {
  ...
  methods: {
    ...
    getdata: function () {
      ...
      var st = that.st;
			that.loading = true;
      app.post('ApiExchangeCard/dhlog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
