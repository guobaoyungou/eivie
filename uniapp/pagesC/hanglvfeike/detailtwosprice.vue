<template>
    ...
    <view style="background-color: #fff;padding: 20rpx;">
      <view style="display: flex;justify-content: space-between;align-items: center;">
          ...
          <view v-if="airlinePic" style="width: 50rpx;height: 50rpx;">
            <image :src="airlinePic" style="width: 100%;height: 100%;"></image>
          </view>
          <view>
            <view style="font-size: 26rpx;color: #999;">
              {{fromDate}} {{week}}
              {{deparAirportName}}{{departTerminal}} - {{arriveAirportName}}{{arriveTerminal}}
            </view>
            <view  style="display: flex;align-items: center;margin: 25rpx 0;">
              <view style="font-size:36rpx;font-weight:bold;">{{departTime}}</view>
              <image :src="pre_url+'/static/img/planeticket/jiantou.png'" mode="widthFix" style="margin: 0 20rpx;width: 100rpx;"></image>
              <view style="font-size:36rpx;font-weight:bold;">{{arriveTime}}</view>
            </view>
            <view style="font-size: 26rpx;color: #999;">
              {{airlineName}}{{no}}| {{planeCName}} ({{planeTypeName}}) | {{flightTime}}
            </view>
          </view>
      </view>
              ...
              <view style="padding: 10rpx 20rpx;">
                <view style="display: flex;justify-content: space-between;align-items: center;" >
                  <view :style="'color:'+t('color1')">
                    <text>￥</text>
                    <text style="font-size: 40rpx;">{{item.fdPrice}}</text>
                  </view>
                ...
                <view @tap="getguize" style="color: #999;line-height: 50rpx;display: flex;align-items: center;border-radius:0 0 8rpx 8rpx;">{{item.discount}}折退改详情 <image :src="pre_url+'/static/img/arrowright.png'" style="width: 28rpx;height: 28rpx;"></image></view>
              </view>
            </view>
          </block>
        </view>
    ...
    <uni-popup id="popup" ref="popup" type="bottom" >
        ...
        <view style="width:100%;height:100rpx"></view>
    		<scroll-view :scrollIntoView="intoviewid" :scrollWithAnimation="true" scroll-y style="height: auto;max-height: 90vh;">
          <view style="padding:0 20rpx;">
            ...
            <view style="border: 2rpx solid #f1f1f1;border-radius: 8rpx 8rpx;">
                ...
                <view style="display: flex;align-items: center;border-top: 2rpx solid #f1f1f1;">
                  <view class="popup-item-title" >
                    托运行李
                  </view>
                  <view class="popup-item-content">
                    {{guize?guize.checkedluggage:''}}
                  </view>
                </view>
            ...
            <view style="border: 2rpx solid #f1f1f1;border-radius: 8rpx 8rpx;">
                ...
                <view style="display: flex;align-items: center;border-top: 2rpx solid #f1f1f1;">
                  ...
                  <view class="popup-item-content">
                    <block v-if="guize && guize.refundStipulate && guize.refundStipulate.length>0">
                ...
                <view style="display: flex;align-items: center;border-top: 2rpx solid #f1f1f1;">
                  ...
                  <view class="popup-item-content">
                    <block v-if="guize && guize.changeStipulate && guize.changeStipulate.length>0">
...
<script>
    ...
    onLoad: function (opt) {
      ...
      that.flightNo= that.opt.flightNo || '';
      that.cabinNo= that.opt.cabinNo || '';
      
      that.deparAirportName = that.opt.deparAirportName || '';
      that.departTerminal = that.opt.departTerminal || '';
      that.arriveAirportName = that.opt.arriveAirportName || '';
      that.arriveTerminal = that.opt.arriveTerminal || '';
      that.airlineName = that.opt.airlineName;
      that.no = that.opt.no;
      that.planeCName = that.opt.planeCName;
      that.planeTypeName = that.opt.planeTypeName;
      that.flightTime = that.opt.flightTime;
      that.departTime = that.opt.departTime;
      that.arriveTime = that.opt.arriveTime;
      
      that.fromCityname= that.opt.fromCityname || '';
      that.toCityname = that.opt.toCityname || '';
      that.fromDate = that.opt.fromDate || '';
      that.taxFee  = that.opt.taxFee || '';
      that.fuelFee  = that.opt.fuelFee || '';
    ...
    onShow:function(){
      ...
      gobuy:function(e){
        ...
        var url = 'buy?flightNo='+that.flightNo+'&cabinNo='+that.cabinNo+'&deparAirportName='+that.deparAirportName+'&departTerminal='+that.departTerminal+'&arriveAirportName='+that.arriveAirportName+'&arriveTerminal='+that.arriveTerminal;
            url += '&airlineName='+that.airlineName+'&no='+that.no+'&planeCName='+that.planeCName+'&planeTypeName='+that.planeTypeName+'&flightTime='+that.flightTime+'&departTime='+that.departTime+'&arriveTime='+that.arriveTime+'&taxFee='+that.taxFee+'&fuelFee='+that.fuelFee;
            url += '&searchNo='+that.searchNo+'&fromCityname='+that.fromCityname+'&toCityname='+that.toCityname+'&fromDate='+that.fromDate+'&week='+that.week;  
            
            url += '&levelname='+datalist[index].levelname + '&fdPrice='+datalist[index].fdPrice;
        app.goto(url);
      }
		}
	}
