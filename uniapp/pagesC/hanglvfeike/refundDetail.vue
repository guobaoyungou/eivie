...
<view class="container">
    ...
    <view v-if="detail.refundusers && detail.refundusers.length>0" class="product">
    	<view v-for="(item, idx) in detail.refundusers" :key="idx" class="content">
    		<view class="detail">
...
export default {
  ...
  methods: {
		...
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiHanglvfeike/refundDetail', {id: that.opt.id}, function (res) {
				that.loading = false;
    ...
    toclose: function (e) {
      ...
      app.confirm('确定要关闭该退款单吗?', function () {
				app.showLoading('提交中');
        app.post('ApiHanglvfeike/refundOrderClose', {id: id}, function (data) {
