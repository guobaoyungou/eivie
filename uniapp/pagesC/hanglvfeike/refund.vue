...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.post('ApiHanglvfeike/refundinit', {orderid:that.orderid}, function (res) {
				that.loading = false;
        if(res.status == 1){
    ...
    formSubmit: function (e) {
      ...
      if (reason == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiHanglvfeike/refund', {orderid: orderid,reason: reason,pics:that.pics,ogids:that.selogids}, function (res) {
    ...
    chooseit:function(e){
      ...
      console.log(ordergoods[index]['issel'])
      that.selogids = [];
      var len = ordergoods.length;
       console.log(ordergoods)
      for(var i=0;i<len;i++){
        if(ordergoods[i]['issel']){
          console.log(ordergoods[i]['id'])
