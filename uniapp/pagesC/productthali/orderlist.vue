		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
				...
				<view class="content" style="border-bottom:none">
					<view @tap.stop="goto" :data-url="'detail?id=' + item.proid">
						<image :src="item.propic"></image>
					</view>
					<view class="detail">
						<text class="t1">{{item.proname}}</text>
						<text class="t2">{{item.ggname}}</text>
						<view class="t3"><text class="x1 flex1">￥{{item.totalprice}}</text></view>
					</view>
				</view>
				<view class="bottom">
				...
				<view class="op">
					...
					<block v-if="item.status==2">
						<view v-if="(item.refund_status==0 || item.refund_status==3)" class="btn2" @tap.stop="goto" :data-url="'refund?orderid=' + item.id + '&price=' + item.totalprice">申请退款</view>
						<view class="btn2" @tap.stop="goto" :data-url="'/pagesExt/order/logistics?express_com=' + item.express_com + '&express_no=' + item.express_no" v-if="item.freight_type!=3 && item.freight_type!=4 && item.product_thali_deliver_type==1">查看物流</view>
...
export default {
  ...
  methods: {
    ...
    getdata: function (loadmore) {
		...
		that.nomore = false;
		that.loading = true;
		app.post('ApiProductThali/orderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
			that.loading = false;
    ...
    toclose: function (e) {
      ...
      app.confirm('确定要关闭该订单吗?', function () {
				app.showLoading('提交中');
        app.post('ApiProductThali/closeOrder', {orderid: orderid}, function (data) {
    ...
    todel: function (e) {
      ...
      app.confirm('确定要删除该订单吗?', function () {
				app.showLoading('删除中');
        app.post('ApiProductThali/delOrder', {orderid: orderid}, function (data) {
					app.showLoading(false);
          app.success(data.msg);
          setTimeout(function () {
            that.getdata();
    ...
    orderCollect: function (e) {
      ...
      app.confirm('确定要收货吗?', function () {
		app.showLoading('提交中');
        app.post('ApiProductThali/orderCollect', {orderid: orderid}, function (data) {
			app.showLoading(false);
			app.success(data.msg);
			setTimeout(function () {
				that.getdata();
