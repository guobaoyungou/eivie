	...
	<block v-if="isload">
		<view class="container">
			<view class="datalist">
				...
				<view :data-url="'detail?id=' + item.id" @tap="goto" class="collage-product">
					...
					<view class="product-info">
						...
						<view class="p2">
							<view class="p2-1">
								<text class="t1" :style="{color:t('color1')}"><text style="font-size:24rpx"></text></text>
								<text class="t2" v-if="item.market_price*1 > item.sell_price*1"></text>
							</view>
						</view>
						<view class="p3">
							<view class="t1">商品数量<text style="font-size:28rpx;color:#f40;padding:0 2rpx;">{{item.perlimit}}</text></view>
							<view class="t2" :style="{borderColor:t('color1'),color:t('color1')}">
								<text class="x2" :style="{backgroundColor:t('color1')}">查看详情</text>
							</view>
						</view>
					</view>
				</view>
...
export default {
  ...
  methods: {
		getdata: function (loadmore) {
			...
			that.nodata = false;
			that.nomore = false;
			app.post('ApiProductThali/index', {bid:that.bid,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
