	...
	<block v-if="isload">
		...
		<view class="ind_business">
			<view class="ind_buslist" id="datalist">
				...
				<view @tap="goto" :data-url="'/pages/index/index?select_bid=' + item.id">
					<view class="ind_busbox flex1 flex-row">
						<view>
							<view class="bus_title">{{item.name}}</view>
							<view class="bus_address"><text class="x1">{{item.address}}</text><text class="x2">{{item.juli}}</text><text class="x2" @tap.stop="phone" :data-phone="item.tel">{{item.tel}}</text></view>
						</view>
					</view>
				</view>
				</block>
...
.ind_business {width: 100%;margin-top: 120rpx;font-size:26rpx;padding:0 24rpx}
