...
<view class="container">
	<block v-if="isload">
		...
		<view class="content">
			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				...
				<view class="f2">
					<view class="t1">
						...
						<text class="x2">{{date(item.createtime)}}</text>
			...
			<block v-if="st==2">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1"><text>{{item.remark}} {{date(item.createtime)}}</text></view>
				<view class="f2">
					<view class="t1">
						<view v-for="(item2, index) in item.oglist" :key="index" class="item2">
							...
							<text class="x2">{{date(item2.createtime)}}</text>
...
export default {
  ...
  methods: {
      ...
      var st = that.st;
			...
			that.nodata = false;
      that.nomore = false;
			app.get('ApiAgent/areafenhong',{st:st,pagenum: pagenum},function(res){
				that.loading = false;
