	...
	<block v-if="isload">
		...
		<view class="form-content">
			<view class="form-item1">
				<view class="label">{{text['酒店']}}信息</view>
				<view class="product flex">
					<view class="img"><image :src="order.pic"></image></view>
					<view class="info flex1">
						<view class="f1">{{order.hotelname}}</view>
						<view class="f2">{{order.title}}</view>
						<view class="f3">￥{{order.sell_price}}</view>
					</view>
				</view>
			</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			var oid = that.opt.oid;
			that.loading = true;
			app.get('ApiHotel/comment', {oid: oid}, function (res) {
				that.loading = false;
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiHotel/comment', {oid: oid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
