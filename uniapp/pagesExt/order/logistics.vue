...
<!--	<block v-if="express_com=='同城配送'">-->
	<block v-if="express_com.indexOf('同城配送') !== -1">
...
export default {
  ...
  onLoad: function (opt) {
    ...
    this.ordertype = this.opt.ordertype || '';
		this.getdata();
  },
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiOrder/logistics', {express_com: that.express_com,express_no: that.express_no,type:that.type,orderid:that.orderid,ordertype:that.ordertype}, function (res) {
				that.loading = false;
				//if(that.express_com == '同城配送'){
				if(that.express_com.indexOf('同城配送') !== -1){
