		...
		getdata: function() {
			...
			app.post('ApiUnifiedOrder/detail', {
				...
				if (res.status == 1) {
					...
					console.log('[ai_pick_detail] auto_pay:', that.opt.auto_pay, 'status:', res.data.status, 'payorderid:', res.data.payorderid);
					if (that.opt.auto_pay == '1' && res.data.status == 0 && res.data.payorderid) {
						console.log('[ai_pick_detail] 满足条件，自动跳转到支付页，payorderid:', res.data.payorderid);
						setTimeout(function() {
