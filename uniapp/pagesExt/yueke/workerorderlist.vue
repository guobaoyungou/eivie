...
<view class="container">
	<block v-if="isload">
		<dd-tab :itemdata="['全部','已付款','已完成']" :itemst="['all','1','3']" :st="st" :isfixed="true" @changetab="changetab"></dd-tab>
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap.stop="goto" :data-url="'workerorderdetail?id=' + item.id">
					<view class="head">
					...
					<view class="content" style="border-bottom:none">
						...
						<view class="detail">
							...
							<text class="t2" v-if="item.yy_time.length > 4">开课时间：{{item.yy_time}}</text>
							<text class="t2" v-if="item.kechengnum">课程数量：{{item.kechengnum}}</text>
							<text class="t2" v-if="item.kechengnum">剩余课程：{{item.surpluskechengnum}}</text>
					...
					<view class="op">
						<view v-if="item.yuyue_model == 2" @tap.stop="goto" :data-url="'/pagesB/yueke/workerstudyrecord?orderid=' + item.id" class="btn2">学习记录</view>
					</view>
				</view>
			</block>
		</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiYueke/workerorderlist', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
				that.loading = false;
