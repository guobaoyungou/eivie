			...
			<view class="form-item1">
				...
				<view class="product flex">
					...
					<view class="info flex1">
						...
						<view class="f2">{{og.ggname}} | <text style="margin-left: 12rpx;" v-if="og.every_day"> {{og.every_day}}</text><text v-else> {{og.pspl}}</text></view>
...
export default {
  ...
  methods: {
		getdata: function () {
			...
			that.loading = true;
			app.get('ApiCycle/comment', {orderid: orderid}, function (res) {
				that.loading = false;
				that.og = res.og;
				if (res.comment){
    ...
    formSubmit: function (e) {
      ...
      if (content == '') {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiCycle/comment', {orderid: orderid,content: content,content_pic: content_pic.join(','),score: score}, function (data) {
