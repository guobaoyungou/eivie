        ...
        <block v-if="isload">
            <form @submit="formSubmit">
                <view class="mymoney" :style="{background:t('color1')}">
                    ...
                    <view class="f3" @tap="goto" :data-url="'moneylog?st=1&type='+type"><text>明细</text>
                    <text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
                </view>
                <view class="content2" v-if="canWithdraw">
                    <view class="item2">
                    ...
                    <view class="item3">
                        ...
                        <view class="f2"><input class="input" type="digit" name="money" value="" placeholder="请输入提现金额"
                                placeholder-style="color:#999;font-size:40rpx" @input="moneyinput" /></view>
                    </view>
                    <view class="item4" v-if="sysset.withdrawfee>0 || sysset.withdrawmin>0">
                        <text v-if="sysset.withdrawmin>0" style="margin-right:10rpx">最低提现金额{{sysset.withdrawmin}}元
                        </text>
                        <text v-if="sysset.withdrawfee>0">提现手续费{{sysset.withdrawfee}}% </text>
                    </view>
                </view>
                <view class="withdrawtype" v-if="canWithdraw">
                    <view class="f1">选择提现方式：</view>
                    <view class="f2">
                        <view class="item" v-if="sysset.withdraw_weixin==1" @tap.stop="changeradio" data-paytype="微信钱包">
                            <view class="t1">
                                <image class="img" :src="pre_url+'/static/img/withdraw-weixin.png'" />微信钱包
                            </view>
                        ...
                        <label class="item" v-if="sysset.withdraw_aliaccount==1" @tap.stop="changeradio"
                            data-paytype="支付宝">
                            <view class="t1">
                                <image class="img" :src="pre_url+'/static/img/withdraw-alipay.png'" />支付宝
                            </view>
                        ...
                        <label class="item" v-if="sysset.withdraw_bankcard==1" @tap.stop="changeradio"
                            data-paytype="银行卡">
                            <view class="t1">
                                <image class="img" :src="pre_url+'/static/img/withdraw-cash.png'" />银行卡
                            </view>
                ...
                <button v-if="canWithdraw" class="btn" @tap="formSubmit" :style="{background:t('color1')}">立即提现</button>
                ...
                <view v-if="paytype=='支付宝'"
                    ...
                    @tap="goto" data-url="/pagesExt/my/setaliaccount">设置支付宝账户
                    <image :src="pre_url+'/static/img/arrowright.png'" style="width:30rpx;height:30rpx" />
                </view>
                <view v-if="paytype=='银行卡'"
                    style="width:100%;margin-top:40rpx;text-align:center;color:#999;display:flex;align-items:center;justify-content:center"
                    @tap="goto" data-url="/pagesExt/my/setbankinfo">设置银行卡账户
                    <image :src="pre_url+'/static/img/arrowright.png'" style="width:30rpx;height:30rpx" />
                </view>
            </form>
        </block>
    ...
    export default {
        ...
        methods: {
            getdata: function() {
                var that = this;
                that.loading = true;
                app.get('ApiMy/otherwithdraw', {type:that.type}, function(res) {
                    that.loading = false;
                    if(res.status == 1){
                        that.type_name= res.type_name;
                        uni.setNavigationBarTitle({
                            title: that.type_name + '提现'
                        });
                        if(res.canWithdraw){
                            that.canWithdraw = true;
                        }
                        ...
                        if ((!sysset.withdraw_weixin || sysset.withdraw_weixin == 0) && (!sysset
                                .withdraw_aliaccount || sysset.withdraw_aliaccount == 0) && sysset
                            .withdraw_bankcard == 1) {
                            paytype = '银行卡';
                        }
            ...
            moneyinput: function(e) {
                ...
                if (money < 0) {
                    ...
                    app.error('可提现' + this.type_name + '不足');
                }
                this.money = money;
            },
                ...
                app.post('ApiMy/otherwithdraw', {
                    ...
                    if (res.status == 0) {
                        ...
                        that.subscribeMessage(function() {
                            setTimeout(function() {
                                app.goto('moneylog?st=1&type='+that.type);
                            }, 1000);
                        });
                    }
                });
