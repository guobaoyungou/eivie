		...
		<view class="content-container">
			...
			<view class="nav_right">
				<view class="nav_right-content">
                  ...
                  <view :style="{color:t('color1')}" v-if="item.showgivescore">
                    <text style="font-size: 24rpx;">赠送{{item.showgivescore}}{{t('积分')}}</text>
									...
									<view v-if="item.price_show_type =='0' || !item.price_show_type ">
										...
										<view class="p2 flex" v-if="(item.price_type != 1 || item.sell_price > 0) && (item.showprice_dollar || item.show_sellprice)">
                          ...
                          <block v-if="item.product_unit">/{{item.product_unit}}</block>
                          <text v-if="item.price_show && item.price_show_text" style="margin: 0 15rpx;font-size: 24rpx;font-weight: 400;">{{item.price_show_text}}</text>
                          <text v-if="!isNull(item.service_fee) && item.service_fee > 0">+{{item.service_fee}}{{t('服务费')}}</text>
												</view>
											</block>
											<text class="t2" v-if="item.market_price*1 > item.sell_price_origin*1">￥{{item.market_price}}</text>
										</view>
									</view>
									<!-- 价格展示普通价和会员等级价 -->
										...
										<view v-if="item.is_vip == '1'">
											<view class="flex" v-if=" item.lvprice ==1 ">
												<view class="member flex" :style="'border-color:' + t('color1')">
													<view :style="{background:t('color1')}" class="member_lable flex-y-center">{{item.level_name_show}}</view>
													<view :style="'color:' + t('color1')" class="member_value flex-y-center" >
														￥<text style="font-size: 32rpx;">{{item.sell_price || 0}}</text>
                            <text v-if="item.price_show && item.price_show_text" style="margin: 0 15rpx;font-size: 24rpx;font-weight: 400;">{{item.price_show_text}}</text>
													</view>
												</view>
											</view>
                  ...
                  <view class="p3">
                    <view class="p3-1" v-if="item.product_show_costprice_fanwei"><text style="overflow:hidden">成本价￥{{item.cost_price_fanwei}}</text></view>
                  </view>
									<view class="p3">
...
.classify-li{flex-shrink:0;display:flex;background:#F5F6F8;border-radius:22rpx;color:#6C737F;font-size:24rpx;text-align: center;height:44rpx; line-height:44rpx;padding:0 28rpx;margin:12rpx 10rpx 12rpx 0}
