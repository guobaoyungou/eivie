					...
					<view class="filter-scroll-view-box">
						<view class="search-filter">
							...
							<view class="search-filter-btn">
								<view class="btn btn-class" @tap.stop="filterReset">重置</view>
								<view class="btn2 btn-class" :style="{background:t('color1')}" @tap.stop="filterConfirm">确定</view>
							</view>
						</view>
					</view>
					<view class="filter-cut"></view>
				</scroll-view>
			</uni-drawer>
...
export default {
  ...
  onLoad: function (opt) {
		...
		this.brand_id = this.opt.brand_id ? this.opt.brand_id : 0;
  ...
  methods: {
    ...
    getprolist: function () {
      ...
      app.post('ApiShop/getprolist',{pagenum: pagenum,keyword: keyword,field: field,order: order,gid: gid,cid: cid,cid2:cid2,cpid:cpid,bid:bid,latitude: that.latitude,longitude: that.longitude,proparams:that.proparams,mendian_id:that.mendianid,area:that.area,brand_id:brand_id}, function (res) {
				that.loading = false;
        var data = res.data;
...
.filter-cut{width: 100%;height: 200rpx;}
