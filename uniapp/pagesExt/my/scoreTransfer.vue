...
<!-- 				<view class="f2" v-if="mid>0">
      ...
      <view class="item5">
      	<view :class="mid>0?'redtxt':''">您的当前{{t('积分')}}：{{myscore}}，转赠后不可退回</view>
      	<view v-if="set.score_transfer_sxf_ratio > 0" class="redtxt">
          <block v-if="set.score_transfer_sxf_type && set.score_transfer_sxf_type == 1">扣除对方</block>
          <block v-else-if="!set.score_transfer_sxf_type || set.score_transfer_sxf_type == 0">扣除我方</block>
          转赠手续费{{set.score_transfer_sxf_ratio}}%
        </view>
      </view>
    ...
    <view class="withdrawtype" v-if="set.score_transfer_sxf_type && set.score_transfer_sxf_type == 2">
      ...
      if (isNaN(money) || money <= 0) {
        ...
        app.error(this.t('积分') + '不足');return;
			...
			app.confirm('确定要赠送吗？', function(){
				app.showLoading();
				app.post('ApiMy/scoreTransfer', {integral: money,mobile: mobile,mid:mid,paypwd:paypwd,feetype:that.feetype}, function (data) {
					app.showLoading(false);
				  if (data.status == 0) {
				    app.error(data.msg);
