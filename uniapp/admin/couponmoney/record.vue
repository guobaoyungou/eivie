	...
	<block v-if="isload">
		<view class="content">
			<view v-for="(item, index) in datalist" :key="index" class="item">
				<view class="f1">
						<text class="t1">名称：{{item.couponname}}</text>
						<text class="t2">时间：{{dateFormat(item.createtime)}}</text>
						<view class="t3 flex-y-center">使用人：<image :src="item.headimg" style="width:40rpx;height:40rpx"/>{{item.nickname}}</view>
				</view>
				<view class="f2">
						<text class="t1">+{{item.decmoney}}</text>
				</view>
			</view>
		</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminCouponmoney/record', {pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
