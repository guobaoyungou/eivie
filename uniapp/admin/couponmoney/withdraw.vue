...
<view class="container">
	...
	<form @submit="formSubmit">
		<view class="mymoney" :style="{background:t('color1')}">
			...
			<view class="f2"><text style="font-size:26rpx">￥</text>{{userinfo.couponmoney}}</view>
			<view class="f3" @tap="goto" data-url="withdrawlog"><text>提现记录</text><text class="iconfont iconjiantou" style="font-size:20rpx"></text></view>
		</view>
		<view class="content2">
			...
			<view class="item4" v-if="sysset.couponwithdrawfee>0 || sysset.withdrawmin>0">
				<text v-if="sysset.withdrawmin>0" style="margin-right:10rpx">最低提现金额{{sysset.withdrawmin}}元 </text>
				<text v-if="sysset.couponwithdrawfee>0">提现手续费{{sysset.couponwithdrawfee}}% </text>
			</view>
		</view>
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminCouponmoney/withdraw', {}, function (res) {
				that.loading = false;
    ...
    formSubmit: function (e) {
      ...
      if (paytype == '银行卡' && (!this.userinfo.bankname || !this.userinfo.bankcarduser || !this.userinfo.bankcardnum)) {
        ...
        return;
			...
			app.showLoading('提交中');
      app.post('ApiAdminCouponmoney/withdraw', {money: money,paytype: paytype}, function (res) {
