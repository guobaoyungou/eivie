		...
		<form @submit="subform">
			...
			<view class="form-box" v-for="(item,index) in gglist" :key="index">
				...
				<view class="form-item" style="height:80rpx;line-height:80rpx">
					...
					<view class="f2"><input type="text" @input="gglistInput" :data-index="index" data-field="cost_price" :name="'cost_price['+index+']'" :value="item.cost_price" placeholder="请填写成本价" placeholder-style="color:#888" :disabled="cost_bili_with_edit == 0"></input></view>
				</view>
				<view class="form-item" style="height:80rpx;line-height:80rpx">
  ...
  data() {
    return {
      ...
      info:{},
			...
			showtjArr:['-1'],
			cidsArr:"",
			cost_bili_with_edit:1, //是否允许商户修改商品成本 0关闭 1开启
            submitable: true
    };
  },
  ...
  methods: {
		getdata:function(){
			...
			app.get('ApiAdminProduct/edit',{id:id}, function (res) {
				...
				if(res.cost_bili_with_edit != undefined){
					that.cost_bili_with_edit = res.cost_bili_with_edit;
				}
