...
<view class="container">
	<block v-if="isload">
		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'cycleorderdetail?id=' + item.id">
				<view class="head">
  ...
  onReachBottom: function () {
	...
	methods: {
		toCycleList:function(e){
			...
			var url='cycleplanlist?id='+id
			app.goto(url);
		},
		toDetail:function(e){
			var id = e.currentTarget.dataset.id
			app.goto('cycleorderdetail?id='+id);
		},
		...
		getdata: function (loadmore) {
		  ...
		  var st = that.st;
				...
				that.nomore = false;
				that.loading = true;
		  app.post('ApiAdminOrder/cycleorder', {st: st,pagenum: pagenum,keyword:that.keyword}, function (res) {
					that.loading = false;
