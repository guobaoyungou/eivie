...
<view class="container">
	<block v-if="isload">
		...
		<view class="product">
			<view class="content">
				<view @tap="goto" :data-url="'/activity/collage/product?id=' + detail.proid">
		...
		<view class="bottom notabbarbot">
			...
			<view v-if="team.status==2 && detail.show_operate" class="btn2" @tap="refund" :data-id="detail.id">退款</view>
		</view>
      ...
      if (totalsec <= 0) {
		...
		gotoRefundMoney:function(){
			...
			app.confirm('确定要退款吗?', function () {
				...
				app.showLoading('提交中');
				app.post('ApiAdminOrder/collageorderrefund',{
