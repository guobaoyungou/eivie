	...
	<block v-if="isload">
		...
		<view class="order-content">
			<block v-for="(item, index) in datalist" :key="index">
				<view class="order-box" @tap="goto" :data-url="'luckycollageorderdetail?id=' + item.id">
					<view class="head">
						...
						<block v-if="item.status==1">
							<block v-if="item.buytype!=1">
								<text v-if="item.status==1" class="st1">拼团中</text>
								<text v-if="item.status==2 && item.freight_type!=1" class="st1">拼团成功,待发货</text>
								<text v-if="item.status==2 && item.freight_type==1" class="st1">拼团成功,待提货</text>
								<text v-if="item.status==3" class="st4">拼团失败,已退款</text>
							</block>
							<block v-else>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminOrder/luckycollageorder', {keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
