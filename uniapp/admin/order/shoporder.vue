							...
							<view class="detail">
								...
								<block>
									...
									<view class="t3" v-else>
										<text class="x1 flex1">￥{{item2.sell_price}}
...
<!--								<block>
          ...
          <view class="bottom"v-if="item.wx_express_intracity && item.wxtc_status_name" style="color: red;font-size: 13px">
            <text>配送状态：{{item.wxtc_status_name}}</text>
          </view>
          <view class="bottom" v-if="item.wx_express_intracity && item.freight_time" style="color: red;font-size: 13px">
            <text>配送时间：{{item.freight_time}}</text>
          </view>
					<view class="op"  >
            <view v-if="wifiprintAuth" style="display: flex;justify-content: flex-end;align-items: center;">
              <view @click.stop="">
            ...
            <view :data-id="item.id" v-if="item.shoporder_update && item.status == 0" @tap.stop="goto" :data-url="'/adminExt/order/updateOrder?id=' + item.id" data-type="1" class="btn2">订单修改</view>
            <view :data-id="item.id" v-if="item.shoporder_shdimg_mobile" @tap.stop="goto" :data-url="'/adminExt/order/shdImg?id=' + item.id" data-type="1" class="btn2">送货单</view>
            <view v-if="shoporder_copy" :data-id="item.id" @tap.stop="copyOrder" class="btn2">复制</view>
            <view v-if="shoporder_update_member && item.status == 0" :data-id="item.id" @tap.stop="goto" :data-url="'../member/index?type=updateOrder&update_oid=' + item.id" class="btn2">更换ID</view>
					</view>
				</view>
			</block>
			</checkbox-group>
  ...
  data() {
    return {
      ...
      codtxt: "",
			...
			shdimg:'',//送货单图片
      updateoid:'',
      shoporder_copy:false,//订单复制按钮
      shoporder_update_member:false,//修改下单人
    };
  },
  onLoad: function (opt) {
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      app.post('ApiAdminOrder/shoporder', {mid:mid,keyword:that.keyword,st: st,pagenum: pagenum}, function (res) {
				...
				that.shoporder_copy = res.shoporder_copy;
				that.shoporder_update_member = res.shoporder_update_member;
    ...
    copyOrder:function(e){
      var that = this;
      var id = e.currentTarget.dataset.id;
      app.confirm('确认复制吗?',function(){
        that.loading = true;
        app.post('ApiAdminOrder/copyOrder', {id:id}, function (res) {
          that.loading = false;
          if(res.status==1){
            app.success('复制成功');
            that.getdata();
...
.order-box .op{ display:flex;justify-content:flex-end;align-items:center;width:100%; padding: 10rpx 0px; border-top: 1px #f4f4f4 solid; color: #555;flex-wrap: wrap;}
