...
<view class="container">
	<block v-if="isload">
		<view class="myscore" :style="{background:t('color1')}">
			...
			<view class="f-row">
				<view class="f2"><text>{{myscore}}</text></view>
				<view class="score-option">
					<view v-if="businessTransfer" class="btn-mini"  @tap="goto" data-url="/pagesA/score/transfer">转入/转出</view>
					<view v-if="scoreWithdraw" class="btn-mini"  @tap="goto" data-url="/pagesB/business/scorewithdraw">提现</view>
				</view>
			</view>
		</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminFinance/scorelog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
...
.btn-mini {width: 140rpx;height: 50rpx;text-align: center;border: 1px solid #e6e6e6;border-radius: 10rpx;color: #fff;font-size: 24rpx;font-weight: bold;display: inline-flex;align-items: center;justify-content: center;margin-bottom: 6rpx;}
.f-row{display: flex;justify-content: space-between;align-items: center;}
.score-option{width: 160rpx;display: flex; flex-direction: column;}
</style>