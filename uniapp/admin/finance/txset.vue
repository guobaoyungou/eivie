...
<view class="container">
	<block v-if="isload">
		...
		<view class="form">
			<view class="form-item" v-if="info.show_aliaccountname">
				<text class="label">支付宝户名</text>
				<input type="text" class="input" placeholder="请输入支付宝户名" placeholder-style="color:#BBBBBB;font-size:28rpx" name="aliaccountname" :value="info.aliaccountname"/>
			</view>
			<view class="form-item">
		...
		<view class="form">
			<view class="form-item">
					...
					<input type="text" class="input" placeholder="请输入开户行" name="bankname" :value="info.bankname" placeholder-style="color:#BBBBBB;font-size:28rpx"/>
			</view>
			<view class="form-item">
...
export default {
  ...
  methods: {
		getdata: function () {
			var that = this;
			that.loading = true;
			app.get('ApiAdminFinance/txset', {}, function (data) {
				that.loading = false;
				that.info = data.info;
				that.loaded();
			});
    ...
    formSubmit: function (e) {
      ...
      //}
			app.showLoading('提交中');
      app.post("ApiAdminFinance/txset", {bankname:bankname,bankcarduser:bankcarduser,bankcardnum:bankcardnum,weixin:weixin,aliaccount:aliaccount,aliaccountname:aliaccountname,code:code}, function (data) {
