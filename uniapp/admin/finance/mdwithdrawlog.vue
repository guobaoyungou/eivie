			...
			<view v-for="(item, index) in datalist" :key="index" class="item">
				...
				<view class="f3">
						...
						<block v-if="item.status==4">
							...
							<view class="t1" v-else-if="item.state=='FAIL'">转账失败</view>
  ...
  methods: {
    getdata: function (loadmore) {
      ...
      var st = that.st;
			...
			that.nomore = false;
			that.loading = true;
      app.post('ApiAdminFinance/mdwithdrawlog', {st: st,pagenum: pagenum}, function (res) {
				that.loading = false;
        var data = res.data;
        if (pagenum == 1) {
					that.datalist = data;
    ...
    async confirm_shoukuan(id){
      var that = this;
      var a = await that.shoukuan(id,'mendian_withdrawlog','');
      console.log(a);
      that.getdata();
    }
  }
...
.content .item .f3 .btn1{height:60rpx;line-height:60rpx;color:#fff;border-radius:3px;text-align:center;padding: 0 15rpx;float: right;font-size: 14px;margin-left: 10rpx}
