		...
		<form @submit="subform">
			<view class="form-box">
				<view class="form-item">
					<view class="f1">餐桌分类名称<text style="color:red"> *</text></view>
					<view class="f2"><input type="text" name="name" :value="info.name" placeholder="请填写餐桌分类名称" placeholder-style="color:#888"></input></view>
				</view>

<!--				<view class="form-item">-->
<!--					<view class="f1">服务费</view>-->
<!--					<view class="f2"><input type="text" name="service_fee" :value="info.service_fee" placeholder="" placeholder-style="color:#888"></input></view>-->
<!--				</view>-->
				<view class="form-item">
					<view class="f1">最低消费</view>
					<view class="f2"><input type="text" name="limit_fee" :value="info.limit_fee" placeholder="请输入最低消费" placeholder-style="color:#888"></input></view>
				</view>
				<view class="form-item">
					<view class="f1">预定费</view>
					<view class="f2"><input type="text" name="booking_fee" :value="info.booking_fee" placeholder="请输入预定费" placeholder-style="color:#888"></input></view>
				</view>
				<view class="form-item">
					<view class="f1">座位数</view>
					<view class="f2"><input type="text" name="seat" :value="info.seat" placeholder="请输入座位数" placeholder-style="color:#888"></input></view>
				</view>
...
export default {
  ...
  methods: {
		getdata:function(){
			...
			that.loading = true;
			app.get('ApiAdminRestaurantTableCategory/edit',{id:id}, function (res) {
				that.loading = false;
				that.info = res.info;
    ...
    subform: function (e) {
      ...
      var formdata = e.detail.value;
			if(formdata.name == '') {
				app.error('请填写分类名');return;
			}

      var id = that.opt.id ? that.opt.id : '';
      app.post('ApiAdminRestaurantTableCategory/save', {id:id,info:formdata}, function (res) {
        if (res.status == 0) {
          app.error(res.msg);
        } else {
          app.success(res.msg);
          setTimeout(function () {
            app.goto('tableCategory', 'redirect');
          }, 1000);
        }
      });
    },
		todel: function (e) {
		  var that = this;
		  var id = that.opt.id ? that.opt.id : '';
		  app.confirm('确定要删除吗?分类下餐桌也会删除', function () {
		    app.post('ApiAdminRestaurantTableCategory/del', {id: id}, function (res) {
		      if (res.status == 1) {
		        app.success(res.msg);
		        app.goback(true)
		      } else {
