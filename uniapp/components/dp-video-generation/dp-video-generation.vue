			...
			<view class="fresh-waterfall-col fresh-waterfall-left">
				...
				>
					<view class="fresh-card-cover">
						...
						<!-- 视频播放图标覆盖层 -->
						<view class="fresh-card-play-icon">
							<text class="fresh-card-play-triangle">▶</text>
						</view>
						<!-- overlay按钮位置 -->
	...
	export default {
		...
		methods: {
			...
			getCardCover: function(item) {
				// 视频模板优先使用 gif_cover（Animated WebP 动态预览）
				if (item.gif_cover && item.gif_cover !== '') return item.gif_cover;
			...
			onFreshCardTap: function(item) {
				if (!this.freshBtnVisible) {
					...
					app.goto('/ailvpai/create?type=2&template_id=' + templateId);
				}
			},
