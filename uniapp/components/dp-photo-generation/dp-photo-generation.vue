...
var app = getApp();
	export default {
		...
		computed: {
			// 封面比例，默认1:1
			coverRatio() {
				return this.params.cover_ratio || '1:1';
			},
			// 封面圆角，默认8rpx
			coverRadius() {
		...
		methods: {
			...
			onMakeSame(index) {
				...
				that.currentTemplateId = templateId;
				// 获取模板详情
				app.showLoading('加载中');
