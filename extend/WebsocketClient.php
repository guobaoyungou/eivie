...
class WebsocketClient{
    ...
    public function send($data){
        //打开socket
        $sock = fsockopen($this->ip, $this->port, $errno, $errstr, 1);
		stream_set_timeout($sock, 1);  // 2秒后为超时
        //握手
        if(!fwrite($sock, $this->getHead($data) ) ) \think\facade\Log::write('websocket create error:'.$errno.':'.$errstr);
        $headers = fread($sock, 2000);
        //发送数据
        if(!fwrite($sock, self::hybi10Encode($data))) \think\facade\Log::write('websocket send error:'.$errno.':'.$errstr);
