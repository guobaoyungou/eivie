...
ALTER TABLE `ddwx_ai_travel_photo_synthesis_setting`
ADD COLUMN `prompt_rewrite_template` text COMMENT '提示词改写模板（支持{自动标签性别}{自动标签年龄}{模板绑定模型}等系统变量）' AFTER `prompt_rewrite_model`;
...
SELECT 'system_variables', '{"自动标签性别":"人像自动识别的性别（Male→男性 / Female→女性）","自动标签年龄":"人像自动识别的年龄段（如：职场青年、中年主力等）","模板绑定模型":"当前合成模板绑定的AI模型名称（如：stable-diffusion-xl）"}'
