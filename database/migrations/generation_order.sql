...
CREATE TABLE IF NOT EXISTS `ddwx_generation_order` (
  ...
  `ref_images` text DEFAULT NULL COMMENT '参考图URL列表（JSON数组）',
  `template_snapshot` text DEFAULT NULL COMMENT '下单时场景模板快照（JSON）',
  `model_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '模型ID（模型直选时使用）',
